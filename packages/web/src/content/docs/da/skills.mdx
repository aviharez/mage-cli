...
Agentferdigheter lar Mage oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Mage søker etter disse stedene:

- Prosjektkonfigurasjon: `.mage/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/mage/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Mage opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.mage/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.mage/skills/git-release/SKILL.md` slik:
...
Mage viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `mage.json`:
...
**For innebygde agenter** (i `mage.json`):
