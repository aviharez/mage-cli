---
title: Konfiguration
description: Ved at bruge Mage JSON konfig.
---

Du kan konfigurere Mage ved hjælp af en JSON-konfigurationsfil.

---

## Format

Mage understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/mage`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/mage/mage.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`MAGE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`mage.json` i projekt) - projektspecifikke indstillinger
5. **`.mage` mapper** - agent, kommandoer, plugins
6. **Inline config** (`MAGE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.mage` og `~/.config/mage` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/mage`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Mage-konfiguration i `~/.config/mage/mage.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `mage.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Mage starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `MAGE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `MAGE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.mage` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.mage` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `mage serve` og `opencode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Mage-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Mage-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Mage at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Mage-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/mage/agents/` eller `.mage/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Mage falde tilbage til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/mage/commands/` eller `.mage/commands/`. [Learn more here](/docs/commands).
...
Mage vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard mage **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Mage med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.mage/plugins/` eller `~/.config/mage/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Mage til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
