---
title: Plugins
description: Skriv dine egne plugins for at udvide Mage.
---

Plugins giver dig mulighed for at udvide Mage ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Mages standardadfærd.
...
- `.mage/plugins/` - Plugins på projektniveau
...
1. Global konfiguration (`~/.config/mage/mage.json`)
2. Projektkonfiguration (`mage.json`)
3. Global plugin-mappe (`~/.config/mage/plugins/`)
4. Projekt plugin bibliotek (`.mage/plugins/`)
...
Mage kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En mage SDK klient til interaktion med AI.
...
:::note
Hvis du bruger Mage desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå mage i at læse `.env` filer:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som mage kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for mage sammen med indbyggede værktøjer.
