...
Her er alle de innebygde verktøyene som er tilgængelige i Mage.
...
:::note
Dette verktøyet er bare tilgængelig når `MAGE_EXPERIMENTAL_LSP_TOOL=true` (eller `MAGE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Mage-leverandøren eller når miljøvariabelen `MAGE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
