---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

Mage bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Mage-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i Mage-konfigurationen.
...
Mage Zen er en liste over modeller leveret af Mage-teamet, som er blevet testet og verificeret til at fungere godt med Mage. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Mage Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg mage og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i Mage og er helt valgfrit at bruge.

---

## Mage Go

Mage Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Mage-teamet, som er testet og verificeret til at fungere godt med Mage.

1. Kør kommandoen `/connect` i TUI, vælg `Mage Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Mage:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `mage.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Mage understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere mage til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at mage skal fungere korrekt.
   :::
...
gemme tokenet i mage auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Mage bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Mage
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`mage.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Mage:
...
Sådan bruger du Google Vertex AI med Mage:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Mage-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Mage-samtale opført som en separat session.
...
Du kan konfigurere mage til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere mage til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere mage til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Mage. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Mage:
...
3. Kopier API-nøglen til brug i Mage.
...
6. **Vigtigt**: Før du bruger skymodeller i Mage, skal du hente modelinformationen lokalt:
...
Mage Zen er en liste over testede og verificerede modeller leveret af Mage-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Mage Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Mage Zen**.

   ```txt
   /connect
   ```

3. Indtast Mage API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Mage:
...
4. Opret eller opdater `mage.json`-filen i projektkataloget:
...
`limit`-felterne lader Mage forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `mage auth list` for at se om legitimationsoplysningerne
