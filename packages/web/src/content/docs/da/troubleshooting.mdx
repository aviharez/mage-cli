...
For at fejlfinde problemer med Mage, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
mage gemmer sessionsdata og andre applikationsdata på disken på:
...
Mage Desktop kører en lokal Mage-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Mage`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/mage/mage.jsonc` (eller `~/.config/mage/mage.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/mage.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Mage kan genopbygge den.

1. Afslut Mage Desktop helt.
2. Slet cache-mappen:
...
3. Start Mage Desktop på ny.

---

### Løs problemer med serverforbindelse

Mage Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `mage.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `MAGE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `MAGE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Mage Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Mage.

---

### Meddelelser vises ikke

Mage Desktop viser kun systemvarsler når:

- varsler er aktiveret for Mage i dine OS-indstillinger, og
...
1. Afslut Mage Desktop.
2. Find og slet disse filer (de findes i Mage Desktop-appens datamappe):
...
Hvis du oplever problemer med Mage:
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. mage installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start mage på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge mage til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
mage vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
