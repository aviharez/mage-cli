---
title: プラグイン
description: Mage を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Mage を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Mage のデフォルトの動作を変更したりできます。
...
- `.mage/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/mage/plugins/` - グローバルプラグイン
...
1. グローバル設定 (`~/.config/mage/mage.json`)
2. プロジェクト設定 (`mage.json`)
3. グローバルプラグインディレクトリ (`~/.config/mage/plugins/`)
4. プロジェクトプラグインディレクトリ (`.mage/plugins/`)
...
Mage は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Mage SDK クライアント。
...
ここでは、Mage を拡張するために使用できるプラグインの例をいくつか示します。
...
プラグインは Mage にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Mage が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Mage で使用できます。
