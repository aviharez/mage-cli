---
title: Mage Zen
description: Mage によって提供されるモデルの厳選されたリスト。
---
...
Mage Zen は、Mage チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Mage のほかのプロバイダーと同じように動作します。Mage Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Mage を使うために必須ではありません。
...
:::tip
Mage でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Mage Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Mage Zen は、Mage のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Mage Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Mage Zen を選んで API キーを貼り付けます。
...
Mage 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
- DeepSeek V4 Flash Free は期間限定で Mage で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で Mage で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Super Free は期間限定で Mage で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Mage で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Mage では常にほかのプロバイダーも使えるようにすること。
