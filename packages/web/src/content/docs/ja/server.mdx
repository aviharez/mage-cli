---
title: サーバー
description: HTTP 経由で Mage サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`mage serve` コマンドは、Mage クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
HTTP 基本認証でサーバーを保護するには、`MAGE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `mage` になるか、`MAGE_SERVER_USERNAME` を設定してオーバーライドします。これは、`mage serve` と `opencode web` の両方に当てはまります。
...
`mage` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Mage サーバーを使用して、プログラムで Mage と対話します。
:::

`mage serve` を実行してスタンドアロンサーバーを起動できます。すでに Mage TUI を実行している場合でも、`mage serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Mage [IDE](/docs/ide) プラグインによって使用されます。
