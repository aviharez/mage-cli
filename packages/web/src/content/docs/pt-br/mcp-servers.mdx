...
Você pode adicionar ferramentas externas ao mage usando o _Model Context Protocol_, ou MCP. O mage suporta servidores locais e remotos.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/mage`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O mage lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o mage irá:
...
Se o servidor requer autenticação, o mage solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `opencode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o mage armazenará os tokens de forma segura em `~/.local/share/opencode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no opencode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do mage como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o mage à sua conta do Sentry.
