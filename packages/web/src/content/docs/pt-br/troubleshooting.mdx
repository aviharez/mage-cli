...
O mage armazena dados de sessão e outros dados do aplicativo no disco em:
...
O mage Desktop executa um servidor mage local (o sidecar `opencode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Mage` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/mage.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\mage.jsonc`
...
O mage também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o mage possa reconstruí-lo.

1. Saia completamente do mage Desktop.
...
3. Reinicie o mage Desktop.

---

### Corrigindo problemas de conexão com o servidor

O mage Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `mage.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `MAGE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `MAGE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o mage Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.
...
O mage Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o mage nas configurações do seu sistema operacional, e
...
1. Saia do mage Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo mage Desktop):
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O mage instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
2. Reinicie o mage para reinstalar os pacotes de provedores mais recentes

Isso forçará o mage a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O mage detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
