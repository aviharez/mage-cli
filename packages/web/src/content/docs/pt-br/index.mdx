...
Para usar o mage no seu terminal, você precisará de:
...
A maneira mais fácil de instalar o mage é através do script de instalação.
...
- **Usando Homebrew no macOS e Linux**
  ...
  > Recomendamos usar o tap do mage para as versões mais atualizadas. A fórmula oficial `brew install opencode` é mantida pela equipe do Homebrew e é atualizada com menos frequência.
...
O suporte para instalar o mage no Windows usando Bun está atualmente em andamento.
...
Se você é novo no uso de provedores de LLM, recomendamos usar o [Mage Zen](/docs/zen).
É uma lista selecionada de modelos que foram testados e verificados pela equipe do opencode.

1. Execute o comando `/connect` no TUI, selecione mage e acesse [opencode.ai/auth](https://opencode.ai/auth).
...
Em seguida, inicialize o mage para o projeto executando o seguinte comando.

```bash frame="none"
/init
```

Isso fará com que o mage analise seu projeto e crie um arquivo `AGENTS.md` na raiz do projeto.

:::tip
Você deve commitar o arquivo `AGENTS.md` do seu projeto no Git.
:::

Isso ajuda o mage a entender a estrutura do projeto e os padrões de codificação utilizados.

---

## Uso

Agora você está pronto para usar o mage para trabalhar em seu projeto. Sinta-se à vontade para perguntar qualquer coisa!
...
Você pode pedir ao mage para explicar a base de código para você.
...
Você pode pedir ao mage para adicionar novos recursos ao seu projeto. Embora primeiro recomendemos pedir para ele criar um plano.

1. **Criar um plano**

   O mage tem um _Modo de Plano_ que desabilita sua capacidade de fazer alterações e, em vez disso, sugere _como_ implementará o recurso.
   ...
   Você quer dar ao mage detalhes suficientes para entender o que você deseja. Ajuda conversar com ele como se você estivesse falando com um desenvolvedor júnior da sua equipe.

   :::tip
   Dê ao mage bastante contexto e exemplos para ajudá-lo a entender o que você deseja.
   :::
...
2. **Iterar sobre o plano**
   ...
   O mage pode escanear qualquer imagem que você fornecer e adicioná-la ao prompt. Você pode fazer isso arrastando e soltando uma imagem no terminal.
...
Para alterações mais simples, você pode pedir ao mage para construí-las diretamente sem precisar revisar o plano primeiro.
...
Você quer ter certeza de fornecer uma boa quantidade de detalhes para que o mage faça as alterações corretas.

---

### Desfazendo alterações

Vamos supor que você peça ao mage para fazer algumas alterações.
...
O mage agora reverterá as alterações que você fez e mostrará sua mensagem original novamente.

```txt frame="none" "@packages/functions/src/api/index.ts"
Can you refactor the function in @packages/functions/src/api/index.ts?
```

A partir daqui, você pode ajustar o prompt e pedir ao mage para tentar novamente.
...
As conversas que você tem com o mage podem ser [compartilhadas com sua equipe](/docs/share).
