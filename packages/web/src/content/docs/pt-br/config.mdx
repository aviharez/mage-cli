...
Você pode configurar o mage usando um arquivo de configuração JSON.

---

## Formato

O mage suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/mage`) - padrões organizacionais
2. **Configuração global** (`~/.config/mage/mage.json`) - preferências do usuário
3. **Configuração personalizada** (`MAGE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`mage.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.mage`** - agentes, comandos, plugins
6. **Configuração inline** (`MAGE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.mage` e `~/.config/mage` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/mage`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do mage em `~/.config/mage/mage.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/mage/tui.json`.
...
Adicione `mage.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o mage é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `MAGE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `MAGE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.mage`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.mage`, então ele **pode substituir** suas configurações.
...
Use `MAGE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `mage.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `mage serve` e `opencode web` através da opção `server`.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do mage através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o mage tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/mage/agents/` ou `.mage/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o mage recuará para `"build"` com um aviso.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/mage/commands/` ou `.mage/commands/`. [Saiba mais aqui](/docs/commands).
...
O mage fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o mage **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o mage com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.mage/plugins/` ou `~/.config/mage/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o mage para usar apenas provedores específicos, em vez de desabilitá-los um a um.
