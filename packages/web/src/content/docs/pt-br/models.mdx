...
O mage usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
Aqui, o ID completo é `provider_id/model_id`. Por exemplo, se você estiver usando [Mage Zen](/docs/zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muitos modelos suportam várias variantes com diferentes configurações. O mage vem com variantes padrão integradas para provedores populares.

### Variantes integradas

O mage vem com variantes padrão para muitos provedores:
...
Quando o mage é iniciado, ele verifica modelos na seguinte ordem de prioridade:
