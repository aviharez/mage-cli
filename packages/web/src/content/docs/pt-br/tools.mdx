...
As ferramentas permitem que o LLM execute ações em sua base de código. O mage vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
:::note
Esta ferramenta está disponível apenas quando `MAGE_EXPERIMENTAL_LSP_TOOL=true` (ou `MAGE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor mage ou quando a variável de ambiente `MAGE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
