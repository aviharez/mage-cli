...
O mage fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar mage inicia o TUI para o diretório atual.
...
Isso é separado do `mage.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o Mage espera depois da leader key. O padrão é `2000`.
...
Use `MAGE_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o Mage espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
