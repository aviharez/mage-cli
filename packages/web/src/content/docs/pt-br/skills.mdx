...
As habilidades do agente permitem que o mage descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O mage pesquisa nesses locais:

- Configuração do projeto: `.mage/skills/<name>/SKILL.md`
- Configuração global: `~/.config/mage/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o mage sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.mage/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.mage/skills/git-release/SKILL.md` assim:
...
O mage lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `mage.json`:
...
**Para agentes embutidos** (em `mage.json`):
