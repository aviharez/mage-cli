---
title: GitLab
description: Use o mage em problemas e solicitações de mesclagem do GitLab.
---

O mage se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o mage será executado em seus runners do GitLab.

---

## GitLab CI

O mage funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para mage — [nagyv/gitlab-opencode](https://gitlab.com/nagyv/gitlab-opencode).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o mage com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do opencode.
- **Configuração mínima**: O componente CI configura o mage em segundo plano, você só precisa criar a configuração do mage e o prompt inicial.
...
1. Armazene seu JSON de autenticação do mage como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
   ...
   include:
     - component: $CI_SERVER_FQDN/nagyv/gitlab-opencode/opencode@2
       inputs:
         ...
         auth_json: $MAGE_AUTH_JSON # O nome da variável para seu JSON de autenticação do Mage
...
O mage se integra ao seu fluxo de trabalho do GitLab.
Mencione `@opencode` em um comentário, e o mage executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao mage para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O mage é executado em seus runners do GitLab.

---

### Configuração

O mage é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
6.  Crie um arquivo de configuração de fluxo, aqui está um exemplo:
    ...
    commands:
      ...
      - echo $GITLAB_HOST
      - echo "Criando configuração de auth do Mage"
...
Aqui estão alguns exemplos de como você pode usar o mage no GitLab.
...
- **Explicar um problema**
  ...
  O mage lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O mage criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O mage revisará a solicitação de mesclagem e fornecerá feedback.
