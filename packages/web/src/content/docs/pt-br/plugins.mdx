...
Plugins permitem que você estenda o mage conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do opencode.
...
- `.mage/plugins/` - Plugins em nível de projeto
- `~/.config/mage/plugins/` - Plugins globais
...
1. Configuração global (`~/.config/mage/mage.json`)
2. Configuração do projeto (`mage.json`)
3. Diretório de plugins global (`~/.config/mage/plugins/`)
4. Diretório de plugins do projeto (`.mage/plugins/`)
...
O mage executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do mage para interagir com a AI.
...
Impeça o mage de ler arquivos `.env`:
...
O helper `tool` cria uma ferramenta personalizada que o mage pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o mage junto com as ferramentas integradas.
