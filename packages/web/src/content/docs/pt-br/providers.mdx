...
mage usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
Mage Zen é uma lista de modelos fornecidos pela equipe do mage que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Mage Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Mage Zen` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Funciona como qualquer outro provedor no mage e é completamente opcional.

---

## Mage Go

Mage Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do mage que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `Mage Go` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `mage.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no mage não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o mage para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o mage funcione corretamente.
   :::
...
mage usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
...
arquivo `mage.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `mage.json`:
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do mage como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do mage listada como uma sessão separada.
...
Você pode configurar o mage para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o mage para usar modelos locais através do LM Studio.
...
Você pode configurar o mage para usar modelos locais através do Ollama.
...
Mage Zen é uma lista de modelos testados e verificados fornecidos pela equipe do opencode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Mage Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Mage Zen**.
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no mage.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `mage.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o mage entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `mage auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do mage e:
