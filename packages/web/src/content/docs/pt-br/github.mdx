---
title: GitHub
description: Use o mage em problemas e pull-requests do GitHub.
---

O mage integra-se ao seu fluxo de trabalho do GitHub. Mencione `/opencode` ou `/oc` em seu comentário, e o mage executará tarefas dentro do seu runner do GitHub Actions.

---

## Recursos

- **Triagem de problemas**: Peça ao mage para analisar um problema e explicá-lo para você.
- **Corrigir e implementar**: Peça ao mage para corrigir um problema ou implementar um recurso. E ele trabalhará em um novo branch e enviará um PR com todas as alterações.
- **Seguro**: O mage é executado dentro dos runners do seu GitHub.
...
- `prompt`: Prompt personalizado opcional para substituir o comportamento padrão. Use isso para personalizar como o mage processa solicitações.
- `token`: Token de acesso do GitHub opcional para realizar operações como criar comentários, confirmar alterações e abrir pull requests. Por padrão, o mage usa o token de acesso da instalação do aplicativo GitHub opencode, então commits, comentários e pull requests aparecem como se fossem da aplicação.
...
O mage pode ser acionado pelos seguintes eventos do GitHub:

| Tipo de Evento                | Acionado Por                                        | Detalhes                                                                                                                               |
| ----------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Comentário em um problema ou PR                     | Mencione `/opencode` ou `/oc` em seu comentário. O mage lê o contexto e pode criar branches, abrir PRs ou responder.               |
| `pull_request_review_comment` | Comentário em linhas de código específicas em um PR | Mencione `/opencode` ou `/oc` enquanto revisa o código. O mage recebe o caminho do arquivo, números das linhas e contexto do diff. |
| `issues`                      | Problema aberto ou editado                          | Aciona automaticamente o mage quando problemas são criados ou modificados. Requer entrada de `prompt`.                             |
| `pull_request`                | PR aberto ou atualizado                             | Aciona automaticamente o mage quando PRs são abertos, sincronizados ou reabertos. Útil para revisões automatizadas.                |
| `schedule`                    | Cron baseado em agendamento                         | Execute o mage em um cronograma. Requer entrada de `prompt`. A saída vai para logs e PRs (sem problema para comentar).             |
| `workflow_dispatch`           | Acionamento manual pela interface do GitHub         | Acione o mage sob demanda através da aba Ações. Requer entrada de `prompt`. A saída vai para logs e PRs.                           |

### Exemplo de Agendamento

Execute o mage em um cronograma para realizar tarefas automatizadas:
...
Para eventos de `pull_request`, se nenhum `prompt` for fornecido, o mage padrão será revisar o pull request.
...
Substitua o prompt padrão para personalizar o comportamento do mage para seu fluxo de trabalho.
...
Aqui estão alguns exemplos de como você pode usar o mage no GitHub.
...
- **Explicar um problema**
  ...
  O mage lerá toda a conversa, incluindo todos os comentários, e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  E o mage criará um novo branch, implementará as alterações e abrirá um PR com as mudanças.
...
- **Revisar PRs e fazer alterações**
  ...
  O mage implementará a alteração solicitada e a confirmará no mesmo PR.

- **Revisar linhas de código específicas**

  Deixe um comentário diretamente nas linhas de código na aba "Files" do PR. O mage detecta automaticamente o arquivo, os números das linhas e o contexto do diff para fornecer respostas precisas.
  ...
  Ao comentar sobre linhas específicas, o mage recebe:
