---
title: Servidor
description: Interaja com o servidor mage via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `mage serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente mage pode usar.
...
```bash
mage serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
mage serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `MAGE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `mage`, ou defina `MAGE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `mage serve` quanto ao `opencode web`.

```bash
MAGE_SERVER_PASSWORD=your-password mage serve
```
...
Quando você executa `mage`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor mage para interagir com o mage programaticamente.
:::

Essa arquitetura permite que o mage suporte múltiplos clientes e permite que você interaja com o mage programaticamente.

Você pode executar `mage serve` para iniciar um servidor autônomo. Se você tiver o TUI do mage em execução, `mage serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do mage [IDE](/docs/ide).
...
O servidor mage expõe as seguintes APIs.
