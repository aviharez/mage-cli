---
title: IDE
description: A extensão mage para VS Code, Cursor e outros IDEs
---

O mage integra-se com VS Code, Cursor ou qualquer IDE que suporte um terminal. Basta executar `mage` no terminal para começar.

---

## Uso

- **Lançamento Rápido**: Use `Cmd+Esc` (Mac) ou `Ctrl+Esc` (Windows/Linux) para abrir o mage em uma visualização de terminal dividido, ou focar em uma sessão de terminal existente se uma já estiver em execução.
- **Nova Sessão**: Use `Cmd+Shift+Esc` (Mac) ou `Ctrl+Shift+Esc` (Windows/Linux) para iniciar uma nova sessão de terminal opencode, mesmo que uma já esteja aberta. Você também pode clicar no botão mage na interface.
...
Para instalar o mage no VS Code e forks populares como Cursor, Windsurf, VSCodium:

1. Abra o VS Code
2. Abra o terminal integrado
3. Execute `mage` - a extensão será instalada automaticamente
...
- Certifique-se de que você está executando `mage` no terminal integrado.
