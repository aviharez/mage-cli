---
title: Web
description: Usando o mage no seu navegador.
---

O mage pode ser executado como uma aplicação web no seu navegador, proporcionando a mesma poderosa experiência de codificação com IA sem precisar de um terminal.
...
Isso inicia um servidor local em `127.0.0.1` com uma porta aleatória disponível e abre automaticamente o mage no seu navegador padrão.

:::caution
Se `MAGE_SERVER_PASSWORD` não estiver definido, o servidor ficará sem segurança. Isso é aceitável para uso local, mas deve ser configurado para acesso à rede.
:::
...
Por padrão, o mage escolhe uma porta disponível. Você pode especificar uma porta:
...
Por padrão, o servidor se vincula a `127.0.0.1` (apenas localhost). Para tornar o mage acessível na sua rede:

```bash
mage web --hostname 0.0.0.0
```

Ao usar `0.0.0.0`, o mage exibirá endereços locais e de rede:
...
Para proteger o acesso, defina uma senha usando a variável de ambiente `MAGE_SERVER_PASSWORD`:

```bash
MAGE_SERVER_PASSWORD=secret mage web
```

O nome de usuário padrão é `mage`, mas pode ser alterado com `MAGE_SERVER_USERNAME`.
...
Você também pode configurar as configurações do servidor no seu arquivo de configuração `mage.json`:
