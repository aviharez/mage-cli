...
Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o mage programaticamente.
...
A CLI do mage também possui os seguintes comandos.
...
Anexe um terminal a um servidor backend do mage já em execução, iniciado via comandos `serve` ou `web`.

```bash
mage attach [url]
```

Isso permite usar o TUI com um backend mage remoto. Por exemplo:
...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `MAGE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `MAGE_SERVER_USERNAME` ou `mage`) |
...
O mage é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `mage auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.

```bash
mage auth login
```

Quando o mage é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Execute o mage em modo não interativo passando um prompt diretamente.
...
```bash "opencode run"
mage run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `mage serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
mage run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor mage em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `MAGE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `MAGE_SERVER_USERNAME` ou `mage`) |
...
Inicie um servidor mage sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
mage serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do mage sem a interface TUI. Defina `MAGE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `mage`).
...
Inicie um servidor mage sem cabeça com uma interface web.

```bash
mage web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o mage através de uma interface web. Defina `MAGE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `mage`).
...
Desinstale o mage e remova todos os arquivos relacionados.
...
Atualiza o mage para a versão mais recente ou uma versão específica.
...
A CLI do mage aceita as seguintes flags globais.
...
O mage pode ser configurado usando variáveis de ambiente.

| Variável                              | Tipo    | Descrição                                                             |
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `MAGE_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `MAGE_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `MAGE_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `MAGE_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `MAGE_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `MAGE_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `MAGE_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `MAGE_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `MAGE_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `MAGE_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `MAGE_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `MAGE_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `MAGE_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `MAGE_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `MAGE_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `MAGE_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `MAGE_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `MAGE_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `MAGE_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `MAGE_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `MAGE_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `MAGE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `mage`) |
| `MAGE_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `MAGE_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `MAGE_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `MAGE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `MAGE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `MAGE_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `MAGE_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `MAGE_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `MAGE_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `MAGE_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `MAGE_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `MAGE_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `MAGE_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `MAGE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `MAGE_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `MAGE_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `MAGE_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `MAGE_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `MAGE_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
