...
O mage usa a configuração `permission` para decidir se uma determinada ação deve ser executada automaticamente, solicitar sua aprovação ou ser bloqueada.
...
Use `external_directory` para permitir chamadas de ferramentas que tocam em caminhos fora do diretório de trabalho onde o mage foi iniciado. Isso se aplica a qualquer ferramenta que aceite um caminho como entrada (por exemplo, `read`, `edit`, `glob`, `grep` e muitos comandos `bash`).
...
As permissões do mage são indexadas pelo nome da ferramenta, além de alguns guardas de segurança:
...
Se você não especificar nada, o mage começa com padrões permissivos:
...
Quando o mage solicita aprovação, a interface oferece três resultados:
