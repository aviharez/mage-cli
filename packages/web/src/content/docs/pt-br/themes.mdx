...
Por padrão, o mage usa nosso próprio tema `mage`.
...
O mage vem com vários temas embutidos.
...
- Querem que o mage corresponda à aparência do seu terminal
...
O mage suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/mage/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.mage/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.mage/themes/*.json`
