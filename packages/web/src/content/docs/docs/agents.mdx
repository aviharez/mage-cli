...
There are two types of agents in Mage; primary agents and subagents.
...
Mage comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Mage comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Mage comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Mage's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `mage.json` config file:
...
If no temperature is specified, Mage uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Mage config and the project specific config.
...
The model ID in your Mage config uses the format `provider/model-id`. For example, if you're using [Mage Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
