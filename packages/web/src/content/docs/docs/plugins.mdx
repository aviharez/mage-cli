---
title: Plugins
description: Write your own plugins to extend Mage.
---

Plugins allow you to extend Mage by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Mage's default behavior.
...
- `.mage/plugins/` - Project-level plugins
- `~/.config/mage/plugins/` - Global plugins
...
3. Global plugin directory (`~/.config/mage/plugins/`)
4. Project plugin directory (`.mage/plugins/`)
...
Mage runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An mage SDK client for interacting with the AI.
...
:::note
If you’re using the Mage desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent mage from reading `.env` files:
...
The `tool` helper creates a custom tool that mage can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to mage alongside built-in tools.
