---
title: Config
description: Using the Mage JSON config.
eyebrow: Docs · Configure
---

You can configure Mage using a JSON config file.

---

## Format

Mage supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="mage.jsonc"
{
...
1. **Remote config** (from `.well-known/mage`) - organizational defaults
2. **Global config** (`~/.config/mage/mage.json`) - user preferences
3. **Custom config** (`MAGE_CONFIG` env var) - custom overrides
4. **Project config** (`mage.json` in project) - project-specific settings
5. **`.mage` directories** - agents, commands, plugins
6. **Inline config** (`MAGE_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.mage` and `~/.config/mage` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/mage` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/mage"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Mage config in `~/.config/mage/mage.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/mage/tui.json`.
...
Add `mage.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Mage starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `MAGE_CONFIG` environment variable.

```bash
export MAGE_CONFIG=/path/to/my/custom-config.json
mage run "Hello world"
```
...
Specify a custom config directory using the `MAGE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.mage` directory, and should
follow the same structure.

```bash
export MAGE_CONFIG_DIR=/path/to/my/config-directory
mage run "Hello world"
```

The custom directory is loaded after the global config and `.mage` directories, so it **can override** their settings.
...
Drop an `mage.json` or `mage.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/opencode/` |
| Linux    | `/etc/opencode/`                         |
| Windows  | `%ProgramData%\mage`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Mage reads managed preferences from the `id.mbb.mage.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Mage checks these paths:

1. `/Library/Managed Preferences/<user>/id.mbb.mage.managed.plist`
2. `/Library/Managed Preferences/id.mbb.mage.managed.plist`

The plist keys map directly to `mage.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `id.mbb.mage.managed` PayloadType. The Mage config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>id.mbb.mage.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.mage.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.mage</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Use `MAGE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `mage.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `mage serve` and `opencode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Mage server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `mage.local`. Useful for running multiple instances on the same network.
...
If not specified, Mage will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Mage config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Mage tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Mage normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Mage omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/mage/agents/` or `.mage/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Mage will fall back to `"build"` with a warning.
...
You can also define commands using markdown files in `~/.config/mage/commands/` or `.mage/commands/`. [Learn more here](/docs/commands).
...
Mage uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Mage will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, mage **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Mage with custom tools, hooks, and integrations.

Place plugin files in `.mage/plugins/` or `~/.config/mage/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Mage to only use specific providers rather than disabling them one by one.
