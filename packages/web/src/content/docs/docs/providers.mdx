---
title: Providers
description: Using any LLM provider in Mage.
eyebrow: Docs · Configure
---

import config from "../../../../config.mjs"
export const console = config.console

Mage uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Mage config.
...
You can customize the providers through the `provider` section in your Mage
config.
...
Mage Zen is a list of models provided by the Mage team that have been
tested and verified to work well with Mage. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Mage Zen.
:::

1. Run the `/connect` command in the TUI, select `Mage Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Mage and is completely optional to use.

---

## Mage Go

Mage Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Mage team that have been
tested and verified to work well with Mage.

1. Run the `/connect` command in the TUI, select `Mage Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Mage:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `mage.json`:

   ```json title="mage.json"
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
Mage. Anthropic explicitly prohibits this.

Previous versions of Mage came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Mage with zero setup:
...
You can configure mage to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="mage.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for mage to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your mage config.
...
Mage supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Mage uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Mage. Sign in and approve.

   :::note
   Mage requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Mage surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Mage forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Mage is experimental. Features, configuration, and
behavior may change in future releases.
:::

Mage integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in mage auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Mage uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Mage
to only use your own GitLab-hosted instance, add the following to your
`mage.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Mage will:
...
4. Route tool execution requests through Mage's permission-gated tool system
...
To use Google Vertex AI with Mage:
...
```jsonc title="~/.config/mage/mage.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Mage conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Mage conversation listed as a separate session.
...
You can configure mage to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="mage.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure mage to use local models through LM Studio.

```json title="mage.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure mage to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Mage. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="mage.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
3. Copy the API key for use in Mage.
...
6. **Important**: Before using cloud models in Mage, you must pull the model information locally:
...
Mage Zen is a list of tested and verified models provided by the Mage team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Mage Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Mage Zen**.

   ```txt
   /connect
   ```

3. Enter your Mage API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your mage config.
...
5. You can also customize them through your mage config. Here's an example of specifying a provider
...
You can also customize models through your mage config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Mage opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Mage refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Mage is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Mage — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Mage prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Mage polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in mage.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `mage.json` file in your project directory:

   ```json title="mage.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="mage.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Mage to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `mage auth list` to see if the credentials
...
2. For custom providers, check the mage config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your mage config.
