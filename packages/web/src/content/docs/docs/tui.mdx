---
title: TUI
description: Using the Mage terminal user interface.
eyebrow: Docs · Usage
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Mage provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Mage starts the TUI for the current directory.
...
When using the Mage TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Mage. Allows you to select from available providers and add their API keys.
...
This is separate from `mage.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Mage waits after the leader key. Defaults to `2000`.
...
Use `MAGE_TUI_CONFIG` to load a custom TUI config path.
