---
title: Plugins
description: Écrivez vos propres plugins pour étendre Mage.
---

Les plugins vous permettent d'étendre Mage en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Mage.
...
- `.mage/plugins/` - Plugins au niveau du projet
- `~/.config/mage/plugins/` - Plugins globaux
...
1. Configuration globale (`~/.config/mage/mage.json`)
2. Configuration du projet (`mage.json`)
3. Répertoire global des plugins (`~/.config/mage/plugins/`)
4. Répertoire des plugins du projet (`.mage/plugins/`)
...
Mage exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK mage pour interagir avec l'IA.
...
:::note
Si vous utilisez l'application de bureau Mage, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez mage de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à mage :
...
L'assistant `tool` crée un outil personnalisé que mage peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour mage aux côtés des outils intégrés.
