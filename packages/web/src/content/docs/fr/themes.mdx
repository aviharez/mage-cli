...
Avec Mage, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Mage utilise notre propre thème `mage`.
...
Mage est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Mage corresponde à l'apparence de leur terminal
...
Mage prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/mage/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.mage/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.mage/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/mage/themes
vim ~/.config/mage/themes/my-theme.json
```
