---
title: Serveur
description: Interagissez avec le serveur mage via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `mage serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client mage peut utiliser.
...
Définissez `MAGE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `mage`, ou définissez `MAGE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `mage serve` et à `opencode web`.
...
Lorsque vous exécutez `mage`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur mage pour interagir avec mage par programmation.
:::

Cette architecture permet à mage de prendre en charge plusieurs clients et vous permet d'interagir avec mage par programme.

Vous pouvez exécuter `mage serve` pour démarrer un serveur autonome. Si vous avez le
mage TUI en cours d'exécution, `mage serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Mage [IDE](/docs/ide).
...
Le serveur mage expose les API suivantes.
