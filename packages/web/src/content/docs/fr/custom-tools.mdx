...
Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de mage comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.mage/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/mage/tools/`.
