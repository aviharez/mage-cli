...
Pour déboguer les problèmes avec Mage, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
mage stocke les données de session et autres données d'application sur le disque à l'emplacement :
...
Mage Desktop exécute un serveur Mage local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Mage` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/mage/mage.jsonc` (ou `~/.config/mage/mage.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/mage.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Mage puisse le reconstruire.

1. Quittez complètement Mage Desktop.
...
3. Redémarrez le bureau Mage.

---

### Résoudre les problèmes de connexion au serveur

Mage Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `mage.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `MAGE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `MAGE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Mage Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Mage.

---

### Les notifications ne s'affichent pas

Mage Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Mage dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Mage.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Mage Desktop) :
...
Si vous rencontrez des problèmes avec Mage :
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. mage installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
2. Redémarrez mage pour réinstaller les derniers packages du fournisseur

Cela forcera mage à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
mage détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
