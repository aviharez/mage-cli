...
Il existe deux types d'agents dans Mage ; agents primaires et sous-agents.
...
Mage est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Mage est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Mage est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `mage.json` :
...
Si aucune température n'est spécifiée, Mage utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Mage et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Mage utilise le format `provider/model-id`. Par exemple, si vous utilisez [Mage Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
