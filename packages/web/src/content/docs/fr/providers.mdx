---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

Mage utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Mage.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Mage
configuration.
...
Mage Zen est une liste de modèles fournis par l'équipe Mage qui ont été
testé et vérifié pour fonctionner correctement avec Mage. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Mage Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez mage et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Mage et son utilisation est totalement facultative.

---

## Mage Go

Mage Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Mage qui ont été
testé et vérifié pour fonctionner correctement avec Mage.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Mage Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec Mage :
...
Pour une configuration spécifique au projet ou persistante, utilisez `mage.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Mage n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer mage pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que mage fonctionne correctement.
   :::
...
Mage utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Mage
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `mage.json`. Il est également recommandé de désactiver le partage de session.
...
Pour utiliser Google Vertex AI avec Mage :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Mage en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Mage répertoriée comme une session distincte.
...
Vous pouvez configurer mage pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer mage pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer mage pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Mage. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Mage :
...
3. Copiez la clé API à utiliser dans Mage.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Mage, vous devez extraire les informations du modèle localement :
...
Mage Zen est une liste de modèles testés et vérifiés fournie par l'équipe Mage. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Mage Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Mage Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Mage API.
...
4. Créez ou mettez à jour votre fichier `mage.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Mage de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `mage auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration mage et :
