---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Mage.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Mage fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Mage démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Mage TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Mage. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `mage.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Mage attend après la leader key. Valeur par défaut : `2000`.
