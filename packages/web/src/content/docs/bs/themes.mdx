...
U Mage mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Mage koristi nasu `mage` temu.
...
Mage dolazi sa vise ugradenih tema.
...
- Zele da Mage odgovara izgledu njihovog terminala
...
Mage podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/mage/themes/*.json` ili `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.mage/themes/*.json`
4. **Trenutni radni direktorij** - `./.mage/themes/*.json`
