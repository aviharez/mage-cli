---
title: Server
description: Komunicirajte s mage serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `mage serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji mage klijent moze koristiti.
...
Postavite `MAGE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `mage`, ili postavite `MAGE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `mage serve` i za `opencode web`.
...
Kada pokrenete `mage`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite mage server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava mage podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `mage serve` da startate standalone server. Ako je mage TUI vec pokrenut, `mage serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Mage [IDE](/docs/ide) pluginovi.
...
mage server izlaže sljedece API-je.
