---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

Mage koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Mage konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Mage
config.
...
Mage Zen je lista modela koje je obezbedio Mage tim koji su bili
testirano i potvrđeno da dobro radi sa Mage. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Mage Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite mage i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Mage i potpuno je opcionalan za korištenje.

---

## Mage Go

Mage Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Mage tim i koji su testirani i verificirani da dobro rade s Mage-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Mage Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Mage:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `mage.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Mage.
:::
...
Možete konfigurirati mage za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi mage ispravno radio.
   :::
...
Također možete dodati modele kroz svoju mage konfiguraciju.
...
da pohrani token u mage auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Mage koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Mage
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`mage.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Za korištenje Google Vertex AI s Mage:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Mage razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Mage razgovor naveden kao zasebna sesija.
...
Možete konfigurirati mage za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati mage za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati mage za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Mage. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Mage:
...
3. Kopirajte API ključ za korištenje u Mage.
...
6. **Važno**: Prije upotrebe modela oblaka u Mage, morate lokalno povući informacije o modelu:
...
Mage Zen je lista testiranih i verifikovanih modela koju je obezbedio Mage tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Mage Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Mage Zen**.

```txt
   /connect
```

3. Unesite svoj Mage API ključ.
...
Također možete dodati dodatne modele putem vaše mage konfiguracije.
...
5. Također ih možete prilagoditi putem vaše mage konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju mage konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `mage.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Mage da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `mage auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Mage konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj mage konfiguraciji.
