...
Da biste otklonili probleme s Mage, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Mage pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
Mage Desktop pokreće lokalni Mage server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Mage` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/mage/mage.jsonc` (ili `~/.config/mage/mage.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/mage.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Mage mogao ponovo izgraditi.

1. Potpuno zatvorite Mage Desktop.
...
3. Ponovo pokrenite Mage Desktop.

---

### Rješavanje problema sa vezom na serveru

Mage Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `mage.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `MAGE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `MAGE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Mage Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Mage karakteristikama.

---

### Obavještenja se ne prikazuju

Mage Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Mage u postavkama vašeg OS-a, i
...
1. Zatvorite Mage Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Mage Desktop aplikacije):
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Mage dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite Mage da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Mage da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Mage će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
