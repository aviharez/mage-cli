---
title: Konfiguracija
description: Korištenje Mage JSON konfiguracije.
---

Možete konfigurirati Mage koristeći JSON konfiguracijski fajl.

---

## Format

Mage podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/mage`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/mage/mage.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`MAGE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`mage.json` u projektu) - postavke specifične za projekat
5. **`.mage` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`MAGE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.mage` i `~/.config/mage` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/mage` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Mage konfiguraciju u `~/.config/mage/mage.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/mage/tui.json`.
...
Dodajte `mage.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Mage pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `MAGE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `MAGE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.mage` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.mage`, tako da **može nadjačati** njihove postavke.
...
Koristite `MAGE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `mage.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `mage serve` i `opencode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Mage server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Mage konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Mage pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/mage/agents/` ili `.mage/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Mage će se vratiti na `"build"` s upozorenjem.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/mage/commands/` ili `.mage/commands/`. [Saznajte više ovdje](/docs/commands).
...
Mage će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Mage **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Mage sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.mage/plugins/` ili `~/.config/mage/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Mage da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
