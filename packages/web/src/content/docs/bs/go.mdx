...
Mage Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Mage-u. Pretplatite se na Mage Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Mage.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Mage-om.
:::
...
Mage Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Mage Go radi kao bilo koji drugi provajder u Mage-u.

1. Prijavite se na **<a href={console}>Mage Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Mage Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Mage Go.
:::
...
Mage Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Mage konfiguraciji
...
Napravili smo Mage Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Mage koristite i bilo kojeg drugog provajdera.
