...
Agent skills omogucavaju Mage da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Mage pretrazuje ove lokacije:

- Konfiguracija projekta: `.mage/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/mage/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Mage ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.mage/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.mage/skills/git-release/SKILL.md` ovako:
...
Mage navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `mage.json`:
...
**Za ugradene agente** (u `mage.json`):
