...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Mage dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Mage.
...
:::note
Ovaj alat je dostupan samo kada je `MAGE_EXPERIMENTAL_LSP_TOOL=true` (ili `MAGE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Mage provajdera ili kada je varijabla `MAGE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Mage:
