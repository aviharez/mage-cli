---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Mage.
---

Dodaci vam omogućavaju da proširite Mage spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Mage.
...
- `.mage/plugins/` - Dodaci na nivou projekta
- `~/.config/mage/plugins/` - Globalni dodaci
...
1. Globalna konfiguracija (`~/.config/mage/mage.json`)
2. Konfiguracija projekta (`mage.json`)
3. Globalni direktorij dodataka (`~/.config/mage/plugins/`)
4. Direktorij dodataka projekta (`.mage/plugins/`)
...
Mage pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Mage.
...
Spriječite mage da čita `.env` fajlove:
...
Pomoćnik `tool` kreira prilagođeni alat koji mage može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za mage zajedno sa ugrađenim alatima.
