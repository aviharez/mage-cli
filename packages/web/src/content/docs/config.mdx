...
On macOS, Mage reads managed preferences from the `id.bca.mage.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Mage checks these paths:

1. `/Library/Managed Preferences/<user>/id.bca.mage.managed.plist`
2. `/Library/Managed Preferences/id.bca.mage.managed.plist`
...
Use the `id.bca.mage.managed` PayloadType. The Mage config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>id.bca.mage.managed</string>
