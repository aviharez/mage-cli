---
title: Complementos
description: Escriba sus propios complementos para extender Mage.
---

Los complementos le permiten extender Mage al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Mage.
...
- `.mage/plugins/` - Complementos a nivel de proyecto
- `~/.config/mage/plugins/` - Complementos globales
...
1. Configuración global (`~/.config/mage/mage.json`)
2. Configuración del proyecto (`mage.json`)
3. Directorio global de complementos (`~/.config/mage/plugins/`)
4. Directorio de complementos del proyecto (`.mage/plugins/`)
...
Mage ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK mage para interactuar con la IA.
...
:::note
Si está utilizando la aplicación de escritorio Mage, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que mage lea archivos `.env`:
...
El ayudante `tool` crea una herramienta personalizada a la que mage puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para mage junto con las herramientas integradas.
