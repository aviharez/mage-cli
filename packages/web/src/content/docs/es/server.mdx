---
title: Servidor
description: Interactuar con el servidor mage a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `mage serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente mage puede usar.
...
Configure `MAGE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `mage`, o configure `MAGE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `mage serve` como a `opencode web`.
...
Cuando ejecuta `mage`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor mage para interactuar con mage mediante programación.
:::

Esta arquitectura permite que mage admita múltiples clientes y le permite interactuar con mage mediante programación.

Puede ejecutar `mage serve` para iniciar un servidor independiente. Si tienes el
mage TUI ejecutándose, `mage serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Mage [IDE](/docs/ide).
...
El servidor mage expone las siguientes API.
