...
Las habilidades del agente permiten a Mage descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Mage busca estas ubicaciones:

- Configuración del proyecto: `.mage/skills/<name>/SKILL.md`
- Configuración global: `~/.config/mage/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Mage sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.mage/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.mage/skills/git-release/SKILL.md` así:
...
Mage enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `mage.json`:
...
**Para agentes integrados** (en `mage.json`):
