...
Con Mage puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Mage usa nuestro propio tema `mage`.
...
Mage viene con varios temas integrados.
...
- Quiere que Mage coincida con la apariencia de su terminal
...
Mage admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/mage/themes/*.json` o `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.mage/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.mage/themes/*.json`
