---
title: Proveedores
description: Usando cualquier proveedor LLM en Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

Mage usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Mage.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Mage
configuración.
...
Mage Zen es una lista de modelos proporcionados por el equipo Mage que han sido
probado y verificado para funcionar bien con Mage. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Mage Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione mage y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en Mage y su uso es completamente opcional.

---

## Mage Go

Mage Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Mage que han sido
probado y verificado para funcionar bien con Mage.

1. Ejecute el comando `/connect` en TUI, seleccione `Mage Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con Mage:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `mage.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Mage no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar mage para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que mage funcione correctamente.
   :::
...
Mage utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Mage
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `mage.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `mage.json`:
...
Para utilizar Google Vertex AI con Mage:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Mage como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Mage enumerada como una sesión separada.
...
Puede configurar mage para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar mage para usar modelos locales a través de LM Studio.
...
Puedes configurar mage para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Mage. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Mage:
...
3. Copie la clave API para usarla en Mage.
...
6. **Importante**: Antes de usar modelos en la nube en Mage, debe obtener la información del modelo localmente:
...
Mage Zen es una lista de modelos probados y verificados proporcionada por el equipo Mage. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Mage Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Mage Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Mage API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Mage:
...
4. Cree o actualice su archivo `mage.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Mage comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `mage auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración mage y:
