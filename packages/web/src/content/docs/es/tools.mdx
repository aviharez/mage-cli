...
Las herramientas permiten que LLM realice acciones en su código base. Mage viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Mage.
...
:::note
Esta herramienta solo está disponible cuando `MAGE_EXPERIMENTAL_LSP_TOOL=true` (o `MAGE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Mage o cuando la variable de entorno `MAGE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Mage:

```bash
MAGE_ENABLE_EXA=1 opencode
```
