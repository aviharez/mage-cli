...
要调试 Mage 的问题，请先检查其存储在磁盘上的日志和本地数据。
...
Mage Desktop 会在后台运行一个本地 Mage 服务器（即 `opencode-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。

### 快速检查

- 完全退出并重新启动应用。
- 如果应用显示错误页面，请点击**重新启动**并复制错误详情。
- 仅限 macOS：`Mage` 菜单 -> **Reload Webview**（当 UI 空白或冻结时有效）。
...
- **macOS/Linux**（旧版安装）: `~/.local/share/opencode/mage.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage 还可以从磁盘加载本地插件。临时将这些插件移走（或重命名文件夹），然后重新启动桌面应用：
...
如果禁用插件没有帮助（或插件安装卡住了），请清除缓存以便 Mage 重新构建。
...
Mage Desktop 可以启动自己的本地服务器（默认行为），也可以连接到你配置的服务器 URL。
...
如果你的 `mage.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。

#### 检查环境变量

如果你在环境中设置了 `MAGE_PORT`，桌面应用将尝试使用该端口作为本地服务器端口。

- 取消设置 `MAGE_PORT`（或选择一个空闲端口）并重新启动。
...
在 Windows 上，Mage Desktop 需要 Microsoft Edge **WebView2 Runtime**。如果应用打开后是空白窗口或无法启动，请安装或更新 WebView2 后重试。

---

### Windows: 常见性能问题

如果你在 Windows 上遇到性能缓慢、文件访问问题或终端问题，请尝试使用 [WSL (Windows Subsystem for Linux)](/docs/windows-wsl)。WSL 提供了一个 Linux 环境，能更好地与 Mage 的功能兼容。

---

### 通知不显示

Mage Desktop 仅在以下情况下显示系统通知：

- 在操作系统设置中已为 Mage 启用通知，且
- 应用窗口未处于焦点状态。
...
1. 退出 Mage Desktop。
2. 找到并删除以下文件（它们位于 Mage Desktop 应用数据目录中）：
...
如果你遇到 API 调用错误，可能是由于提供商包过期导致的。Mage 会根据需要动态安装提供商包（OpenAI、Anthropic、Google 等）并将它们缓存到本地。
...
2. 重新启动 Mage 以重新安装最新的提供商包

这将强制 Mage 下载最新版本的提供商包，通常可以解决模型参数和 API 变更带来的兼容性问题。
...
Mage 会检测你是否正在使用 Wayland 并优先使用 `wl-clipboard`，否则将按以下顺序尝试查找剪贴板工具：`xclip` 和 `xsel`。
