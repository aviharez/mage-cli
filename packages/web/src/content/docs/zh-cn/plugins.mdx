---
title: 插件
description: 编写自己的插件来扩展 Mage。
---

插件允许你通过挂钩各种事件和自定义行为来扩展 Mage。你可以创建插件来添加新功能、集成外部服务，或修改 Mage 的默认行为。
...
1. 全局配置 (`~/.config/mage/mage.json`)
2. 项目配置 (`mage.json`)
3. 全局插件目录 (`~/.config/mage/plugins/`)
4. 项目插件目录 (`.mage/plugins/`)
...
Mage 会在启动时运行 `bun install` 来安装这些依赖项。之后你的插件和工具就可以导入它们了。
...
- `directory`：当前工作目录。
- `worktree`：git 工作树路径。
- `client`：用于与 AI 交互的 Mage SDK 客户端。
...
:::note
如果你使用 Mage 桌面应用，它可以在响应就绪或会话出错时自动发送系统通知。
:::
...
`tool` 辅助函数用于创建 Mage 可调用的自定义工具。它接受一个 Zod schema 函数，并返回一个工具定义，包含：

- `description`：工具的功能描述
- `args`：工具参数的 Zod schema
- `execute`：工具被调用时执行的函数

你的自定义工具将与内置工具一起在 Mage 中可用。
