...
Dzięki mage możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie mage używa naszego własnego motywu `mage`.
...
mage ma kilka wbudowanych motywów.
...
- Chcą, aby Mage pasował do wyglądu ich terminala
...
mage obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/mage/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.mage/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.mage/themes/*.json`
