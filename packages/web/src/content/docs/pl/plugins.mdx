...
Wtyczki umożliwiające wyłączenie mage poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania opencode.
...
- `.mage/plugins/` - Wtyczki na poziomie projektu
- `~/.config/mage/plugins/` - Wtyczki globalne
...
1. Konfiguracja globalna (`~/.config/mage/mage.json`)
2. Konfiguracja projektu (`mage.json`)
3. Globalny katalog wtyczek (`~/.config/mage/plugins/`)
4. Katalog wtyczek projektu (`.mage/plugins/`)
...
mage uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z mage do sztucznej inteligencji.
...
Zablokuj mage czytanie plików `.env`:
