---
title: Serwer
description: Interakcja z serwerem mage za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `mage serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient opencode.
...
Ustaw `MAGE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `mage` lub ustaw `MAGE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `mage serve`, jak i `opencode web`.
...
Kiedy uruchomisz `mage`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera mage do programowej interakcji z opencode.
:::

Ta architektura umożliwia obsługę wielu klientów przez mage i programową interakcję z kodem otwartym.

Możesz uruchomić `mage serve`, aby uruchomić samodzielny serwer. Jeśli masz
mage TUI działa, `mage serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki mage [IDE](/docs/ide).
...
Serwer mage udostępnia następujące interfejsy API.
