...
Możesz dodać dodatkowe informacje do mage za pomocą _Model Context Protocol_, w skrócie MCP. mage obsługuje zarówno serwery lokalne, jak i zdalne.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/mage`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
mage automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, opencode:
...
Jeśli serwer wymaga uwierzytelnienia, mage poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `opencode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji mage bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć mage z kontem Sentry.
