...
mage używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.
...
Mage Zen to lista modeli dostarczonych przez zespół opencode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Mage Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Mage Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w mage i jest całkowicie opcjonalny w użyciu.

---

## Mage Go

Mage Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół opencode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Mage Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `mage.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w mage nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować mage do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod mage działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
mage używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
...
`mage.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację mage jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja mage będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować mage tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować mage tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować mage tak, aby korzystał z modeli lokalnych poprzez Ollamę.
...
6. **Ważne**: Przed użyciem modeli chmurowych w mage musisz pobrać informacje o modelu lokalnie:
...
Mage Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Mage Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Mage Zen**.
...
4. Utwórz lub zaktualizuj plik `mage.json` w katalogu projektu:
...
Pola `limit` pozwalają mage zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `mage auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację mage i:
