...
mage używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługi uruchamiania modeli pierwszych.
...
Tutaj pełny identyfikator do `provider_id/model_id`. Na przykład, użycie [Mage Zen](/docs/zen), przestrzeganie `opencode/gpt-5.1-codex` dla Kodeksu GPT 5.1.
...
Wiele modeli obsługuje wiele wariantów lub różnych wybranych. mage jest dostarczany z podstawowymi wariantami wariantów dla dostawców.

### Wbudowane warianty

mage jest dostarczany z domyślnymi wariantami dla wielu dostawców:
...
Po uruchomieniu mage sprawdzanie modeli w następującej kolejności:
