...
Aby uzyskać dostęp do mage w swoim terminalu, będziesz potrzebować:
...
Najłatwiej zainstalować mage za pomocą skryptu instalacyjnego.
...
Obecnie pracujemy nad instalacją mage w systemie Windows za pomocą Bun.
...
Dzięki mage możesz korzystać z dowolnego dostawcy LLM, konfigurując jego klucze API.

Jeśli będziesz korzystać z wielu dostawców LLM, zalecamy skorzystanie z [Mage Zen](/docs/zen).
Aby uzyskać dostęp do wyselekcjonowanej listy modeli, które zostały zatwierdzone i zweryfikowane przez zespół opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz mage i przejdź do [opencode.ai/auth](https://opencode.ai/auth).
...
Następnie zainicjuj mage dla projektu, uruchamiając odpowiednie polecenie.

```bash frame="none"
/init
```

Spowoduje to, że mage przeanalizuje Twój projekt i utworzy plik `AGENTS.md` w katalogu głównym projektu.

:::tip
Powinieneś zacommitować plik `AGENTS.md` swojego projektu w Git.
:::

Pomaga to mage zrozumieć strukturę projektu i stosowane wzorce kodowania.
...
Możesz poprosić mage o wyjaśnienie bazy kodu.
...
Możesz poprosić mage o dodanie nowej funkcji do Twojego projektu. Najpierw zalecamy poproszenie o przygotowanie planu.

1. **Stwórz Plan**

   mage ma _Plan Mode_, który umożliwia wyłączenie zmian i
   ...
   Wersja mage jest bardzo inteligentna, aby zrozumieć, czego chcesz. To pomaga
   rozmawiaj z nim tak, jakbyś rozmawiał z młodszym programistą w swoim zespole.

   :::tip
   Podaj mage kontekst, aby zrozumiał, czego chcesz.
   :::
...
2. **Udoskonalanie planu**
   ...
   mage może zeskanować obrazy, które mu przekażesz i zrozumieć je. Możesz to zrobić, przeciągając i upuszczając obraz do terminala.
...
Aby wprowadzić kolejną zmianę, możesz poprosić mage lub zrobić to bezpośrednio bez konieczności wcześniejszego przeglądania planu.
...
Musisz upewnić się, że dostarczyłeś odpowiednie pliki, aby mage mógł wprowadzić zmiany.

---

### Cofnij zmiany

Załóżmy, że prosisz mage o wprowadzenie zmian.
...
mage cofnie teraz zmiany i wyświetli wiadomość ponownie.

```txt frame="none" "@packages/functions/src/api/index.ts"
Can you refactor the function in @packages/functions/src/api/index.ts?
```

Możesz edytować monit i poprosić mage o ponowną próbę.
...
Rozmowy z mage można [udostępniać swojemu zespołowi](/docs/share).
