...
Umiejętności agenta pozwalają mage odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
mage przeszukuje te lokalizacje:

- Project config: `.mage/skills/<name>/SKILL.md`
- Global config: `~/.config/mage/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu mage przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.mage/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.mage/skills/git-release/SKILL.md` w ten sposób:
...
mage wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `mage.json`:
...
**Dla agentów wbudowanych** (w `mage.json`):
