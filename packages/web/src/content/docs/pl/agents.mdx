...
W Mage obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Mage zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Mage ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Mage ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Mage, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `mage.json`:
...
- Globalnie: `~/.config/mage/agents/`
- W projekcie: `.mage/agents/`
...
Jeśli nie zostanie skonfigurowana, Mage używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Mage, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Mage w `provider/model-id`. Na przykład, [Mage Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
