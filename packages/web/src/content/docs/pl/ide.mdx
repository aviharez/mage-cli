---
title: IDE
description: Rozszerzenie mage dla VS Code, Cursor i innych IDE
---

mage integruje się z VS Code, Cursorem lub obsługującym terminal IDE. Aby uruchomić, po prostu uruchom `mage` w terminalu.

---

## Stosowanie

- **Szybkie uruchomienie**: użycie `Cmd+Esc` (Mac) lub `Ctrl+Esc` (Windows/Linux), aby uruchomić mage w uruchamianiu terminala lub skoncentruj się na uruchamianiu terminala, jeśli jest już uruchomiony.
- **Nowa sesja**: `Cmd+Shift+Esc` (Mac) lub `Ctrl+Shift+Esc` (Windows/Linux), aby została nowa konfiguracja terminala opencode, nawet jeśli jest już otwarta. Możesz także kliknąć przycisk mage w interfejsie użytkownika.
...
Aby poznać mage na VS Code w forkach, takich jak Cursor, Windsurf, VSCodium:

1. Otwórz kod VS
2. Otwórz innowacyjny terminal
3. Uruchom `mage` — rozszerzenie instaluje się automatycznie
...
- zastosowanie się, że użycie `mage` w terminalu.
