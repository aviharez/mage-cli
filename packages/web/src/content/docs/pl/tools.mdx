...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Mage posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Mage.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `MAGE_EXPERIMENTAL_LSP_TOOL=true` (lub `MAGE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Mage lub gdy zmienna środowiskowa `MAGE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Mage:
