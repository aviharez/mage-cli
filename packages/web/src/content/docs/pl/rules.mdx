...
mage obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/mage/AGENTS.md`. Jest to stosowane we wszystkich sesjach opencode.
...
W przypadku użytkowników migrujących z Claude Code mage obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:

- **Zasady projektu**: `CLAUDE.md` w katalogu projektu (używane, jeśli nie istnieje `AGENTS.md`)
- **Zasady globalne**: `~/.claude/CLAUDE.md` (używane, jeśli nie istnieje `~/.config/mage/AGENTS.md`)
...
Po uruchomieniu mage szuka plików reguł w następującej kolejności:

1. **Pliki lokalne** poprzez przejście z bieżącego katalogu (`AGENTS.md`, `CLAUDE.md`)
2. **Plik globalny** w `~/.config/mage/AGENTS.md`
3. **Plik Claude Code** w `~/.claude/CLAUDE.md` (chyba że wyłączony)

Pierwszy pasujący plik wygrywa w każdej kategorii. Na przykład, jeśli masz zarówno `AGENTS.md`, jak i `CLAUDE.md`, używany jest tylko `AGENTS.md`. Podobnie `~/.config/mage/AGENTS.md` ma pierwszeństwo przed `~/.claude/CLAUDE.md`.

---

## Instrukcje niestandardowe

Możesz określić niestandardowe pliki instrukcji w swoim `mage.json` lub globalnym `~/.config/mage/mage.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Chociaż mage nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:

### Używanie mage.json

Zalecanym podejściem jest użycie pola `instructions` w `mage.json`:
...
Możesz nauczyć mage czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że mage ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania

:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `mage.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
