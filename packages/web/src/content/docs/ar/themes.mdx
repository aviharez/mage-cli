...
مع Mage يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Mage سمتنا `mage`.
...
- يريدون أن يطابق Mage مظهر terminal لديهم
...
يدعم Mage نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/mage/themes/*.json` أو `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.mage/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.mage/themes/*.json`
