...
الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في mage مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.mage/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/mage/tools/`.
