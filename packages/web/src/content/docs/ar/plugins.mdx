---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Mage.
---

تتيح لك الإضافات توسيع Mage عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Mage.
...
- `.mage/plugins/` - إضافات على مستوى المشروع
- `~/.config/mage/plugins/` - إضافات عامة
...
1. إعدادات عامة (`~/.config/mage/mage.json`)
2. إعدادات المشروع (`mage.json`)
3. دليل الإضافات العام (`~/.config/mage/plugins/`)
4. دليل إضافات المشروع (`.mage/plugins/`)
...
يشغّل Mage الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ mage للتفاعل مع الذكاء الاصطناعي.
...
:::note
إذا كنت تستخدم تطبيق Mage لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ mage استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ mage إلى جانب الأدوات المضمنة.
