---
title: المزوّدون
description: استخدام أي موفّر LLM في Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Mage ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Mage.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Mage.
...
Mage Zen هي قائمة نماذج يوفّرها فريق Mage وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Mage. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Mage Zen.
:::
...
يعمل مثل أي مزوّد آخر في Mage واستخدامه اختياري بالكامل.

---

## Mage Go

Mage Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Mage، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Mage.

1. شغّل الأمر `/connect` في TUI، واختر `Mage Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Mage:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `mage.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Mage غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين mage لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل mage بشكل صحيح.
   :::
...
:::note[ملاحظة امتثال]
يستخدم Mage نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Mage
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`mage.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام Google Vertex AI مع Mage:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Mage تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Mage مدرجة كجلسة مستقلة.
...
يمكنك ضبط mage لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط mage لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط mage لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Mage. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
3. انسخ مفتاح API لاستخدامه في Mage.
...
6. **مهم**: قبل استخدام نماذج السحابة في Mage، يجب سحب معلومات النموذج محليا:
...
Mage Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Mage. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Mage Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Mage Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Mage.
...
4. أنشئ أو حدّث ملف `mage.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Mage بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `mage auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات mage وتأكد من:
