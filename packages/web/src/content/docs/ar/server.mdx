---
title: الخادم
description: تفاعل مع خادم mage عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `mage serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل mage استخدامها.
...
عيّن `MAGE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `mage`، أو عيّن `MAGE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `mage serve` و `opencode web`.
...
عند تشغيل `mage` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم mage للتفاعل مع mage برمجيا.
:::

تتيح هذه البنية لـ mage دعم عدة عملاء وتمكّنك من التفاعل مع mage برمجيا.

يمكنك تشغيل `mage serve` لبدء خادم مستقل. إذا كانت واجهة mage في terminal (TUI)
قيد التشغيل، فسيبدأ `mage serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Mage لـ [IDE](/docs/ide).
...
يعرض خادم mage واجهات API التالية.
