...
Чтобы использовать mage в вашем терминале, вам понадобится:
...
Самый простой способ установить mage — через сценарий установки.
...
- **Использование Homebrew в macOS и Linux**
  ...
  > Мы рекомендуем использовать tap mage для получения самых последних версий. Официальная формула `brew install opencode` поддерживается командой Homebrew и обновляется реже.
...
В настоящее время добавляется поддержка установки mage в Windows с помощью Bun.
...
С mage вы можете использовать любого поставщика LLM, настроив его ключи API.

Если вы новичок в использовании поставщиков LLM, мы рекомендуем использовать [Mage Zen](/docs/zen).
Это тщательно подобранный список моделей, протестированных и проверенных opencode.

1. Запустите команду `/connect` в TUI, выберите mage и перейдите по адресу [opencode.ai/auth](https://opencode.ai/auth).
...
Затем инициализируйте mage для проекта, выполнив следующую команду.

```bash frame="none"
/init
```

Это позволит mage проанализировать ваш проект и создать файл `AGENTS.md` в
корень проекта.
...
Это помогает mage понять структуру проекта и шаблоны кодирования.
использовал.
...
Теперь вы готовы использовать mage для работы над своим проектом. Не стесняйтесь спрашивать о чем угодно!
...
Вы можете попросить mage объяснить вам кодовую базу.
...
Вы можете попросить mage добавить новые функции в ваш проект. Хотя мы сначала рекомендуем попросить его создать план.

1. **Составьте план**

   mage имеет _режим планирования_, который отключает возможность вносить изменения и
   ...
   Вы хотите предоставить mage достаточно подробностей, чтобы понять, чего вы хотите. Это помогает
   поговорить с ним так, как будто вы разговариваете с младшим разработчиком в своей команде.

   :::tip
   Дайте mage много контекста и примеров, чтобы помочь ему понять, что вы
   хотеть.
   :::
...
2. **Итерация плана**
   ...
   mage может сканировать любые изображения, которые вы ему предоставляете, и добавлять их в командную строку. Вы можете
...
Для более простых изменений вы можете попросить mage создать их напрямую.
...
Убедитесь, что предоставили достаточно деталей, чтобы mage внес корректные изменения.

---

### Отмена изменений

Допустим, вы просите mage внести некоторые изменения.
...
mage отменит внесенные вами изменения и покажет исходное сообщение.
снова.
...
Отсюда вы можете настроить подсказку и попросить mage повторить попытку.
