...
Создайте `.mage/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию mage или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем mage [config](/docs/config):
...
- Глобальный: `~/.config/mage/commands/`
- Для каждого проекта: `.mage/commands/`
...
mage включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
