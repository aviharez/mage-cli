---
title: Windows (WSL)
description: Запускайте mage в Windows через WSL.
---

import { Steps } from "@astrojs/starlight/components"

mage можно запускать напрямую в Windows, но для лучшего опыта мы рекомендуем [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install). WSL дает Linux-среду, которая отлично работает с возможностями opencode.
...
2. **Установите mage в WSL**

   После настройки WSL откройте терминал WSL и установите mage одним из [способов установки](/docs/).

   ```bash
   curl -fsSL https://opencode.ai/install | bash
   ```

3. **Запускайте mage из WSL**
...
Если вы предпочитаете mage Desktop, но хотите запускать сервер в WSL:
...
:::caution
При использовании `--hostname 0.0.0.0` задайте `MAGE_SERVER_PASSWORD`, чтобы защитить сервер.
...
:::tip
Для максимально плавной работы стоит клонировать или скопировать репозиторий в файловую систему WSL (например, в `~/code/`) и запускать mage оттуда.
:::
...
- Даже для проектов на дисках Windows запускайте mage в WSL, чтобы получить более плавный доступ к файлам
- Используйте mage вместе с [расширением WSL для VS Code](https://code.visualstudio.com/docs/remote/wsl) для единого рабочего процесса
- Конфигурация и сессии mage хранятся в среде WSL по пути `~/.local/share/opencode/`
