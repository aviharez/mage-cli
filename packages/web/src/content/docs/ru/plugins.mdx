...
Плагины позволяют расширять opencode, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения mage по умолчанию.
...
- `.mage/plugins/` – плагины уровня проекта.
- `~/.config/mage/plugins/` — глобальные плагины
...
1. Глобальная конфигурация (`~/.config/mage/mage.json`)
2. Конфигурация проекта (`mage.json`)
3. Глобальный каталог плагинов (`~/.config/mage/plugins/`)
4. Каталог плагинов проекта (`.mage/plugins/`)
...
mage запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
Плагины также могут добавлять в mage собственные инструменты:
