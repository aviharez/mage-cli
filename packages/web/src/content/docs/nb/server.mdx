---
title: Tjener
description: Samhandle med Mage-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `mage serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Mage-klient kan bruke.
...
Sett `MAGE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `mage` som standard, eller sett `MAGE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `mage serve` og `opencode web`.
...
Når du kjører `mage` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Mage-serveren til å samhandle med Mage programmatisk.
:::

Denne arkitekturen lar Mage støtte flere klienter og lar deg samhandle med Mage programmatisk.

Du kan kjøre `mage serve` for å starte en frittstående server. Hvis du har
Mage TUI kjører, vil `mage serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Mage [IDE](/docs/ide) plugins.
