---
title: Konfigurasjon
description: Ved å bruke mage JSON-konfigurasjonen.
---

Du kan konfigurere Mage ved å bruke en JSON konfigurasjonsfil.

---

## Format

Mage støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "mage"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/mage`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/mage/mage.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`MAGE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`mage.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.mage` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`MAGE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.mage` og `~/.config/mage` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/mage`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Mage-konfigurasjon i `~/.config/mage/mage.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `mage.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Mage starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `MAGE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `MAGE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.mage` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.mage`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan konfigurere serverinnstillinger for kommandoene `mage serve` og `opencode web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Mage-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Mage-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Mage å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/mage/agents/` eller `.mage/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Mage falle tilbake til `"build"` med en advarsel.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/mage/commands/` eller `.mage/commands/`. [Les mer her](/docs/commands).
...
Mage vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Mage alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Mage med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.mage/plugins/` eller `~/.config/mage/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Mage til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
