...
Det er to typer agenter i Mage; primære agenter og underagenter.
...
Mage kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Mage kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Mage kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Mage sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `mage.json`:
...
- Globalt: `~/.config/mage/agents/`
- Per prosjekt: `.mage/agents/`

```markdown title="~/.config/mage/agents/review.md"
---
...
Hvis ingen temperatur er spesifisert, bruker Mage modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Mage-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Mage-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Mage Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```markdown title="~/.config/mage/agents/docs-writer.md"
---
...
```markdown title="~/.config/mage/agents/security-auditor.md"
---
