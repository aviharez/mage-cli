...
Mage gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Mage starter TUI for gjeldende katalog.
...
Når du bruker Mage TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Mage. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Dette er atskilt fra `mage.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Mage venter etter leader key. Standard er `2000`.
