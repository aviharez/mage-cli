...
Du kan legge til eksterne verktøy i Mage ved å bruke _Model Context Protocol_ eller MCP. Mage støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Mage Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="mage.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/mage`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Mage håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Mage:
...
Hvis serveren krever autentisering, vil Mage be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
```bash
mage mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Mage lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
mage mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Mage, sammen med innebygde verktøy. Så du kan administrere dem gjennom Mage-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Mage til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Mage-samtale oppført som en separat økt.
