---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

Mage bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Mage-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Mage
konfig.
...
Mage Zen er en liste over modeller levert av Mage-teamet som har vært
testet og verifisert for å fungere godt med Mage. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Mage Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Mage Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i Mage og er helt valgfritt å bruke.

---

## Mage Go

Mage Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Mage-teamet som har vært
testet og verifisert for å fungere godt med Mage.

1. Kjør kommandoen `/connect` i TUI, velg `Mage Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Mage:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Mage:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `mage.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i mage støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere mage til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Mage skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Mage-konfigurasjonen.
...
for å lagre token i Mage auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Mage bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Mage
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`mage.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Mage:
...
Slik bruker du Google Vertex AI med Mage:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Mage, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Mage-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Mage-samtale oppført som en separat økt.
...
Du kan konfigurere Mage for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Mage for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Mage for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Mage. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Mage:
...
3. Kopier API-nøkkelen for bruk i Mage.
...
6. **Viktig**: Før du bruker skymodeller i Mage, må du hente modellinformasjonen lokalt:
...
Mage Zen er en liste over testede og verifiserte modeller levert av Mage-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Mage Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Mage Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Mage API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Mage-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Mage-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Mage:
...
Du kan også tilpasse modeller gjennom Mage-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Mage. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `mage.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Mage forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `mage auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Mage-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Mage-konfigurasjonen.
