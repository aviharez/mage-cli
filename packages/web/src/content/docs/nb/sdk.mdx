---
title: SDK
description: Typesikker JS-klient for Mage-server.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Mage JS/TS SDK gir en typesikker klient for samhandling med serveren.
Bruk den til å bygge integrasjoner og kontrollere Mage programmatisk.
...
```bash
npm install @mybcabisnis/mage-sdk
```
...
Du kan sende et konfigurasjonsobjekt for å tilpasse virkemåten. Forekomsten henter fortsatt din `mage.json`, men du kan overstyre eller legge til konfigurasjon inline:

```javascript
import { createOpencode } from "@mybcabisnis/mage-sdk"

const mage = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
Hvis du allerede har en kjørende forekomst av Mage, kan du opprette en klientforekomst for å koble til den:
...
```typescript
import type { Session, Message, Part } from "@mybcabisnis/mage-sdk"
```
