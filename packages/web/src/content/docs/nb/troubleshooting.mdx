...
For å feilsøke problemer med Mage, start med å sjekke loggene og lokale data den lagrer på disken.
...
Mage lagrer øktdata og andre applikasjonsdata på disken på:
...
Mage Desktop kjører en lokal Mage-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `Mage`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/mage.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at Mage kan gjenoppbygge den.

1. Avslutt Mage Desktop helt.
...
3. Start Mage Desktop på nytt.

---

### Løs problemer med servertilkobling

Mage Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis `mage.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `MAGE_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `MAGE_PORT` (eller velg en ledig port) og start på nytt.
...
På Windows krever Mage Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til Mage.

---

### Varsler vises ikke

Mage Desktop viser bare systemvarsler når:

- varsler er aktivert for Mage i OS-innstillingene dine, og
...
1. Avslutt Mage Desktop.
2. Finn og slett disse filene (de finnes i Mage Desktop-appens datakatalog):
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. Mage installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start Mage på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge Mage til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
Mage vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
