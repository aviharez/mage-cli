...
mage 저장 세션 데이터 및 디스크에 다른 응용 데이터:
...
mage 데스크톱은 배경에서 로컬 mage 서버 (`opencode-cli` sidecar)를 실행합니다. 대부분의 문제는 misbehaving 플러그인, 손상된 캐시, 또는 나쁜 서버 설정에 의해 발생합니다.

## 빠른 확인

- 완전히 종료하고 앱을 다시 시작.
- 앱이 오류 화면을 보여 주면 ** Restart**를 클릭하고 오류 세부 정보를 복사합니다.
- macOS만: `Mage` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/mage/mage.jsonc` (또는 `~/.config/mage/mage.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/mage.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\mage.jsonc`
...
- **글로벌 플러그인** -**macOS/리눅스**: `~/.config/mage/plugins/`
...
mage 데스크톱은 자체 로컬 서버(기본)를 시작하거나 구성된 서버 URL에 연결할 수 있습니다.
...
`mage.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.

### 환경 변수 확인

`MAGE_PORT`가 있는 경우, 데스크탑 앱은 로컬 서버의 포트를 사용하려고 합니다.

- `MAGE_PORT` (또는 무료 포트를 선택) 및 재시작.
...
Windows에서 mage 데스크톱은 Microsoft Edge ** WebView2 실행 시간 **를 요구합니다. 앱이 공백 창에 열거나 시작하지 않을 경우, install/update WebView2를 설치하고 다시 시도하십시오.
...
API 호출 오류가 발생하면, 이 공급 업체 패키지로 인해 발생할 수 있습니다. mage 동적 설치 공급자 패키지 (OpenAI, Anthropic, Google 등) 필요 하 고 로컬로 캐시.
