---
title: 플러그인
description: Mage를 확장하기 위해 자신만의 플러그인을 작성하세요.
---
...
- `.mage/plugins/` - 프로젝트 레벨 플러그인
- `~/.config/mage/plugins/` - 글로벌 플러그인
...
1. 글로벌 구성 (`~/.config/mage/mage.json`)
2. 프로젝트 구성 (`mage.json`)
3. 글로벌 플러그인 디렉토리 (`~/.config/mage/plugins/`)
4. 프로젝트 플러그인 디렉토리 (`.mage/plugins/`)
...
- `client`: AI와 상호 작용을 위한 mage SDK 클라이언트.
...
:::note
mage 데스크톱 앱을 사용하는 경우 응답이 준비되어 있거나 세션 오류가 있을 때 시스템 알림을 자동으로 보낼 수 있습니다.
:::
