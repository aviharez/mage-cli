...
opencode는 Windows에서 직접 실행할 수 있지만, 가장 좋은 사용 경험을 위해 [Windows Subsystem for Linux (WSL)](https://learn.microsoft.com/en-us/windows/wsl/install) 사용을 권장합니다. WSL은 mage 기능과 자연스럽게 동작하는 Linux 환경을 제공합니다.
...
mage 데스크톱 앱을 쓰면서 서버는 WSL에서 실행하고 싶다면 다음처럼 진행하세요.
...
:::caution
`--hostname 0.0.0.0`을 사용할 때는 `MAGE_SERVER_PASSWORD`를 설정해 서버를 보호하세요.
...
- mage 설정과 세션은 WSL 환경의 `~/.local/share/opencode/`에 저장됩니다
