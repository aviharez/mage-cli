---
title: Regeln
description: Lege eigene Anweisungen fuer mage fest.
---

Du kannst mage ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in mage den Befehl `/init` aus.
...
So kann mage sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht mage sie zu erweitern.
...
mage kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/mage/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt Mage dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/mage/AGENTS.md` existiert)
...
Beim Start sucht mage in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/mage/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `mage.json` oder in `~/.config/mage/mage.json` angeben.
...
mage parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit mage.json

Empfohlen ist das `instructions`-Feld in der `mage.json`:
...
Du kannst mage explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `mage.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
