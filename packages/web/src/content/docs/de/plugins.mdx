---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Mage zu erweitern.
---

Mit Plugins können Sie Mage erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Mage zu ändern.
...
- `.mage/plugins/` – Plugins auf Projektebene
- `~/.config/mage/plugins/` – Globale Plugins
...
1. Globale Konfiguration (`~/.config/mage/mage.json`)
2. Projektkonfiguration (`mage.json`)
3. Globales Plugin-Verzeichnis (`~/.config/mage/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.mage/plugins/`)
...
Mage führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".mage/plugins/my-plugin.ts"
...
```js title=".mage/plugins/example.js"
...
- `client`: Ein Mage-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Mage erweitern können.
...
```js title=".mage/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "mage"'`
      }
    },
  }
}
...
:::note
Wenn Sie die Mage-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Mage `.env`-Dateien liest:

```javascript title=".mage/plugins/env-protection.js"
...
```javascript title=".mage/plugins/inject-env.js"
...
Plugins können Mage auch benutzerdefinierte Tools hinzufügen:

```ts title=".mage/plugins/custom-tools.ts"
import { type Plugin, tool } from "@mybcabisnis/mage-plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Mage aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Mage zur Verfügung.
...
```ts title=".mage/plugins/compaction.ts"
import type { Plugin } from "@mybcabisnis/mage-plugin"
...
```ts title=".mage/plugins/custom-compaction.ts"
import type { Plugin } from "@mybcabisnis/mage-plugin"
