...
description: Erstellen Sie Tools, die der LLM in Mage aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Mage wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.mage/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/mage/tools/` platzieren.
...
```ts title=".mage/tools/database.ts" {1}
...
```ts title=".mage/tools/project.ts" {8}
import { tool } from "@mybcabisnis/mage-plugin"
...
```python title=".mage/tools/add.py"
import sys
...
```ts title=".mage/tools/python-add.ts" {10}
import { tool } from "@mybcabisnis/mage-plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".mage/tools/add.py")
