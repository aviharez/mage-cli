...
Agent Skills erlauben Mage, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Mage sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Mage vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.mage/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.mage/skills/git-release/SKILL.md` zum Beispiel so:
...
Mage listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `mage.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `mage.json`):
