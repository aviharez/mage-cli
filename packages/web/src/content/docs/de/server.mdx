...
Der Befehl `mage serve` startet einen headless HTTP-Server.
...
Setze `MAGE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `mage`, kann aber mit `MAGE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `mage serve` und `opencode web`.
...
Wenn du `mage` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opencode-Server, um mage programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `mage serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `mage serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Mage-[IDE](/docs/ide)-Plugins.
