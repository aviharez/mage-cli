...
Wenn Mage Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
mage speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Mage Desktop startet im Hintergrund einen lokalen Mage-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Mage`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/mage/mage.jsonc` (oder `~/.config/mage/mage.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/mage.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Mage ihn neu aufbauen kann.

1. Beende Mage Desktop komplett.
...
3. Starte Mage Desktop neu.

---

### Server-Verbindungsprobleme beheben

Mage Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `mage.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `MAGE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `MAGE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Mage Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Mage-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Mage Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Mage in den OS-Einstellungen aktiviert sind, und
...
1. Beende Mage Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Mage Desktop):
...
Wenn du Probleme mit Mage hast:
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. mage installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte mage neu, um die neuesten Provider-Pakete zu installieren
...
mage erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
