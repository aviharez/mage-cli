...
To debug issues with Mage, start by checking the logs and local data it stores on disk.
...
mage stores session data and other application data on disk at:
...
Mage Desktop runs a local Mage server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Mage` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/mage/mage.jsonc` (or `~/.config/mage/mage.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/mage.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Mage can rebuild it.

1. Quit Mage Desktop completely.
...
3. Restart Mage Desktop.

---

### Fix server connection issues

Mage Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `mage.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `MAGE_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `MAGE_PORT` (or pick a free port) and restart.
...
On Windows, Mage Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Mage's features.

---

### Notifications not showing

Mage Desktop only shows system notifications when:

- notifications are enabled for Mage in your OS settings, and
...
1. Quit Mage Desktop.
2. Find and delete these files (they live in the Mage Desktop app data directory):
...
If you're experiencing issues with Mage:
...
If you encounter API call errors, this may be due to outdated provider packages. mage dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart mage to reinstall the latest provider packages

This will force mage to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
mage will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
