...
mage normal bir GitLab işlem hattında çalışır. Bunu bir [CI component](https://docs.gitlab.com/ee/ci/components/) olarak bir işlem hattına oluşturabilirsiniz.

Burada mage için topluluk tarafından oluşturulan bir CI/CD bileşeni kullanıyoruz — [nagyv/gitlab-opencode](https://gitlab.com/nagyv/gitlab-opencode).

---

### Özellikler

- **İş başına özel işlemleri kullanın**: mage parçalarının kullanıma veya devre dışı bırakmak için opencode'u özel bir yapılandırma dizini ile yapılandırın; örneğin `./config/#custom-directory`.
- **Minimum kurulum**: CI bileşeni opencode'u arka planda kurar; yalnızca mage yapılandırmasını ve ilk istemi oluşturmanız gerekir.
...
1. mage kimlik doğrulamanızı JSON **Ayarlar** > **CI/CD** > **Değişkenler** altında Dosya türü CI ortam değişkenleri olarak saklayın. Bunları "Maskeli ve gizli" olarak işaretlediğinizden emin olun.
...
- **Bir sorunu açıklama**
  ...
  mage konuyu okuyacak ve net bir açıklama ile yanıt verecektir.
...
- **Bir sorunu düzeltme**
  ...
  mage yeni bir dal oluşturacak, değişiklikleri uygulayacak ve değişiklikleri içeren bir birleştirme isteği açacaktır.
