---
title: Kurallar
description: mage için özel talimatları ayarlayın.
---

Bir `AGENTS.md` dosyası oluşturarak mage için özel talimatlar verebilirsiniz. Bu, Cursor rules yaklaşımına benzer. Özel projeniz için davranışını özelleştirmek amacıyla LLM'in içeriğine dahil edilecek talimatları içerir.
...
mage ayrıca `AGENTS.md` dosyasının birden fazla konumdan okunmasını da destekler. Ve bu farklı amaçlara hizmet ediyor.
...
Ayrıca `~/.config/mage/AGENTS.md` dosyasında genel kurallara sahip olabilirsiniz. Bu, tüm mage oturumlarına uygulanır.
...
- **Global kurallar**: `~/.claude/CLAUDE.md` (`~/.config/mage/AGENTS.md` yoksa kullanılır)
...
2. **Global dosya** `~/.config/mage/AGENTS.md` adresinde
3. `~/.claude/CLAUDE.md` adresindeki **Claude Kod dosyası** (devre dışı bırakılmadığı sürece)

Her kategoride ilk eşleşen dosya kazanır. Örneğin, hem `AGENTS.md` hem de `CLAUDE.md` varsa yalnızca `AGENTS.md` kullanılır. Benzer şekilde, `~/.config/mage/AGENTS.md`, `~/.claude/CLAUDE.md`'ye göre önceliklidir.

---

## Özel talimatlar

Özel talimat dosyalarını `mage.json` veya global `~/.config/mage/mage.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
### mage.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `mage.json` alanını kullanmaktır:
...
:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `mage.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
