...
description: Mage, LSP sunucularınızla bütünleşir.
---

Mage, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Mage, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`MAGE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve mage bir dosyayı açtığında:
...
LSP sunucularını, mage yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
