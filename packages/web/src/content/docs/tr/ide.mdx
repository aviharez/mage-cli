---
title: IDE
description: VS Code, Cursor ve diğer IDE'ler için mage uzantısı
---

opencode, VS Code, Cursor veya bir terminali destekleyen herhangi bir IDE ile entegre olur. Başlamak için terminalde `mage` komutunu çalıştırmanız yeterli.
...
- **Yeni Oturum**: Zaten açık olsa bile yeni bir mage terminal oturumu başlatmak için `Cmd+Shift+Esc` (Mac) veya `Ctrl+Shift+Esc` (Windows/Linux) kullanın. Ayrıca kullanıcı arayüzündeki mage düğmesini de tıklayabilirsiniz.
- **Bağlam Farkındalığı**: Geçerli seçiminizi veya sekmenizi mage ile otomatik olarak paylaşın.
...
3. `mage` komutunu çalıştırın - uzantı otomatik olarak yüklenir
...
- Entegre terminalde `mage` çalıştırdığınızdan emin olun.
