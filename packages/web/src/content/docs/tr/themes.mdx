...
mage ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak mage kendi `mage` temasını kullanır.
...
mage birden fazla yerleşik temayla gelir.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/mage/themes/*.json` veya `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.mage/themes/*.json`
4. **Geçerli çalışma dizini** - `./.mage/themes/*.json`
