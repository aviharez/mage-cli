...
Burada, mage ile belirli bir sıraya göre iyi çalışan birkaç model yer almaktadır. (Bu kapsamlı bir liste değildir ve mutlaka güncel olması da gerekmez):
...
Burada tam kimlik `provider_id/model_id`'dir. Örneğin, [Mage Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencode/gpt-5.1-codex` kullanırsınız.
...
mage birçok sağlayıcı için varsayılan değişkenlerle birlikte gelir:
...
mage başlatıldığında modelleri aşağıdaki öncelik sırasına göre kontrol eder:

1. `--model` veya `-m` komut satırı bayrağı. Biçim, yapılandırma dosyasındakiyle aynıdır: `provider_id/model_id`.

2. mage yapılandırmasındaki model listesi.
