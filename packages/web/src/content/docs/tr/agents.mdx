...
Mage, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Mage iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Harici dokümanlar ve bağımlılık araştırmaları için salt okunur bir agent. Bir bağımlılık repository'sini Mage'un yönetilen cache'ine clone etmeniz, kütüphane kaynak kodunu incelemeniz veya workspace'inizi değiştirmeden yerel kodu upstream implementasyonlarla karşılaştırmanız gerektiğinde bunu kullanın.
...
Agent'ları `mage.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse mage modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global mage yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
mage yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Mage Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencode/gpt-5.1-codex` kullanırsınız.
