---
title: Eklentiler
description: Mage'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Mage'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Mage'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.mage/plugins/` - Proje düzeyinde eklentiler
- `~/.config/mage/plugins/` - Genel eklentiler
...
1. Global config (`~/.config/mage/mage.json`)
2. Project config (`mage.json`)
3. Global eklenti dizini (`~/.config/mage/plugins/`)
4. Proje eklenti dizini (`.mage/plugins/`)
...
Mage bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Mage SDK istemcisi.
...
Mage'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Mage masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Mage'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Mage'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Mage'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Mage'a da hazır olacaktır.
