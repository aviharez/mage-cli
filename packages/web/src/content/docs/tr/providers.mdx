...
2. Sağlayıcıyı mage yapılandırmanızda yapılandırın.
...
Mage Zen, mage ekibi tarafından sağlanan modellerin bir listesidir.
mage ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Mage Zen ile başlamanızı öneririz.
:::
...
Mage Go, mage ile iyi çalıştığı test edilmiş ve doğrulanmış, mage ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Mage Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u mage ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `mage.json` kullanın:
...
`mage.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`mage.json` üzerinden özelleştirin:
...
Google Vertex AI'yi mage ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları mage çalıştırılırken ayarlayın.
...
Helicone'un mage aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her mage görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her mage görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
:::tip
Ollama kendisini mage için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::
...
Ollama Cloud'u mage ile kullanmak için:
...
Mage Zen, mage ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Mage Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Mage Zen**'i arayın.

   ```txt
   /connect
   ```

3. mage API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   mage yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca mage yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri mage yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı mage ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `mage.json` dosyanızı oluşturun veya güncelleyin:
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `mage auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için mage yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, mage yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
