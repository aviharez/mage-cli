...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. mage yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
mage içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `MAGE_EXPERIMENTAL_LSP_TOOL=true` (veya `MAGE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca mage provider kullanılırken veya `MAGE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

mage başlatırken etkinleştirmek için:
