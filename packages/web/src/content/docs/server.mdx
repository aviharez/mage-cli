---
title: Server
description: Interact with mage server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `mage serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an mage client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `mage.local` |
...
Set `MAGE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `mage`, or set `MAGE_SERVER_USERNAME` to override it. This applies to both `mage serve` and `opencode web`.
...
When you run `mage` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the mage server to interact with mage programmatically.
:::

This architecture lets mage support multiple clients and allows you to interact with mage programmatically.

You can run `mage serve` to start a standalone server. If you have the
mage TUI running, `mage serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Mage [IDE](/docs/ide) plugins.
...
The mage server exposes the following APIs.
