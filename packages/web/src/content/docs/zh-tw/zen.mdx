...
Mage Zen 是由 Mage 團隊提供、經過測試與驗證的模型清單。

Zen 的運作方式和 Mage 中的其他供應商一樣。你登入 Mage Zen 並取得
...
Mage Zen 是一個 AI 閘道，讓你可以存取這些模型。

---

## 運作方式

Mage Zen 的運作方式和 Mage 中的其他供應商一樣。

1. 你登入 **<a href={console}>Mage Zen</a>**，加入帳單資訊，然後複製你的 API 金鑰。
2. 你在 TUI 中執行 `/connect` 指令，選擇 Mage Zen，然後貼上你的 API 金鑰。
...
Mage 設定中的 [模型 ID](/docs/config/#models) 會使用 `opencode/<model-id>`
...
- DeepSeek V4 Flash Free 在 Mage 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- MiMo-V2.5 Free 在 Mage 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Nemotron 3 Super Free 在 Mage 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個隱身模型，在 Mage 上限時免費提供。團隊正在利用這段時間收集回饋並改進模型。
...
4. 保持 **無鎖定**，讓你能把它與任何其他編碼代理一起使用，同時也始終能在 Mage 中使用任何其他供應商。
