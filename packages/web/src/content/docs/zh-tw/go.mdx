...
Mage Go 是一項低成本的訂閱服務——**首月 $5 美元**，之後**每月 $10 美元**——讓您能穩定使用受歡迎的開源寫程式模型。

Go 的運作方式與 Mage 中的任何其他供應商相同。您訂閱 Mage Go 並取得您的 API key。這是**完全可選的**，您不需要使用它也能使用 Mage。
...
:::tip
我們測試了一組精選的模型和供應商，它們與 Mage 搭配運作良好。
:::
...
Mage Go 讓您可以存取這些模型，**首月只需 $5 美元**，之後**每月 $10 美元**。

---

## 運作方式

Mage Go 的運作方式與 Mage 中的任何其他供應商相同。

1. 您登入 **<a href={console}>Mage Zen</a>**，訂閱 Go，然後複製您的 API key。
2. 您在 TUI 中執行 `/connect` 命令，選擇 `Mage Go`，然後貼上您的 API key。
3. 在 TUI 中執行 `/models` 即可查看透過 Go 可用的模型清單。

:::note
每個工作區只能有一位成員訂閱 Mage Go。
:::
...
您的 Mage 設定中的 [model id](/docs/config/#models) 使用 `opencode-go/<model-id>` 格式。例如，Kimi K2.6 在設定中應使用 `opencode-go/kimi-k2.6`。
...
4. 允許您在 Mage 中同時使用任何其他供應商，確保**不被單一廠商綁定**。
