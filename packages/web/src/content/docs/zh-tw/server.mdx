---
title: 伺服器
description: 透過 HTTP 與 mage 伺服器互動。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`mage serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 mage 客戶端使用。
...
設定 `MAGE_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `mage`，也可以設定 `MAGE_SERVER_USERNAME` 來覆蓋它。這適用於 `mage serve` 和 `opencode web`。
...
當你執行 `mage` 時，它會啟動一個 TUI 和一個伺服器。TUI 是與伺服器通訊的客戶端。伺服器暴露一個 OpenAPI 3.1 規範端點。該端點也用於產生 [SDK](/docs/sdk)。

:::tip
使用 mage 伺服器以程式化方式與 mage 互動。
:::

這種架構讓 mage 支援多個客戶端，並允許你以程式化方式與 mage 互動。

你可以執行 `mage serve` 來啟動一個獨立的伺服器。如果你已經在執行 mage TUI，`mage serve` 會啟動一個新的伺服器。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，你可以預填充或執行一個提示詞。此方式被 Mage [IDE](/docs/ide) 外掛所使用。
