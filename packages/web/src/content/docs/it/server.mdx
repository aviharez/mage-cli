---
title: Server
description: Interagisci con il server mage via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `mage serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client opencode.
...
Imposta `MAGE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `mage`, oppure imposta `MAGE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `mage serve` sia per `opencode web`.
...
Quando esegui `mage` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server mage per interagire con mage in modo programmatico.
:::

Questa architettura permette a mage di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `mage serve` per avviare un server standalone. Se la TUI di mage e gia in esecuzione, `mage serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Mage.
...
Il server mage espone le seguenti API.
