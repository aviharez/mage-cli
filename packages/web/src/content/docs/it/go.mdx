...
Mage Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Mage. Ti abboni a Mage Go e
...
Mage Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Mage Go funziona come qualsiasi altro provider in Mage.

1. Accedi a **<a href={console}>Mage Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Mage Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Mage Go.
:::
...
Mage Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Mage config
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Mage.
