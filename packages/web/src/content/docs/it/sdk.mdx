...
L'SDK JS/TS di mage fornisce un client type-safe per interagire con il server.
Usalo per creare integrazioni e controllare mage in modo programmatico.
...
Puoi passare un oggetto di configurazione per personalizzare il comportamento. L'istanza legge comunque `mage.json`, ma puoi sovrascrivere o aggiungere configurazione inline:
...
Se hai gia un'istanza di mage in esecuzione, puoi creare un client per collegarti:
