---
title: Provider
description: Usa qualsiasi provider LLM in Mage.
---

import config from "../../../../config.mjs"
export const console = config.console

Mage usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Mage.
...
Mage Zen e una lista di modelli forniti dal team Mage, testati e
verificati per funzionare bene con Mage. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Mage Zen.
:::

1. Esegui `/connect` nella TUI, seleziona mage e vai su [opencode.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in Mage ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Mage:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `mage.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Mage non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare mage per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché mage funzioni correttamente.
   :::
...
Mage usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Mage
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `mage.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `mage.json`:
...
Per usare Google Vertex AI con Mage:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Mage come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Mage elencata come sessione separata.
...
Puoi configurare mage per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare mage per usare modelli locali tramite LM Studio.
...
Puoi configurare mage per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Mage. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Mage:
...
3. Copia la chiave API da usare in Mage.
...
6. **Importante**: Prima di usare modelli cloud in Mage, devi scaricare le informazioni del modello localmente:
...
Mage Zen è una lista di modelli testati e verificati forniti dal team Mage. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Mage Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Mage Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Mage.
...
4. Crea o aggiorna il tuo file `mage.json` nella directory del progetto:
...
I campi `limit` permettono a Mage di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `mage auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di mage e:
