...
Le skill degli agenti permettono a Mage di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Mage cerca in queste posizioni:

- Config di progetto: `.mage/skills/<name>/SKILL.md`
- Config globale: `~/.config/mage/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Mage risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.mage/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/mage/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.mage/skills/git-release/SKILL.md` cosi':
...
Mage elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `mage.json`:
...
**Per agenti integrati** (in `mage.json`):
