...
Gli strumenti personalizzati sono funzioni che crei e che l'LLM puo chiamare durante le conversazioni. Funzionano insieme agli [strumenti integrati](/docs/tools) di mage come `read`, `write` e `bash`.
...
- In locale, mettendoli nella directory `.mage/tools/` del progetto.
- Oppure in globale, mettendoli in `~/.config/mage/tools/`.
