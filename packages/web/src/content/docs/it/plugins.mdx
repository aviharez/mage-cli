---
title: Plugin
description: Scrivi plugin per estendere Mage.
---

I plugin ti permettono di estendere Mage agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Mage.
...
- `.mage/plugins/` - plugin a livello progetto
- `~/.config/mage/plugins/` - plugin globali
...
1. Config globale (`~/.config/mage/mage.json`)
2. Config di progetto (`mage.json`)
3. Directory plugin globale (`~/.config/mage/plugins/`)
4. Directory plugin di progetto (`.mage/plugins/`)
...
Mage esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di mage per interagire con l'AI.
...
:::note
Se usi l'app desktop di Mage, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a mage di leggere i file `.env`:
...
L'helper `tool` crea uno strumento personalizzato che mage può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in mage insieme agli strumenti integrati.
