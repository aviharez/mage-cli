...
description: Usa la configurazione JSON di Mage.
---

Puoi configurare Mage usando un file di configurazione JSON.

---

## Formato

Mage supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/mage`) - default dell'organizzazione
2. **Config globale** (`~/.config/mage/mage.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `MAGE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`mage.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.mage`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `MAGE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.mage` e `~/.config/mage` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/mage`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Mage in `~/.config/mage/mage.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/mage/tui.json`.
...
Aggiungi `mage.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Mage si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `MAGE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `MAGE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.mage` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.mage`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `MAGE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `mage.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `mage serve` e `opencode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Mage.
...
Puoi configurare provider e modelli da usare in Mage tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Mage prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/mage/agents/` o `.mage/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Mage fara fallback su `"build"` mostrando un avviso.
...
Puoi anche definire comandi usando file markdown in `~/.config/mage/commands/` o `.mage/commands/`. [Scopri di piu](/docs/commands).
...
Mage scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, mage **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Mage con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.mage/plugins/` o `~/.config/mage/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Mage a usare solo alcuni provider invece di disabilitarli uno per uno.
