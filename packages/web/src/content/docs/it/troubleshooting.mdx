...
Per diagnosticare problemi con Mage, inizia controllando i log e i dati locali che salva su disco.
...
mage salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
Mage Desktop esegue in background un server locale di Mage (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Mage` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/mage.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\mage.jsonc`
...
Mage puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Mage possa ricostruirla.

1. Chiudi completamente Mage Desktop.
...
3. Riavvia Mage Desktop.

---

### Risolvi problemi di connessione al server

Mage Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `mage.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `MAGE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `MAGE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Mage Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Mage.

---

### Notifiche non visualizzate

Mage Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Mage nelle impostazioni del sistema operativo, e
...
1. Chiudi Mage Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Mage Desktop):
...
Se riscontri problemi con Mage:
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. mage installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia mage per reinstallare i pacchetti provider piu recenti

Questo forzera mage a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
mage rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
