---
title: Server LSP
description: Mage si integra con i tuoi server LSP.
---

Mage può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Mage include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `MAGE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e mage apre un file:
