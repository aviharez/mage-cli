---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Mage.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Mage fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Mage avvia la TUI per la directory corrente.
...
Quando usi la TUI di Mage, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Mage. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `mage.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Mage attende dopo il leader key. Default: `2000`.
