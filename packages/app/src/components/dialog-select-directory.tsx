...
import { List } from "@mybcabisnis/mage-ui/list"
import type { ListRef } from "@mybcabisnis/mage-ui/list"
...
function isPathLike(input: string) {
  ...
  return v.startsWith("~") || v.startsWith("/") || v.includes("/") || /^[A-Za-z]:/.test(v)
}

function resolveTyped(input: string, home: string, start: string | undefined): string | undefined {
  const value = cleanInput(input)
  if (!value) return undefined
  const raw = normalizeDriveRoot(value)
  if (!raw) return undefined
  if (raw === "~") return trimTrailing(home) || undefined
  if (raw.startsWith("~/")) return joinPath(home, raw.slice(2)) || undefined
  if (rootOf(raw)) return trimTrailing(raw) || undefined
  if (start) return joinPath(start, raw) || undefined
  return undefined
}
...
export function DialogSelectDirectory(props: DialogSelectDirectoryProps) {
  ...
  const items = async (value: string) => {
    ...
    if (isPathLike(value)) {
      const typed = resolveTyped(value, home(), start())
      if (typed && !rows.some((r) => r.absolute === typed)) {
        rows.unshift({ absolute: typed, search: typed, group: "typed" })
      }
    }
    ...
    <Dialog title={props.title ?? language.t("command.project.open")}>
      <List
        ...
        groupHeader={(group) => {
          ...
          if (group.category === "typed") return language.t("dialog.directory.customPath") || "Custom path"
                ...
                <div class="flex items-center gap-x-3 grow min-w-0">
                  ...
                  <div class="flex items-center text-14-regular min-w-0">
                    <span class="text-text-weak whitespace-nowrap overflow-hidden overflow-ellipsis truncate min-w-0">
                      {getDirectory(item.absolute)}
                    </span>
                    <span class="text-text-strong whitespace-nowrap">{getFilename(item.absolute)}</span>
