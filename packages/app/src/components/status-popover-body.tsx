...
export function StatusPopoverBody(props: { shown: Accessor<boolean> }) {
  ...
  const activeBoilerplateName = createMemo(() => {
    const mage = (sync.data.config as Record<string, any>)?.mage as
      | { activeBoilerplate?: string; boilerplate?: string }
      | undefined
    if (mage?.activeBoilerplate) return mage.activeBoilerplate
    if (mage?.boilerplate) return mage.boilerplate.split("/").at(-1) ?? mage.boilerplate
    return null
  })
    ...
    <div class="flex items-center gap-1 w-[360px] rounded-xl shadow-[var(--shadow-lg-border-base)]">
      ...
      >
        <Tabs.List data-slot="tablist" class="bg-transparent border-b-0 px-4 pt-2 pb-0 gap-4 h-10">
          ...
          <Tabs.Trigger value="boilerplate" data-slot="tab" class="text-12-regular">
            {language.t("status.popover.tab.boilerplate")}
          </Tabs.Trigger>
        </Tabs.List>
        ...
        <Tabs.Content value="boilerplate">
          <div class="flex flex-col px-2 pb-2">
            <div class="flex flex-col p-3 bg-background-base rounded-sm min-h-14">
              <Show
                when={activeBoilerplateName()}
                fallback={
                  <div class="text-14-regular text-text-base text-center my-auto">
                    {language.t("dialog.boilerplate.empty")}
                  </div>
                }
              >
                <div class="flex items-center gap-2 w-full px-2 py-1">
                  <div class="size-1.5 rounded-full shrink-0 bg-icon-success-base" />
                  <span class="text-14-regular text-text-base truncate">{activeBoilerplateName()}</span>
                </div>
              </Show>
            </div>
          </div>
