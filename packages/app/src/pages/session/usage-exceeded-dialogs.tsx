...
import { SessionStatus } from "@mybcabisnis/mage-sdk/v2"
import { onCleanup } from "solid-js"
import { createStore } from "solid-js/store"
import { useSessionLayout } from "./session-layout"
import { useDialog } from "@mybcabisnis/mage-ui/context"
import { DialogUsageExceeded } from "@/components/dialog-usage-exceeded"
import { useI18n } from "@mybcabisnis/mage-ui/context"
...
type SessionStatusWithAction =
  | Exclude<SessionStatus, { type: "retry" }>
  | (Extract<SessionStatus, { type: "retry" }> & { action?: RetryAction })
...
function goUpsellKeys(status: SessionStatusWithAction) {
...
export function useUsageExceededDialogs() {
  ...
  onCleanup(
    sdk.event.on("session.status", (evt) => {
      ...
      const status = evt.properties.status as SessionStatusWithAction
      const { action } = status as Extract<SessionStatusWithAction, { type: "retry" }>
      if (!action) return
