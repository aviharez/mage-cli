...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    magePassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteMage)
    ? value.remoteMage
    : isRecord(value.remote_mage)
      ? value.remote_mage
      : {};
  ...
  const magePassword = parseStoredSecret(authRaw.magePassword || authRaw.mage_password);
...
export const listenDesktopSshStatus = async (
  ...
  const unlisten = await listen('mage:ssh-instance-status', (event) => {
