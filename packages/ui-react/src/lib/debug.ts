...
export const debugUtils = {
  ...
  async getAppStatus() {
    ...
    const mageDirectory = mageClient.getDirectory() ?? null;
    ...
    const isDesktopRuntime = typeof window !== 'undefined' && Boolean((window as { __MAGE_ELECTRON__?: unknown }).__MAGE_ELECTRON__);
    ...
    let mageHealth: unknown = null;

    try {
      const pathResult = await mageClient.getSdkClient().path.get(
    ...
    try {
      const projectResult = await mageClient.getSdkClient().project.current(
        currentDirectory ? { directory: currentDirectory } : undefined
      );
    ...
    try {
      ...
      mageHealth = {
        ...
        magePort: parsed?.magePort ?? null,
        mageRunning: parsed?.mageRunning ?? null,
        mageSecureConnection: parsed?.mageSecureConnection ?? null,
        mageAuthSource: parsed?.mageAuthSource ?? null,
        isMageReady: parsed?.isMageReady ?? null,
        lastMageError: parsed?.lastMageError ?? null,
        preview: body ? body.slice(0, 120) : null,
      };
    } catch (error) {
      mageHealth = { error: error instanceof Error ? error.message : String(error) };
    }
    ...
    const report = {
      ...
      directories: {
        currentDirectory,
        mageDirectory: (pathInfo as { directory?: string; worktree?: string } | null)?.directory
  ...
  findEmptyMessages() {
   ...
   showRetryHelp() {
    ...
    console.log('   __mageDebug.getLastAssistantMessage()\n');
    console.log('2. Find all empty messages in session:');
    console.log('   __mageDebug.findEmptyMessages()\n');
...
if (typeof window !== 'undefined') {
  (window as any).__mageDebug = debugUtils;
  if (streamDebugEnabled()) {
    console.log('[DEBUG] Mage Debug Utils loaded! Use window.__mageDebug in console');
    console.log('Available commands:');
    console.log('  __mageDebug.getLastAssistantMessage() - Get last assistant message details');
    console.log('  __mageDebug.getAllMessages(truncate?) - List all messages (truncate=true for short preview)');
    console.log('  __mageDebug.truncateMessages(messages) - Truncate long fields in messages array');
    console.log('  __mageDebug.getAppStatus() - Show app status snapshot');
    console.log('  __mageDebug.checkLastMessage() - Check if last message is problematic');
    console.log('  __mageDebug.findEmptyMessages() - Find all empty assistant messages');
    console.log('  __mageDebug.showRetryHelp() - Show instructions for handling empty responses');
    console.log('  __mageDebug.getStreamingState() - Get streaming state info');
    console.log('  __mageDebug.analyzeMessageCompletionConsistency(opts?) - Compare time.completed vs part timings');
    console.log('  __mageDebug.checkCompletionStatus() - Check completion status of last message');
  }
