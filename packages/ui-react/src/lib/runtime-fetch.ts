...
export const sanitizeHeadersForBrowser = (init?: HeadersInit): [string, string][] | undefined => {
  ...
  for (const [key, value] of sourceEntries) {
    if (shouldEncodeHeaderValue(key, value)) {
      ...
      dirty = true;
      if (key.toLowerCase() === 'x-mage-directory') encodedDirectoryHint = true;
    } else {
      entries.push([key, value]);
    }
  }
  if (encodedDirectoryHint) {
    entries.push(['x-mage-directory-encoding', 'uri']);
  }
...
// Mage process and delays everything queued behind it (e.g. createSession).
// Coalesce genuinely-concurrent identical GETs to those read endpoints so
// Mage does the work once; every caller gets an independent `clone()`.
//
