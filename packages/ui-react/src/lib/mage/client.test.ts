...
mock.module('@mybcabisnis/mage-sdk/v2', () => ({
  createMageClient: mock(() => ({
    config: {
      get: mock(() => {
        configCalls += 1;
...
const { mageClient } = await import(`./client?cache-test=${Date.now()}`);
...
describe('mageClient getConfig cache', () => {
  test('cleared stale in-flight requests do not repopulate cache or delete newer in-flight requests', async () => {
    ...
    const cached = await mageClient.getConfig('/workspace/project');
    expect(cached).toEqual({ model: 'new/model' });
    expect(configCalls).toBe(2);
  });
});

describe('mageClient prompt retry behavior', () => {
  const sendPrompt = (providerID = 'anthropic') => mageClient.sendMessage({
    id: 'ses_1',
    providerID,
