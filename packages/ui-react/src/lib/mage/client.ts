import { createMageClient, MageClient } from "@mybcabisnis/mage-sdk/v2";
import type { PermissionV2Request, PermissionV2Effect, PermissionV2Source } from "@mybcabisnis/mage-sdk/v2/client";
...
/**
 * Tagged result of `MageService.fetchPermission()`. The caller can
...
const ensureAbsoluteBaseUrl = (candidate: string): string => {
  ...
  try {
    ...
    console.warn("Failed to normalize Mage base URL:", error);
    return normalized;
  }
};
...
const createRuntimeMageClient = (config: { baseUrl: string; directory?: string }): MageClient => {
...
class MageService {
  private client: MageClient;
  private baseUrl: string;
  private scopedClients: Map<string, MageClient> = new Map();
  ...
  constructor(baseUrl: string = DEFAULT_BASE_URL) {
    ...
    this.client = createRuntimeMageClient({ baseUrl: this.baseUrl });
  }
  ...
  getSdkClient(): MageClient {
    return this.client;
  }

  /** Get a scoped SDK client for a specific directory */
  getScopedSdkClient(directory: string): MageClient {
   ...
   */
  getScopedApiClient(directory: string): MageClient {
    ...
    const scoped = createRuntimeMageClient({ baseUrl: this.baseUrl, directory: normalized });
  ...
  setDirectory(directory: string | undefined) {
    ...
    if (this.currentDirectory !== normalized) {
      markStartupTrace('mageClient:setDirectory', {
  ...
  /**
   * Best-effort probe whether a directory is accessible to Mage.
   ...
   * Mage SDK v1.17.12. Wraps `session.permission.create`.
   *
   ...
   * endpoint introduced in Mage SDK v1.17.12. Wraps
  ...
  async getConfig(directory?: string | null): Promise<Config> {
    ...
    if (cached && cached.expiresAt > Date.now()) {
      markStartupTrace('mageClient.getConfig:cacheHit', { directory: effectiveDirectory ?? null });
      return cached.config;
    }

    const existing = this.configInFlight.get(key);
    if (existing) {
      markStartupTrace('mageClient.getConfig:deduped', { directory: effectiveDirectory ?? null });
      return existing;
    }

    const generation = this.configCacheGeneration;
    const request = (async () => {
      markStartupTrace('mageClient.getConfig:start', { directory: effectiveDirectory ?? null });
      ...
      markStartupTrace('mageClient.getConfig:end', {
  ...
  async listAgents(directory?: string | null): Promise<Agent[]> {
    ...
    const request = (async () => {
      ...
      // SDK gap / endpoint drift: current Mage exposes the authoritative
  ...
  async checkHealth(): Promise<boolean> {
    try {
      ...
      const healthUrl = normalizedBase === '/api' || normalizedBase.endsWith('/api')
        ? '/api/mage/health'
        : `${normalizedBase}/mage/health`;
      markStartupTrace('mageClient.checkHealth:url', { baseUrl: this.baseUrl, healthUrl });
      ...
      markStartupTrace('mageClient.checkHealth:response', { status: response.status });
      if (!response.ok) {
        return false;
      }

      const healthData = await response.json();
      markStartupTrace('mageClient.checkHealth:result', { healthy: healthData?.healthy });
  ...
  async setMageWorkingDirectory(directoryPath: string | null | undefined): Promise<DirectorySwitchResult | null> {
    if (!directoryPath || typeof directoryPath !== 'string' || !directoryPath.trim()) {
      console.warn('[MageClient] setMageWorkingDirectory: invalid path', directoryPath);
      return null;
    }

    const url = `${this.baseUrl}/mage/directory`;
    console.log('[MageClient] POST', url, 'with path:', directoryPath);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message =
          typeof error.error === 'string' && error.error.length > 0
            ? error.error
            : 'Failed to update Mage working directory';
        throw new Error(message);
      }
      ...
      console.warn('Failed to update Mage working directory:', error);
      throw error;
    }
  }
}
...
export const mageClient = new MageService();
