...
const createMageClientMock = mock(() => ({
  v2: {
    session: {
      permission: {
...
mock.module('@mybcabisnis/mage-sdk/v2', () => ({
  createMageClient: createMageClientMock,
}));
...
const { mageClient } = await import(`./client?cache-test-permission=${Date.now()}`);
...
describe('mageClient.fetchPermission', () => {
  test('returns state="ok" with the permission when the server returns 200', async () => {
    ...
    const promise = mageClient.fetchPermission('ses_1', 'perm_1');
  ...
  test('returns state="resolved" when the server returns 404', async () => {
    const promise = mageClient.fetchPermission('ses_1', 'perm_gone');
