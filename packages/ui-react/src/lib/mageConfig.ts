...
interface MageConfig {
  ...
  projectNotes?: string;
  projectTodos?: MageProjectTodoItem[];
  projectPlanFiles?: MageProjectPlanFileLink[];
  projectActions?: MageProjectAction[];
  projectActionsPrimaryId?: string;
  draftStarters?: DraftStarterRef[];
}

type MageProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface MageProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: MageProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface MageProjectActionsState {
  actions: MageProjectAction[];
  primaryActionId: string | null;
}

export interface MageProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface MageProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface MageProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface MageProjectNotesTodos {
  notes: string;
  todos: MageProjectTodoItem[];
}

export interface MageProjectContextData extends MageProjectNotesTodos {
  plans: MageProjectPlanFileLink[];
}

export const MAGE_PROJECT_NOTES_MAX_LENGTH = 3000;
export const MAGE_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
const MAGE_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
const MAGE_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
const MAGE_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
const MAGE_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;
const MAGE_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;

const MAGE_ACTION_PLATFORM_SET = new Set<MageProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const getBaseUrl = (): string => {
  const defaultBaseUrl = import.meta.env.VITE_MAGE_URL || '/api';
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__MAGE_HOME__ can be workspace/project-root
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, MAGE_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): MageProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: MageProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), MAGE_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): MageProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: MageProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): MageProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: MageProjectActionPlatform[] = [];
  const seen = new Set<MageProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as MageProjectActionPlatform;
    if (!MAGE_ACTION_PLATFORM_SET.has(normalized) || seen.has(normalized)) {
      continue;
    }
    seen.add(normalized);
...
const sanitizeProjectActions = (value: unknown): MageProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: MageProjectAction[] = [];
  const seenIds = new Set<string>();
  ...
  for (const entry of value) {
    ...
    const id = typeof record.id === 'string' ? record.id.trim() : '';
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', MAGE_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', MAGE_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, MAGE_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      MAGE_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): MageProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): MageProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): MageProjectContextData => {
...
const sanitizePlanTitle = (value: string): string => {
  return trimToMaxLength(value.trim(), MAGE_PROJECT_PLAN_TITLE_MAX_LENGTH);
};
 ...
 */
async function readMageConfig(project: ProjectRef): Promise<MageConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): MageConfig | null => {
  ...
  try {
    const wrote = await writeMageConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyMageConfig(projectDirectory);
    }
  } catch {
 ...
 */
async function writeMageConfig(
  project: ProjectRef,
  config: MageConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  ...
  try {
    ...
    console.error('Failed to write mage config:', error);
    return false;
  }
}
 ...
 */
async function updateMageConfig(
  project: ProjectRef,
  updates: Partial<MageConfig>
): Promise<boolean> {
  const existing = await readMageConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeMageConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readMageConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateMageConfig(project, { 'setup-worktree': filtered });
}
...
export async function saveWorktreeSetupWaitEnabled(project: ProjectRef, enabled: boolean): Promise<boolean> {
  return updateMageConfig(project, { 'setup-worktree-wait': enabled });
}
...
export async function saveProjectDraftStarters(project: ProjectRef, starters: DraftStarterRef[]): Promise<boolean> {
  return updateMageConfig(project, { draftStarters: sanitizeStarterRefs(starters) });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<MageProjectNotesTodos> {
  const config = await readMageConfig(project);
...
export async function saveProjectNotesAndTodos(
  ...
  return updateMageConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<MageProjectContextData> {
  const config = await readMageConfig(project);
...
async function getProjectPlanFiles(project: ProjectRef): Promise<MageProjectPlanFileLink[]> {
  const config = await readMageConfig(project);
...
export async function readProjectPlanFile(path: string): Promise<MageProjectPlanFile | null> {
...
): Promise<MageProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<MageProjectActionsState> {
  const config = await readMageConfig(project);
...
async function deleteLegacyMageConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
