...
declare global {
  interface Window {
    __MAGE_STARTUP_TRACE__?: StartupTraceEvent[];
    __MAGE_STARTUP_TRACE_START__?: number;
  }
}
...
const enabled = () => {
  ...
  try {
    ...
    return params.get('startupTrace') === '1' || window.localStorage?.getItem('MAGE_STARTUP_TRACE') === '1';
  } catch {
    return false;
  }
};
...
export const markStartupTrace = (name: string, data?: Record<string, unknown>) => {
  ...
  window.__MAGE_STARTUP_TRACE_START__ ??= now;
  window.__MAGE_STARTUP_TRACE__ ??= [];
  window.__MAGE_STARTUP_TRACE__.push({
    t: Math.round(now - window.__MAGE_STARTUP_TRACE_START__),
    name,
    ...(data ? { data } : {}),
  });
  if (window.__MAGE_STARTUP_TRACE__.length > MAX_STARTUP_TRACE_EVENTS) {
    window.__MAGE_STARTUP_TRACE__.splice(
      0,
      window.__MAGE_STARTUP_TRACE__.length - MAX_STARTUP_TRACE_EVENTS,
    );
  }
};

const getStartupTraceSummary = () => {
  const trace = typeof window !== 'undefined' ? window.__MAGE_STARTUP_TRACE__ ?? [] : [];
  ...
  return trace.slice(0, endIndex).filter((event) => (
    ...
    || event.name.includes('mageClient:setDirectory')
  ));
};

if (typeof window !== 'undefined') {
  (window as typeof window & { __MAGE_STARTUP_TRACE_SUMMARY__?: typeof getStartupTraceSummary })
    .__MAGE_STARTUP_TRACE_SUMMARY__ = getStartupTraceSummary;
}
