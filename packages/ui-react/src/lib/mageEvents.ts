...
type MageEvent = ScheduledTaskRanEvent;
type Listener = (event: MageEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'mage:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'mage:heartbeat') {
    return;
  }

  if (envelope.type !== 'mage:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const source = new EventSource(getRuntimeUrlResolver().sse('/api/mage/events'));
  source.onopen = () => {
    resetHeartbeatTimer();
  };
...
export const subscribeMageEvents = (listener: Listener): (() => void) => {
