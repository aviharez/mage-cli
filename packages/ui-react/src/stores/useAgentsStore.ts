...
import type { Agent, PermissionConfig } from "@mybcabisnis/mage-sdk/v2";
...
// useDirectoryStore -> useAgentsStore (for refreshAfterMageRestart)
...
const getConfigDirectory = (): string | null => {
  try {
    ...
    // 2. Fallback: current Mage directory (session / runtime)
    const clientDir = mageClient.getDirectory();
    if (clientDir?.trim()) {
 ...
 * connected (external) Mage server could not be reloaded by Mage, so
...
// Checks both top-level hidden and options.hidden (Mage API inconsistency workaround)
      ...
      (set, get) => ({
          ...
          const request = (async () => {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
              try {
                ...
                const agents = await mageClient.listAgents(configDirectory);
                    ...
                    try {
                      ...
                      const response = await runtimeFetch(`/api/config/agents/${encodeURIComponent(agent.name)}${queryParams}`, {
                        headers: {
                          ...
                          ...(configDirectory ? { 'x-mage-directory': configDirectory } : {}),
                        }
                      });
        ...
        createAgent: async (config: AgentConfig) => {
          ...
          try {
            ...
            // External Mage server: persisted to disk but not reloaded.
...
async function waitForMageConnection(delayMs?: number) {
  ...
  while (Date.now() - start < MAX_HEALTH_WAIT_MS) {
    attempt += 1;
    updateConfigUpdateMessage(`Waiting for Mage… (attempt ${attempt})`);

    try {
      const isHealthy = await mageClient.checkHealth();
      if (isHealthy) {
        return;
      }
      lastError = new Error("Mage health check reported not ready");
    } catch (error) {
      lastError = error;
    }
  ...
  throw lastError || new Error("Mage did not become ready in time");
}
...
async function performConfigRefresh(options: {
  ...
  try {
    await waitForMageConnection(delayMs);
    ...
    updateConfigUpdateMessage("Mage refresh failed. Please retry.");
    await sleep(1500);
    throw error;
  } finally {
    finishConfigUpdate();
...
export async function refreshAfterMageRestart(options?: {
  message?: string;
  delayMs?: number;
  scopes?: ConfigChangeScope[];
  mode?: ConfigRefreshMode;
...
export async function reloadMageConfiguration(options?: {
  message?: string;
  delayMs?: number;
  scopes?: ConfigChangeScope[];
  mode?: ConfigRefreshMode;
}) {
  startConfigUpdate(options?.message || "Reloading Mage configuration…");
  ...
  try {
    ...
    if (payload?.requiresReload) {
      ...
      await refreshAfterMageRestart(refreshOptions);
    }
  } catch (error) {
    console.error('[reloadMageConfiguration] Failed:', error);
