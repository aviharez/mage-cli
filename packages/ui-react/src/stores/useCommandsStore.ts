...
// Built-in commands provided by Mage (not defined in user config directories)
      ...
      (set, get) => ({
          ...
          const request = (async () => {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
              try {
                ...
                const commands = await mageClient.withDirectory(
                  directory,
                  () => mageClient.listCommandsWithDetails()
                );
                    ...
                    try {
                      ...
                      const response = await runtimeFetch(`/api/config/commands/${encodeURIComponent(cmd.name)}${queryParams}`, {
                        headers: {
                          ...
                          ...(directory ? { 'x-mage-directory': directory } : {}),
                        }
                      });
        ...
        deleteCommand: async (name: string) => {
          ...
          try {
            ...
            const response = await runtimeFetch(`/api/config/commands/${encodeURIComponent(name)}${queryParams}`, {
              method: 'DELETE',
              headers: directory ? { 'x-mage-directory': directory } : undefined,
            });
...
async function performFullConfigRefresh(options: { message?: string; delayMs?: number } = {}) {
  ...
  try {
    ...
    console.error("[CommandsStore] Failed to refresh configuration after Mage restart:", error);
    updateConfigUpdateMessage("Mage refresh failed. Please retry refreshing configuration manually.");
    await sleep(1500);
    throw error;
  } finally {
    finishConfigUpdate();
