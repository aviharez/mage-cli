...
const getHomeDirectory = () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    const desktopHome =
      (typeof window.__MAGE_HOME__ === 'string' && window.__MAGE_HOME__.length > 0
        ? window.__MAGE_HOME__
        : null);
...
const initializeHomeDirectory = async () => {
  ...
  try {
    const fsHome = await mageClient.getFilesystemHome();
  ...
  try {
    const info = await mageClient.getSystemInfo();
...
if (initialCurrentDirectory) {
  mageClient.setDirectory(initialCurrentDirectory);
}
  ...
  devtools(
    (set, get) => ({
      ...
      setDirectory: (path: string, options?: { showOverlay?: boolean }) => {
        ...
        mageClient.setDirectory(resolvedPath);
      ...
      goBack: () => {
        ...
        if (state.historyIndex > 0) {
          ...
          mageClient.setDirectory(newDirectory);
          invalidateFileSearchCache();
      ...
      synchronizeHomeDirectory: (homePath: string) => {
        ...
        if ((shouldReplaceCurrent || currentChanged) && resolvedReady) {
          ...
          mageClient.setDirectory(nextDirectory);
          invalidateFileSearchCache();
