...
const AUTO_DISCOVER_ACTION_ID = '__mage_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<MageProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<MageProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: MageProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: MageProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<MageProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: MageProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: MageProjectAction, toggleStopIfRunning = false) => {
