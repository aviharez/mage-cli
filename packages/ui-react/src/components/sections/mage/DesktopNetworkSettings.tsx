...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.mage.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.mage.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleLaunchAtLoginToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.mage.desktopNetwork.error.launchAtLoginUnsupported'));
      }
      setLaunchAtLoginEnabled(status.enabled);
    } catch (cause) {
      setLaunchAtLoginEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.mage.desktopNetwork.error.launchAtLoginSaveFailed'));
    } finally {
  ...
  const handleMinimizeToTrayToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status) {
        throw new Error(t('settings.mage.desktopNetwork.error.minimizeToTraySaveFailed'));
      }
      if (!status.supported) {
        throw new Error(t('settings.mage.desktopNetwork.error.minimizeToTrayUnsupported'));
      }
      setMinimizeToTrayEnabled(status.enabled);
    } catch (cause) {
      setMinimizeToTrayEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.mage.desktopNetwork.error.minimizeToTraySaveFailed'));
    } finally {
  ...
  const handleKeepAwakeToggle = React.useCallback(async () => {
    ...
    try {
      ...
      if (!status?.supported) {
        throw new Error(t('settings.mage.desktopNetwork.error.keepAwakeUnsupported'));
      }
      setKeepAwakeEnabled(status.enabled);
    } catch (cause) {
      setKeepAwakeEnabled(!nextValue);
      setError(cause instanceof Error ? cause.message : t('settings.mage.desktopNetwork.error.keepAwakeSaveFailed'));
    } finally {
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.mage.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.mage.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.mage.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <div className="mb-8">
      <div className="mb-1 px-1">
        <h3 className="typography-ui-header font-medium text-foreground">{t('settings.mage.desktopNetwork.title')}</h3>
      </div>
      ...
      <section className="space-y-2 px-2 pb-2 pt-0">
        {launchAtLoginSupported ? (
          ...
          >
            <Checkbox
              ...
              ariaLabel={t('settings.mage.desktopNetwork.field.launchAtLoginAria')}
              disabled={isSavingLaunchAtLogin}
            />
            <div className="min-w-0 flex-1">
              <div className="typography-ui-label text-foreground">{t('settings.mage.desktopNetwork.field.launchAtLogin')}</div>
              <div className="typography-micro text-muted-foreground/70">
                {t('settings.mage.desktopNetwork.field.launchAtLoginDescription')}
              </div>
            </div>
          </div>
        ) : null}
        ...
        {minimizeToTraySupported ? (
          ...
          >
            <Checkbox
              ...
              ariaLabel={t('settings.mage.desktopNetwork.field.minimizeToTrayAria')}
              disabled={isSavingMinimizeToTray}
            />
            <div className="min-w-0 flex-1">
              <div className="typography-ui-label text-foreground">{t('settings.mage.desktopNetwork.field.minimizeToTray')}</div>
              <div className="typography-micro text-muted-foreground/70">
                {t('settings.mage.desktopNetwork.field.minimizeToTrayDescription')}
              </div>
            </div>
          </div>
        ) : null}
        ...
        {keepAwakeSupported ? (
          ...
          >
            <Checkbox
              ...
              ariaLabel={t('settings.mage.desktopNetwork.field.keepAwakeAria')}
              disabled={isSavingKeepAwake}
            />
            <div className="min-w-0 flex-1">
              <div className="typography-ui-label text-foreground">{t('settings.mage.desktopNetwork.field.keepAwake')}</div>
              <div className="typography-micro text-muted-foreground/70">
                {t('settings.mage.desktopNetwork.field.keepAwakeDescription')}
              </div>
            </div>
          </div>
        ) : null}
        ...
        <div data-settings-item="sessions.desktop-ui-password" className="space-y-1 py-1.5">
          <label className="typography-ui-label text-foreground" htmlFor="desktop-ui-password">
            {t('settings.mage.desktopPassword.field.password')}
          </label>
          <Input
            ...
            placeholder={t('settings.mage.desktopPassword.field.passwordPlaceholder')}
            disabled={isLoading || isSaving}
            required={draftValue}
            aria-invalid={lanRequiresPassword}
          />
          <div className="typography-micro text-muted-foreground/70">
            {t('settings.mage.desktopPassword.field.passwordDescription')}
          </div>
        </div>
        ...
        >
          <Checkbox
            ...
            ariaLabel={t('settings.mage.desktopNetwork.field.allowLanAccessAria')}
            disabled={isLoading || isSaving}
          />
          <div className="min-w-0 flex-1">
            <div className="typography-ui-label text-foreground">{t('settings.mage.desktopNetwork.field.allowLanAccess')}</div>
            <div className="typography-micro text-muted-foreground/70">
              {t('settings.mage.desktopNetwork.field.allowLanAccessDescription')}
            </div>
            <div className="typography-micro text-[var(--status-warning)]/85">
              {t('settings.mage.desktopNetwork.field.warning')}
            </div>
            {lanRequiresPassword || lanBlockedByMissingPassword ? (
              <div className="typography-micro text-[var(--status-warning)]/85">
                {t('settings.mage.desktopNetwork.field.passwordRequiredWarning')}
              </div>
            ) : null}
          </div>
        </div>
        ...
        {lanUrl ? (
          <div className="px-2 typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.mage.desktopNetwork.hint.openAfterRestart')
              : t('settings.mage.desktopNetwork.hint.openNow')}
