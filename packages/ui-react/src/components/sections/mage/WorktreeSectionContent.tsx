...
export const WorktreeSectionContent: React.FC<WorktreeSectionContentProps> = ({ projectRef: projectRefProp = null }) => {
  ...
  const persistSetupCommands = React.useCallback(async (commands: string[]): Promise<boolean> => {
    ...
    try {
      ...
      if (!ok) {
        toast.error(t('settings.mage.worktrees.setup.toast.saveFailed'));
        return false;
      }
  ...
  const setupTooltip = (
    <Tooltip>
      ...
      <TooltipContent sideOffset={8} className="max-w-xs">
        {t('settings.mage.worktrees.setup.tooltipPrefix')}
        {' '}
        <code className="font-mono text-xs bg-sidebar-accent/50 px-1 rounded">$ROOT_PROJECT_PATH</code>
        {' '}
        {t('settings.mage.worktrees.setup.tooltipSuffix')}
      </TooltipContent>
    </Tooltip>
  );
  ...
  const listTooltip = (
    <Tooltip>
      ...
      <TooltipContent sideOffset={8} className="max-w-xs">
        {t('settings.mage.worktrees.list.tooltip')}
      </TooltipContent>
    </Tooltip>
  );
    ...
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.mage.worktrees.state.selectProject')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  if (isGitRepoLocal === false) {
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.mage.worktrees.state.gitOnly')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  return (
    <>
      ...
      >
        {isLoadingCommands ? (
          <p className="typography-meta text-muted-foreground">{t('settings.mage.worktrees.setup.loading')}</p>
        ) : (
          <div className={cn('space-y-2', PROJECT_SETTINGS_CONTROL_WIDTH)}>
            {setupCommands.map((command, index) => (
              <div key={index} className="flex w-full gap-2">
                <Input
                  ...
                  placeholder={t('settings.mage.worktrees.setup.commandPlaceholder')}
                ...
                <button
                  ...
                  aria-label={t('settings.mage.worktrees.setup.removeCommandAria')}
                >
            ...
            >
              ...
              {t('settings.mage.worktrees.setup.addCommand')}
            </Button>
            <label
            ...
            >
              <Checkbox
                ...
                ariaLabel={t('settings.mage.worktrees.setup.waitForCommandsAria')}
              />
              <span className={cn(
                'typography-ui-label font-normal',
                waitForSetupCommands ? 'text-foreground' : 'text-foreground/60'
              )}>
                {t('settings.mage.worktrees.setup.waitForCommands')}
              </span>
            </label>
          </div>
        )}
      ...
      <ProjectSettingsSubsection
        title={t('settings.mage.worktrees.list.title')}
        titleAccessory={listTooltip}
      >
        {isLoadingWorktrees ? (
          <p className="typography-meta text-muted-foreground">{t('settings.mage.worktrees.list.loading')}</p>
        ) : availableWorktrees.length === 0 ? (
          <p className="typography-meta text-muted-foreground/70">
            {t('settings.mage.worktrees.list.empty')}
          </p>
        ) : (
            ...
            {availableWorktrees.map((worktree) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 items-center gap-2">
                    <p className="typography-meta min-w-0 truncate text-foreground">
                      {worktree.label || worktree.branch || t('settings.mage.worktrees.list.detachedHead')}
                    </p>
                ...
                <button
                  ...
                  aria-label={t('settings.mage.worktrees.list.deleteWorktreeAria', { name: worktree.branch || worktree.label || worktree.path })}
                >
