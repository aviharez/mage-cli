    ...
    try {
      ...
      if (!oauthRedirectUri.trim() && !isVSCodeAuthRuntime) {
        ...
        if (saved.reloadFailed) {
          throw new Error(saved.warning || saved.message || t('settings.mcp.page.toast.mageReloadFailedAfterCallbackSave'));
        }
            ...
            <div className={cn('rounded-lg border p-3', statusCardClass(effectiveRuntimeStatus?.status))}>
              ...
              {isAuthPolling && (
                <p className="mt-4 typography-micro text-muted-foreground">
                  {t('settings.mcp.page.auth.waitingForMage')}
                </p>
              )}
            </div>
          </div>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <code className="text-foreground">mage.json</code>.
              {' '}
