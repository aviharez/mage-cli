...
type ProviderId = 'mage-go' | 'ollama-cloud' | 'cursor';
...
export const QuotaCredentials: React.FC<{ providerId: ProviderId; providerName: string }> = ({ providerId, providerName }) => {
  ...
  const request = async (path: string, method: string, body?: object) => {
    ...
    try {
      ...
      toast.success(t('settings.providers.page.mageGo.saved'));
    } catch (error) { toast.error(error instanceof Error && error.message ? error.message : t('settings.providers.page.mageGo.saveFailed')); }
  ...
  return <div data-settings-item={`providers.${providerId}-credentials`} className="mb-8">
    ...
    <section className="space-y-3 px-2 pb-2 pt-0">
      {providerId === 'mage-go' && field('workspaceId', t('settings.providers.page.mageGo.workspaceId'), 'wrk_...')}
      {providerId === 'mage-go' && field('authCookie', t('settings.providers.page.mageGo.authCookie'), 'auth=...')}
      {providerId === 'ollama-cloud' && field('cookie', t('settings.providers.page.mageGo.authCookie'), 'session=...')}
      ...
      <div className="flex flex-wrap gap-2">
        <Button size="xs" disabled={busy} onClick={() => request(route, 'PUT', values)}>{status?.configured ? t('settings.providers.page.mageGo.replace') : t('settings.providers.page.mageGo.save')}</Button>
        {status?.configured && <Button variant="outline" size="xs" disabled={busy} onClick={() => request(`${route}/validate`, 'POST')}>{t('settings.providers.page.mageGo.validate')}</Button>}
        ...
        {status?.configured && <Button variant="destructive" size="xs" disabled={busy} onClick={() => request(route, 'DELETE')}>{t('settings.providers.page.mageGo.delete')}</Button>}
      </div>
    </section>
  </div>;
};
