...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({ visible }: WindowsWindowControlsProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('mage:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('mage:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
