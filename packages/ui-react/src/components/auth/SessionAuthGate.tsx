...
const TRUST_DEVICE_STORAGE_KEY = 'mage.uiAuth.trustDevice';
...
const issueDesktopClientToken = async (): Promise<string> => {
  ...
  const response = await runtimeFetch('/api/client-auth/clients', {
    ...
    body: JSON.stringify({ label: 'Mage Desktop', ...desktopClientAuthMetadata() }),
  }).catch(() => null);
  if (!response?.ok) {
    return '';
  }
