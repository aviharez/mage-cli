/**
 * Pure decision helpers for the Mage update toast and PWA install toast.
 *
 * Extracted from `MageUpdateToast.tsx` and `usePwaInstallPrompt.ts` so the
...
export interface MageUpdateToastDecisionInput {
...
/**
 * Returns `true` if the Mage update toast should be shown for `version`.
 *
 ...
 */
export const shouldShowMageUpdateToast = (
  input: MageUpdateToastDecisionInput,
): boolean => {
...
/**
 * Coerces the `detail.version` carried by an `mage:mage-update-available`
 ...
 */
export const resolveMageUpdateVersion = (detail: unknown): string => {
...
export interface MageUpgradeStatusLike {
  readonly available?: boolean | null;
  readonly latestVersion?: string | null;
}

/**
 * Pulls the candidate version out of an `/api/mage/upgrade-status` JSON
 * payload. Returns `''` when the payload is missing the field, has the wrong
 * type, or reports `available !== true`.
 */
export const resolveMageUpgradeStatusVersion = (
  status: MageUpgradeStatusLike | null | undefined,
): string => {
  if (!status) return '';
