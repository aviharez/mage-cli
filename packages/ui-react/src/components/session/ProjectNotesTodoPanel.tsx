...
import {
  ...
  MAGE_PROJECT_NOTES_MAX_LENGTH,
  readProjectPlanFile,
  MAGE_PROJECT_TODO_TEXT_MAX_LENGTH,
  saveProjectNotesAndTodos,
  type MageProjectPlanFileLink,
  type MageProjectTodoItem,
  type ProjectRef,
} from '@/lib/mageConfig';
...
type ProjectPlanListItem = MageProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: MageProjectPlanFileLink,
...
const sortTodosWithCompletedLast = (items: MageProjectTodoItem[]): MageProjectTodoItem[] => [
  ...items.filter((todo) => !todo.completed),
  ...items.filter((todo) => todo.completed),
];

const insertTodoBeforeCompleted = (items: MageProjectTodoItem[], item: MageProjectTodoItem): MageProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<MageProjectTodoItem[]>([]);
  ...
  const initializeNewMageSession = useSessionUIStore((state) => state.initializeNewMageSession);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: MageProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('mage:project-plan-saved', handleProjectContextRefresh);
    window.addEventListener('mage:project-notes-updated', handleProjectContextRefresh);
    return () => {
      window.removeEventListener('mage:project-plan-saved', handleProjectContextRefresh);
      window.removeEventListener('mage:project-notes-updated', handleProjectContextRefresh);
    };
  }, [projectRef]);
  ...
  const handleAddTodo = React.useCallback(() => {
    ...
    const nextTodos = insertTodoBeforeCompleted(todos, {
      id: createTodoId(),
      text: trimmed.slice(0, MAGE_PROJECT_TODO_TEXT_MAX_LENGTH),
      completed: false,
      createdAt: Date.now(),
    });
    setTodos(nextTodos);
  ...
  const todoInputValue = newTodoText.slice(0, MAGE_PROJECT_TODO_TEXT_MAX_LENGTH);
  ...
  const handleConfirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewMageSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewMageSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
  ...
  const handleDeletePlan = React.useCallback(
    async (planId: string) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('mage:project-plan-saved', {
    ...
    <div className={cn('w-full min-w-0 space-y-3 p-3', className)}>
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{notes.length}/{MAGE_PROJECT_NOTES_MAX_LENGTH}</span>
        </div>
        <Textarea
          value={notes}
          onChange={(event) => setNotes(event.target.value.slice(0, MAGE_PROJECT_NOTES_MAX_LENGTH))}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{todoInputValue.length}/{MAGE_PROJECT_TODO_TEXT_MAX_LENGTH}</span>
        </div>

        <div className="flex items-center gap-1.5">
          <Input
            value={todoInputValue}
            onChange={(event) => setNewTodoText(event.target.value.slice(0, MAGE_PROJECT_TODO_TEXT_MAX_LENGTH))}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                event.preventDefault();
                handleAddTodo();
