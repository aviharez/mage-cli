/**
 * Mage deep-link vocabulary — the single source of truth for the `mage://`
...
export const DEEP_LINK_SCHEME = 'mage';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (mage://session/<id>),
  // but be tolerant of authority-less forms (mage:/session/<id>) where it lands in
...
/**
 * Parse a raw `mage://…` string into a typed intent, or `null` if it isn't a
 * recognised Mage deep link. Tolerant by design: unknown routes return `null`
...
/**
 * Build a canonical `mage://…` URL for an intent. Used by anything that needs to hand
