...
const MOBILE_CONNECTIONS_STORAGE_KEY = 'mage.mobile.connections.v1';
const MOBILE_SECURE_STORAGE_PREFIX = 'mage.mobile.';
const MOBILE_DEVICE_ID_STORAGE_KEY = 'mage.mobile.deviceId';
...
// The transport a connect/reconnect settled on.
  ...
  const redeemPairingConnection = React.useCallback(async (payload: PairingConnectionPayload) => {
    ...
    try {
      ...
      const redeemBody = JSON.stringify({
        ...
        clientLabel: 'Mage Mobile',
        clientKind: 'mobile',
        deviceName: 'Mage Mobile',
  ...
  const submitPassword = React.useCallback(async (password: string) => {
    ...
    try {
      ...
      const loginInit = {
        ...
        body: JSON.stringify({ password, trustDevice: true, issueClientToken: true, clientLabel: 'Mage Mobile', clientKind: 'mobile', devicePlatform: mobileDevicePlatform(), dedupeKey: mobileClientDedupeKey() }),
      };
