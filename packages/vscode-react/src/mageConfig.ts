...
const ensureProjectAgentDir = (workingDirectory: string): string => {
  const projectAgentDir = path.join(workingDirectory, '.mage', 'agents');
  if (!fs.existsSync(projectAgentDir)) {
    fs.mkdirSync(projectAgentDir, { recursive: true });
  }
  const legacyProjectAgentDir = path.join(workingDirectory, '.mage', 'agent');
...
const getProjectAgentPath = (workingDirectory: string, agentName: string): string => {
  const pluralPath = path.join(workingDirectory, '.mage', 'agents', `${agentName}.md`);
  const legacyPath = path.join(workingDirectory, '.mage', 'agent', `${agentName}.md`);
...
const ensureProjectCommandDir = (workingDirectory: string): string => {
  const projectCommandDir = path.join(workingDirectory, '.mage', 'commands');
  if (!fs.existsSync(projectCommandDir)) {
    fs.mkdirSync(projectCommandDir, { recursive: true });
  }
  const legacyProjectCommandDir = path.join(workingDirectory, '.mage', 'command');
...
const getProjectCommandPath = (workingDirectory: string, commandName: string): string => {
  const pluralPath = path.join(workingDirectory, '.mage', 'commands', `${commandName}.md`);
  const legacyPath = path.join(workingDirectory, '.mage', 'command', `${commandName}.md`);
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};
...
const getProjectSnippetDirs = (workingDirectory?: string): Array<{ dir: string; source: SnippetScope }> => {
  ...
  return [
    { dir: path.join(workingDirectory, '.mage', 'snippets'), source: 'project' },
    { dir: path.join(workingDirectory, '.mage', 'snippet'), source: 'project' },
  ];
};
  ...
  for (const { dir, source } of [...getGlobalSnippetDirs(), ...getProjectSnippetDirs(workingDirectory)]) {
    ...
    for (const filename of fs.readdirSync(dir)) {
      ...
      try {
        ...
        console.warn(`[Mage][VSCode] Failed to load snippet ${path.join(dir, filename)}:`, error);
      }
    }
  }
  return registry;
...
const getWritableSnippetDir = (scope: SnippetScope, workingDirectory?: string): string => {
  if (scope === 'project') {
    ...
    const preferred = path.join(workingDirectory, '.mage', 'snippet');
    const alternate = path.join(workingDirectory, '.mage', 'snippets');
 ...
 * Priority: root > .mage/, json > jsonc
 */
const getProjectConfigCandidates = (workingDirectory?: string): string[] => {
  if (!workingDirectory) return [];
  return [
    path.join(workingDirectory, 'mage.json'),
    path.join(workingDirectory, 'mage.jsonc'),
    path.join(workingDirectory, '.mage', 'mage.json'),
    path.join(workingDirectory, '.mage', 'mage.jsonc'),
  ];
};
...
const getProjectConfigPath = (workingDirectory?: string): string | null => {
  ...
  // Default to root mage.json for new configs
...
const getConfigPaths = (workingDirectory?: string) => ({
  userPaths: [
    ...
    path.join(MAGE_CONFIG_DIR, 'mage.json'),
    path.join(MAGE_CONFIG_DIR, 'mage.jsonc'),
  ],
...
const writeConfig = (config: Record<string, unknown>, filePath: string = CONFIG_FILE) => {
  if (fs.existsSync(filePath)) {
    const backupFile = `${filePath}.mage.backup`;
    try {
...
const getActiveMageConfigDir = (): string => {
...
const getActiveUserConfigPaths = (): string[] => {
  const configDir = getActiveMageConfigDir();
  return [
    path.join(configDir, 'config.json'),
    path.join(configDir, 'mage.json'),
    path.join(configDir, 'mage.jsonc'),
  ];
};
...
const ensureProjectPluginConfigPath = (workingDirectory?: string | null): string => {
  ...
  return path.join(workingDirectory, '.mage', 'mage.json');
};
...
const getPluginDir = (scope: PluginScope, workingDirectory?: string | null): string => {
  if (scope === 'project') {
    if (!workingDirectory) throw codedError('Project plugin scope requires working directory', 'INVALID_SCOPE');
    return path.join(workingDirectory, '.mage', 'plugins');
  }
  return path.join(getActiveMageConfigDir(), 'plugins');
};
...
const lookupNpmPackage = async (name: string): Promise<NpmLookupResult> => {
  try {
    const response = await fetch(`https://registry.npmjs.org/${encodeURIComponent(name).replace(/^%40/, '@')}`, {
      headers: { Accept: 'application/json', 'User-Agent': 'mage-vscode/dev' },
...
const ensureProjectMcpConfigPath = (workingDirectory: string): string => {
  const projectConfigDir = path.join(workingDirectory, '.mage');
  if (!fs.existsSync(projectConfigDir)) {
    fs.mkdirSync(projectConfigDir, { recursive: true });
  }
  return path.join(projectConfigDir, 'mage.json');
};
...
const parseMdFile = (filePath: string): { frontmatter: Record<string, unknown>; body: string } => {
  ...
  try {
    ...
    console.warn(`[Mage][VSCode] Failed to parse frontmatter for ${filePath}, treating as empty:`, error);
    frontmatter = {};
  }
...
const writeMdFile = (filePath: string, frontmatter: Record<string, unknown>, body: string) => {
  // Filter out null/undefined values - Mage expects keys to be omitted rather than set to null
...
export const createAgent = (agentName: string, config: Record<string, unknown>, workingDirectory?: string, scope?: AgentScope) => {
  ...
  const layers = readConfigLayers(workingDirectory);
  const jsonSource = getJsonEntrySource(layers, 'agent', agentName);
  if (jsonSource.exists) throw new Error(`Agent ${agentName} already exists in mage.json`);
...
export const updateAgent = (agentName: string, updates: Record<string, unknown>, workingDirectory?: string) => {
  ...
  // Check if agent exists in mage.json across all config layers
  const layers = readConfigLayers(workingDirectory);
  const jsonSource = getJsonEntrySource(layers, 'agent', agentName);
...
export const createCommand = (commandName: string, config: Record<string, unknown>, workingDirectory?: string, scope?: CommandScope) => {
  ...
  const layers = readConfigLayers(workingDirectory);
  const jsonSource = getJsonEntrySource(layers, 'command', commandName);
  if (jsonSource.exists) throw new Error(`Command ${commandName} already exists in mage.json`);
...
export type SkillSource = 'mage' | 'claude' | 'agents';
...
const getProjectSkillDir = (workingDirectory: string, skillName: string): string => {
  const pluralPath = path.join(workingDirectory, '.mage', 'skills', skillName);
  const legacyPath = path.join(workingDirectory, '.mage', 'skill', skillName);
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};

const getProjectSkillPath = (workingDirectory: string, skillName: string): string => {
  const pluralPath = path.join(workingDirectory, '.mage', 'skills', skillName, 'SKILL.md');
  const legacyPath = path.join(workingDirectory, '.mage', 'skill', skillName, 'SKILL.md');
  if (fs.existsSync(legacyPath) && !fs.existsSync(pluralPath)) return legacyPath;
  return pluralPath;
};
...
const getSkillScope = (skillName: string, workingDirectory?: string): {
  ...
  if (workingDirectory) {
    // Check .mage/skill first
    const projectPath = getProjectSkillPath(workingDirectory, skillName);
    if (fs.existsSync(projectPath)) {
      return { scope: SKILL_SCOPE.PROJECT, path: projectPath, source: 'mage' };
    }
  ...
  if (fs.existsSync(userPath)) {
    return { scope: SKILL_SCOPE.USER, path: userPath, source: 'mage' };
  }
...
export const discoverSkills = (workingDirectory?: string): DiscoveredSkill[] => {
  ...
  for (const dir of configDirectories) {
    for (const subDir of ['skill', 'skills']) {
      ...
      for (const skillMdPath of walkSkillMdFiles(root)) {
        ...
        addSkillFromMdFile(skills, skillMdPath, scope, 'mage');
      }
    }
  }
  ...
  for (const skillPath of configuredPaths) {
    ...
    for (const skillMdPath of walkSkillMdFiles(resolved)) {
      addSkillFromMdFile(skills, skillMdPath, SKILL_SCOPE.PROJECT, 'mage');
    }
  }
  ...
  for (const cacheRoot of cacheCandidates) {
    ...
    for (const entry of entries) {
      ...
      for (const skillMdPath of walkSkillMdFiles(skillRoot)) {
        addSkillFromMdFile(skills, skillMdPath, SKILL_SCOPE.USER, 'mage');
      }
    }
  }
...
export const getSkillSources = (
  ...
  if (isBuiltInDiscovered) {
    ...
    mdSource = matchedDiscovered?.source || 'mage';
...
export const createSkill = (skillName: string, config: Record<string, unknown>, workingDirectory?: string, scope?: SkillScope): void => {
  ...
  const requestedSource: SkillSource = config.source === 'agents' ? 'agents' : 'mage';
