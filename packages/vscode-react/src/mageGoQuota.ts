type MageGoCredential = { workspaceId: string; authCookie: string };
...
export const fetchMageGoUsage = async (credential: MageGoCredential) => {
  const response = await fetch(`https://mage.ai/workspace/${encodeURIComponent(credential.workspaceId)}/go`, { headers: { Accept: 'text/html', Cookie: `auth=${credential.authCookie}` }, redirect: 'manual', signal: AbortSignal.timeout(15_000) });
  if (response.status === 401 || response.status === 403 || (response.status >= 300 && response.status < 400)) throw new Error('Mage Go authentication failed');
  ...
  if (!Object.keys(windows).length) throw new Error('Mage Go usage data could not be parsed');
  return windows;
};
