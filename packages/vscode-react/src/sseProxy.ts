...
const normalizeSsePath = (path: string): { pathname: '/event' | '/global/event'; searchParams: URLSearchParams; directory: string | null } => {
  const parsed = new URL(path, 'https://mage.invalid');
...
const resolveDefaultDirectory = (manager: MageManager): string => {
...
const createSseHeaders = (manager: MageManager, headers?: Record<string, string>): Record<string, string> => ({
  Accept: 'text/event-stream',
  'Cache-Control': 'no-cache',
  Connection: 'keep-alive',
  ...(headers || {}),
  ...manager.getMageAuthHeaders(),
});
...
const fetchSseResponse = async (
  ...
  if (!response.ok) {
    ...
    const error = new Error(`Mage SSE request failed (${response.status})`);
    (error as Error & { status?: number }).status = response.status;
    throw error;
  }

  if (!response.body) {
    throw new Error('Mage SSE response missing body');
  }
