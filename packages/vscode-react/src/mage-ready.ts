import type { ConnectionStatus, MageManager } from './mage';

const API_URL_WAIT_TIMEOUT_MS = 30000;

export async function waitForApiUrl(
  manager: MageManager | undefined,
  ...
  // Only hand out an API URL once Mage has actually passed its readiness
  ...
  // forward requests to a not-yet-ready Mage (and to a stale port during a
  ...
  // isMageReady hold and closes that pre-ready forwarding window.
