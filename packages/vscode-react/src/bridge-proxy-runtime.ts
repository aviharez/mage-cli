...
import { waitForApiUrl } from './mage-ready';
...
const isSseProxyPath = (requestPath: string): boolean => {
  try {
    const parsed = new URL(requestPath, 'https://mage.invalid');
...
// saturates the single Mage process and delays everything queued behind it
// (e.g. createSession). Coalesce genuinely-concurrent identical GETs to those
// read endpoints so Mage does the work once; every caller gets its own
  ...
  try {
    ...
    return {
      ...
      bodyText: JSON.stringify({
        error: error instanceof Error ? error.message : 'Failed to reach Mage API',
      }),
    };
  }
};
...
export async function handleProxyBridgeMessage(
  ...
  switch (type) {
    ...
    case 'api:proxy': {
      ...
      // single Mage process serves them once. The shared fetch carries no
    ...
    case 'api:session:message': {
      ...
      try {
        ...
        const body = JSON.stringify({
          error: isTimeout ? 'Mage message forward timed out' : error instanceof Error ? error.message : 'Mage message forward failed',
        });
        const data: ApiProxyResponsePayload = {
