...
import { registerManagedProcess, unregisterManagedProcess, reapOrphanedProcesses } from './mageProcessRegistry';
...
export interface MageManager {
  ...
  getMageAuthHeaders(): Record<string, string>;
  getWorkingDirectory(): string;
  isCliAvailable(): boolean;
  getDebugInfo(): MageDebugInfo;
  onStatusChange(callback: (status: ConnectionStatus, error?: string) => void): vscode.Disposable;
}

function generateSecureMagePassword(): string {
...
function buildMageAuthHeader(password: string): string {
  const username = process.env.MAGE_SERVER_USERNAME?.trim() || 'mage';
  return `Basic ${Buffer.from(`${username}:${password}`, 'utf8').toString('base64')}`;
}

function isValidMagePassword(password: string): boolean {
...
let cachedDetectedMageCliPath: string | undefined;

function normalizeConfiguredMageBinary(raw: unknown): string | null {
  ...
  try {
    ...
    if (stat.isDirectory()) {
      return path.join(trimmed, process.platform === 'win32' ? 'mage.exe' : 'mage');
    }
  } catch {
...
function isMacMageAppBundlePath(candidate: string): boolean {
  return process.platform === 'darwin' && /\/Mage(?: Dev| Beta)?\.app\/Contents\/MacOS\/(?:Mage(?: Dev| Beta)?|mage-cli)$/i.test(candidate);
}

function isWindowsMageDesktopAppPath(candidate: string): boolean {
  ...
  return normalized.startsWith(`${localAppData}${path.sep}`)
    && normalized.endsWith(`${path.sep}programs${path.sep}mage${path.sep}mage.exe`);
}

function isKnownMageDesktopAppPath(candidate: string): boolean {
  return isMacMageAppBundlePath(candidate) || isWindowsMageDesktopAppPath(candidate);
}

function createConfiguredMageBinaryError(raw: string, normalized: string): Error {
  const messageSuffix = 'Mage needs the standalone mage CLI. Install it and set mage.mageBinary to the CLI path, for example ~/.mage/bin/mage, or leave the setting empty to use PATH lookup.';
  if (isKnownMageDesktopAppPath(raw) || isKnownMageDesktopAppPath(normalized)) {
    const platformName = process.platform === 'win32' ? 'Windows desktop app install' : 'macOS desktop app bundle';
    return new Error(`Configured Mage binary points at the ${platformName}, not the CLI: ${normalized}. ${messageSuffix}`);
  }

  try {
    const rawStat = fs.statSync(raw);
    if (rawStat.isDirectory()) {
      return new Error(`Configured Mage binary directory does not contain an executable ${process.platform === 'win32' ? 'mage.exe' : 'mage'}: ${raw}. ${messageSuffix}`);
    }
  } catch {
  ...
  try {
    ...
    if (!stat.isFile()) {
      return new Error(`Configured Mage binary is not a file: ${normalized}. ${messageSuffix}`);
    }
    return new Error(`Configured Mage binary is not executable: ${normalized}. ${messageSuffix}`);
  } catch {
    return new Error(`Configured Mage binary not found: ${normalized}. ${messageSuffix}`);
  }
}

function validateConfiguredMageBinaryForManagedStart(): string | null {
  const candidates: string[] = [];
  try {
    const config = vscode.workspace.getConfiguration('mage');
    const raw = config.get<string>('mageBinary') || '';
    if (raw.trim()) {
  ...
  try {
    const settings = readMageSettings();
  ...
  const normalized = normalizeConfiguredMageBinary(raw);
  if (!normalized) {
    return null;
  }

  if (isExecutable(normalized) && !isKnownMageDesktopAppPath(normalized)) {
    return normalized;
  }
...
function resolveMageCliPath(): string | null {
  const configured = (() => {
    try {
      const config = vscode.workspace.getConfiguration('mage');
      return normalizeConfiguredMageBinary(config.get<string>('mageBinary') || '');
    } catch {
      return null;
    }
  })();

  if (configured && isExecutable(configured) && !isKnownMageDesktopAppPath(configured)) {
    return configured;
  }

  const sharedFromMage = (() => {
    try {
      const settings = readMageSettings();
      const candidate = settings.mageBinary;
      if (typeof candidate !== 'string') {
        return null;
      }
      return normalizeConfiguredMageBinary(candidate);
    } catch {
      return null;
    }
  })();

  if (sharedFromMage && isExecutable(sharedFromMage) && !isKnownMageDesktopAppPath(sharedFromMage)) {
    return sharedFromMage;
  }
  ...
  for (const candidate of explicit) {
    if (isExecutable(candidate) && !isKnownMageDesktopAppPath(candidate)) {
      return candidate;
    }
  }

  if (cachedDetectedMageCliPath) {
    if (isExecutable(cachedDetectedMageCliPath) && !isKnownMageDesktopAppPath(cachedDetectedMageCliPath)) {
      return cachedDetectedMageCliPath;
    }
    cachedDetectedMageCliPath = undefined;
  }
  ...
  const winFallbacks = (() => {
    ...
    return [
      ...
      path.join(npmDir, 'node_modules', 'mage-ai', 'bin', 'mage.exe'),
      path.join(npmDir, 'mage.exe'),
      path.join(npmDir, 'mage.cmd'),
      path.join(npmDir, 'mage.bat'),
      // System-wide Node installer keeps the global npm prefix here
      // (npm i -g mage-ai → mage.cmd shim).
      path.join(process.env.ProgramFiles || 'C:\\Program Files', 'nodejs', 'mage.cmd'),
  ...
  if (process.platform !== 'win32') {
    const fromPath = findExecutableInPath('mage');
    if (fromPath && !isKnownMageDesktopAppPath(fromPath)) {
      cachedDetectedMageCliPath = fromPath;
      return fromPath;
    }
  }
  ...
  for (const candidate of fallbacks) {
    if (isExecutable(candidate) && !isKnownMageDesktopAppPath(candidate)) {
      cachedDetectedMageCliPath = candidate;
      return candidate;
    }
  }
  ...
  if (process.platform === 'win32') {
    ...
    try {
      const result = spawnSync('where', ['mage'], {
        encoding: 'utf8',
      ...
      if (result.status === 0) {
        const lines = (result.stdout || '')
          ...
          .map((line) => line.trim())
          .filter(Boolean);
        const found = lines.find((line) => isExecutable(line) && !isKnownMageDesktopAppPath(line));
        if (found) {
          cachedDetectedMageCliPath = found;
          return found;
        }
      }
    } catch {
...
async function waitForReady(
  ...
  const outputChannel = vscode.window.createOutputChannel('MageManager');
  ...
  while (Date.now() - start < timeoutMs) {
    for (const baseUrl of candidates) {
      ...
      try {
        ...
        // Mage readiness check. Use /global/health for Mage 1.15.x compatibility.
...
async function spawnManagedMageServer(
  workingDirectory: string,
  port: number,
  timeoutMs: number
): Promise<{ url: string; close: () => void }> {
  const binary = stripWrappingQuotes(process.env.MAGE_BINARY || 'mage') || 'mage';
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    const onExit = (code: number | null) => {
      cleanup();
      const appBundleHint = isMacMageAppBundlePath(binary)
        ? ' The configured binary appears to point at the macOS desktop app bundle; Mage needs the standalone mage CLI.'
        : '';
      reject(new Error(`Mage process exited before serving with code ${code}. Binary used: ${binary}.${appBundleHint} Output: ${output}`));
    };
    ...
    const timer = setTimeout(() => {
      cleanup();
      // Surface whatever Mage printed while we waited — otherwise a hung or
...
async function allocateManagedMagePort(): Promise<number> {
...
export function createMageManager(context: vscode.ExtensionContext): MageManager {
  ...
  const userProvidedEnvPassword = (() => {
    ...
    return isValidMagePassword(normalized) ? normalized : null;
  })();
  ...
  const getMageAuthHeaders = (): Record<string, string> => {
    const password = (managedPassword || userProvidedEnvPassword || process.env.MAGE_SERVER_PASSWORD || '').trim();
    if (!password) {
      return {};
    }
    return { Authorization: buildMageAuthHeader(password) };
  };
  ...
  const ensureManagedMageServerPassword = async ({ rotateManaged = false }: { rotateManaged?: boolean } = {}): Promise<string> => {
    if (userProvidedEnvPassword) {
      return setManagedPasswordState(userProvidedEnvPassword, 'user-env');
    }

    if (rotateManaged) {
      return setManagedPasswordState(generateSecureMagePassword(), 'rotated');
    }

    if (managedPassword && isValidMagePassword(managedPassword)) {
    ...
    return setManagedPasswordState(generateSecureMagePassword(), 'generated');
  };
  ...
  async function startInternal(
    ...
    // Before spawning our own server, reap any Mage process WE spawned in a
    ...
    if (!reapedOrphansOnce) {
      ...
      try {
        ...
        if (reaped > 0) console.log(`[mage] startup reaped ${reaped} orphaned process(es)`);
      } catch (error) {
        console.warn('[mage] orphan reap failed:', error instanceof Error ? error.message : error);
      }
    }
    ...
    try {
      ...
      const configuredCli = validateConfiguredMageBinaryForManagedStart();
      if (configuredCli) {
      ...
      const resolvedCli = configuredCli || resolveMageCliPath();
      if (resolvedCli) {
        cliPath = resolvedCli;
      ...
      const password = await ensureManagedMageServerPassword({
      ...
      try {
        ...
        const port = await allocateManagedMagePort();
        server = await spawnManagedMageServer(serverCwd, port, READY_CHECK_TIMEOUT_MS);
      } finally {
        try {
      ...
      if (server && server.url) {
        ...
        const ready = await waitForReady(server.url, READY_CHECK_TIMEOUT_MS, getMageAuthHeaders());
      ...
      if (message.includes('ENOENT') || message.includes('spawn mage')) {
        cliMissing = true;
        if (!cliPath) {
          cliPath = resolveMageCliPath();
        }
        const moreInfoLabel = t('More Info');
        setStatus('error', t('Mage CLI not found. Install it and ensure it\'s in PATH.'));
        vscode.window.showErrorMessage(
          t('Mage CLI not found. Please install it and ensure it\'s in PATH.'),
          moreInfoLabel
        ).then(selection => {
        ...
        setStatus('error', t('Failed to start Mage: {0}', message));
      }
    }
  }
  ...
  return {
    ...
    isCliAvailable: () => !cliMissing || Boolean(cliPath || resolveMageCliPath()),
    getDebugInfo: () => {
      const secureConnection = Boolean(getMageAuthHeaders().Authorization);
      const detectedCliPath = cliPath || resolveMageCliPath();
      return {
