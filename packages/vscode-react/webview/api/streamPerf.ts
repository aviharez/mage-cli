...
declare global {
  interface Window {
    __mageVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__mageVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__mageVsCodeStreamPerfState__ = {
  ...
  return window.__mageVsCodeStreamPerfState__;
};
