...
[Mage](https://mage.ai) AI coding agent, right inside your editor. No tab-switching, no context loss.

![VS Code Extension](https://github.com/btriapitsyn/mage/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/btriapitsyn/mage) with even more features.**
...
Plus everything from the shared Mage UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `Mage: Focus Chat` | Focus the chat panel |
| `Mage: New Session` | Start a new chat session |
| `Mage: Open Sidebar` | Open the Mage sidebar |
| `Mage: Open Agent Manager` | Launch parallel multi-model runs |
| `Mage: Open Session in Editor` | Open current or new session in an editor tab |
| `Mage: Settings` | Open extension settings |
| `Mage: Restart API Connection` | Restart the Mage API process |
| `Mage: Show Mage Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **Mage** submenu:
...
|---------|---------|-------------|
| `mage.apiUrl` | _(empty)_ | URL of an external Mage API server. Leave empty to auto-start a local instance. |
| `mage.mageBinary` | _(empty)_ | Absolute path to the `mage` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |

## Requirements

- [Mage CLI](https://mage.ai) installed and available in PATH (or set `MAGE_BINARY` env var)
- VS Code 1.85+
...
- `MAGE_VSCODE_BIN=cursor bun run vscode:dev`
- `MAGE_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/mage-*.vsix`
