...
async function startupCommand(options, action = 'status') {
  ...
  if (!['status', 'enable', 'disable'].includes(normalized)) {
    throw new TunnelCliError(
      `Unknown startup subcommand '${action}'. Use 'mage startup --help'.`,
  ...
  if (normalized === 'enable' && result.activeState === 'failed') {
    throw new TunnelCliError(
      'Startup service was installed but failed to start. Run `journalctl --user -u mage.service -n 80 --no-pager` for details.',
  ...
  if (normalized === 'enable') {
    logStatus('info', 'service command', 'mage serve --foreground');
  }
