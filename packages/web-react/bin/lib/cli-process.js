...
async function getPidFilePath(port) {
  return path.join(getRunDir(), `mage-${port}.pid`);
}

async function getInstanceFilePath(port) {
  return path.join(getRunDir(), `mage-${port}.json`);
}
...
// that PID to an unrelated process — see isMageProcessRunning.
...
function isMageCmdline(cmdline) {
  if (typeof cmdline !== 'string' || cmdline.length === 0) {
    return false;
  }
  // Every install path contains the "mage" segment — the npm package
  // (@mage/web) and the source checkout both do, for the foreground
  ...
  return cmdline.toLowerCase().includes('mage');
}

// Liveness + identity — "is the Mage instance recorded in a pid file
...
function isMageProcessRunning(pid) {
  const state = getMageProcessState(pid);
  return state === 'matched' || state === 'unknown';
}

function getMageProcessState(pid, options = {}) {
  ...
  return isMageCmdline(cmdline) ? 'matched' : 'mismatched';
}

function hasMageRuntimeInfo(info) {
