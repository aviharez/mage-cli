...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running Mage instance found. Start one with `mage serve`.');
    }
    return running;
  }
  ...
  if (options.explicitPort) {
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by Mage Desktop app. Tunnel attach requires a CLI instance from \`mage serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable Mage tunnel instance. Ensure it is healthy and running Mage CLI runtime.`
          );
        }
      }
      return found;
    ...
    throw new Error(`No running Mage instance found on port ${options.port}.`);
  }
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable Mage instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only Mage Desktop instance(s) detected. Tunnel attach requires a CLI instance from `mage serve`.');
    }

    throw new Error('No attachable Mage instance found. Start one with `mage serve`.');
  }
  ...
  throw new Error(`Multiple Mage instances found: ${ports}. Use --port <port> or --all.`);
}
...
async function handleTunnelProfileSubcommand(options, action, { boldText, ensureTunnelProfilesMigrated }) {
  ...
  if (!sub) {
    ...
    if (!isQuietMode(options)) {
      ...
      clackLog.step('List profiles: `mage tunnel profile list`');
      clackLog.step('Show one profile: `mage tunnel profile show --name <name>`');
      clackLog.step('Add profile: `mage tunnel profile add --provider cloudflare --mode managed-remote --name <name> --hostname <host> --token <token>`');
      clackLog.step('Remove profile: `mage tunnel profile remove --name <name>`');
  ...
  if (sub === 'add') {
    ...
    logStatus('info', '[START_PROFILE]', `mage tunnel start --profile ${added.name}`);
    clackOutro('');
    return;
  }
  ...
  throw new TunnelCliError(
    `Unknown tunnel profile subcommand '${sub}'.${hint} Use 'mage tunnel help'.`,
    ...
    switch (subcommand) {
      ...
      case 'ready': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, `/api/mage/tunnel/check?provider=${encodeURIComponent(provider)}`);
            if (!response.ok) {
      ...
      case 'status': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, '/api/mage/tunnel/status');
            if (!response.ok) {
      ...
      case 'doctor': {
          ...
          for (const diagnosticsEntry of diagnosticsEntries) {
            try {
              ...
              const { response, body } = await requestJson(
                ...
                `/api/mage/tunnel/doctor?${query.toString()}`,
                doctorFetchOptions,
              );
        ...
        if (desktopUnavailablePorts.length > 0) {
          clackLog.message('Only CLI instances (mage serve) support tunneling.');
        }

        if (cliPorts.length === 0 && unavailablePorts.length === 0) {
          logStatus('warning', 'No running instances found', 'Start one with `mage serve`.');
          clackOutro('No ports available');
          return;
        }
        if (cliPorts.length === 0) {
          logStatus('warning', 'No CLI instances available for tunneling', 'Start one with `mage serve`.');
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active Mage CLI port.',
                  'Example: `http://127.0.0.1:<port>`',
                  'If CLI picked a different port, update Cloudflare or run `mage serve --port <port>`.',
                ],
              });
            }
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered Mage instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable Mage CLI instances found on safe ports. Start one with `mage serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select Mage instance port',
        ...
        if (instance?.autoStarted && shouldRenderHumanOutput(options)) {
          logStatus(
            ...
            `logs: mage logs -p ${instance.port}`,
          );
        }
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for Mage on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for Mage health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('Mage is still starting');
            throw new Error(
              `Mage on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
              `Wait another minute, then check health with \`curl -fsS ${buildLocalUrl(instance.port, '/health')}\`. ` +
              `If health is OK, retry tunnel start with \`mage tunnel start --port ${instance.port}\`. ` +
              `For diagnostics run \`mage logs -p ${instance.port}\`.`
            );
          }
        ...
        if (selectedProfile && mode === 'managed-remote') {
          ...
          const { response: presetResponse, body: presetBody } = await requestJson(instance.port, '/api/mage/tunnel/managed-remote-token', {
            method: 'PUT',
        ...
        let body;
        try {
          ({ response, body } = await requestJson(instance.port, '/api/mage/tunnel/start', {
            method: 'POST',
            body: JSON.stringify(payload),
            timeoutMs: 60000,
          }));
        } catch (error) {
          if (error instanceof Error && /\/api\/mage\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
            spin?.error('Tunnel start timed out');
            throw new Error(
              `Tunnel start timed out after 60s. cloudflared may still be starting; check with \`mage tunnel status --port ${instance.port}\`. Run \`mage logs -p ${instance.port}\` for details.`
            );
          }
          spin?.error('Tunnel start failed');
          const message = error instanceof Error ? error.message : String(error);
          throw new Error(`${message} Run \`mage logs -p ${instance.port}\` for details.`);
        }
        ...
        if (!response.ok || !body?.ok) {
          ...
          const userError = isCloudflareTimeout
            ...
            : baseError;
          throw new Error(`${userError} Run \`mage logs -p ${instance.port}\` for details.`);
        }
        ...
        if (isJsonMode(options)) {
          ...
          const optionalTips = [
            { line: 'Check status', detail: 'mage tunnel status' },
            { line: 'Stop tunnel', detail: 'mage tunnel stop' },
          ...
          if (!selectedProfile && mode === 'managed-remote' && typeof hostname === 'string' && hostname.trim().length > 0) {
            ...
            optionalTips.push({ line: 'Start from saved profile', detail: 'mage tunnel start --profile <name>' });
          }
      ...
      case 'stop': {
        ...
        for (const entry of entries) {
          ...
          try {
            const { response, body } = await requestJson(entry.port, '/api/mage/tunnel/stop', {
              method: 'POST',
            });
            if (!response.ok) {
      ...
      default: {
        ...
        const hint = suggestion ? ` Did you mean '${suggestion}'?` : '';
        throw new TunnelCliError(
          `Unknown tunnel subcommand '${subcommand}'.${hint} Use 'mage tunnel help'.`,
          EXIT_CODE.USAGE_ERROR
        );
      }
    }
