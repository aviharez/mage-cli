...
const STARTUP_SERVICE_ID = 'dev.mage.web';
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'mage.service'),
    };
  }
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'Mage');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (options.envSnapshot !== false) {
    const mageBinary = process.env.MAGE_BINARY || searchPathFor('mage');
    if (typeof mageBinary === 'string' && mageBinary.trim().length > 0) {
      env.MAGE_BINARY = mageBinary.trim();
    }
  }
  const uiPassword = hasUiPasswordConfigured(options.uiPassword) ? options.uiPassword : undefined;
  if (uiPassword) {
    env.MAGE_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.MAGE_API_ONLY = 'true';
  }
  if (typeof process.env.MAGE_DATA_DIR === 'string' && process.env.MAGE_DATA_DIR.trim().length > 0) {
    env.MAGE_DATA_DIR = path.resolve(process.env.MAGE_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'Mage');
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'mage.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'mage.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'Mage'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'mage.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'mage.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
