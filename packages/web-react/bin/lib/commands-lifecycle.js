...
async function stopCommand(options) {
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (showOutput) {
          logStatus('info', `no Mage instance found on port ${options.port}`);
      ...
      if (explicitInstance.runtime === 'desktop') {
        ...
        if (isJsonMode(options)) {
          printJson({ stoppedCount: 0, results: jsonResults, messages: [{ level: 'warning', code: 'DESKTOP_MANAGED_PORT', message: `Port ${options.port} is managed by Mage Desktop and cannot be stopped with this command.` }] });
        }
        if (showOutput) {
          logStatus('warning', `port ${options.port} is managed by Mage Desktop`, 'cannot be stopped with this command');
      ...
      if (explicitInstance.source === 'probe') {
        ...
        if (showOutput && !unmanagedStopSpin) {
          logStatus('info', `found unmanaged Mage instance on port ${options.port}`, 'attempting shutdown');
        }
        unmanagedStopSpin?.start(`Stopping unmanaged Mage on port ${options.port}...`);
        ...
        if (stopped) {
          unmanagedStopSpin?.stop(`Stopped unmanaged Mage on port ${options.port}`);
          jsonResults.push({ port: options.port, runtime: 'unmanaged', stopped: true });
          if (isJsonMode(options)) {
            printJson({ stoppedCount: 1, results: jsonResults });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('success', `stopped Mage on port ${options.port}`);
          ...
          unmanagedStopSpin?.error(`Could not stop Mage on port ${options.port}`);
          ...
          if (isJsonMode(options)) {
            printJson({
              ...
              stoppedCount: 0,
              results: jsonResults,
              messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop Mage on port ${options.port}.` }],
            });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('error', `could not stop Mage on port ${options.port}`);
            finish('failed');
          }
          printQuietStopResults();
        }
      ...
      if (explicitInstance.source === 'registry-unconfirmed') {
        ...
        if (showOutput && !unconfirmedStopSpin) {
          logStatus('info', `found unconfirmed Mage pid ${explicitInstance.pid} on port ${options.port}`, 'HTTP shutdown endpoint is unreachable; stopping by PID');
        }
        unconfirmedStopSpin?.start(`Stopping unconfirmed Mage on port ${options.port}...`);
        ...
        if (stopped || !isProcessRunning(explicitInstance.pid)) {
          ...
          unconfirmedStopSpin?.stop(`Stopped Mage PID ${explicitInstance.pid}`);
        ...
        unconfirmedStopSpin?.error(`Could not stop Mage PID ${explicitInstance.pid}`);
        ...
        if (isJsonMode(options)) {
          printJson({
            ...
            stoppedCount: 0,
            results: jsonResults,
            messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop Mage PID ${explicitInstance.pid}.` }],
          });
        }
        if (showOutput && !unconfirmedStopSpin) {
      ...
      if (showOutput) {
        logStatus('info', 'No running Mage instances found');
        finish('nothing to stop');
      }
      printQuietStopResults();
      return;
    ...
    for (const instance of runningInstances) {
      ...
      stopSpin?.start(`Stopping Mage on port ${instance.port}...`);
      try {
...
async function restartCommand(options, serveCommand) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running Mage instances to restart');
    ...
    for (const instance of runningInstances) {
      if (instance.runtime === 'desktop') {
        const message = `Port ${instance.port} is managed by Mage Desktop and cannot be restarted with this command.`;
        if (isJsonMode(options)) {
          printJson({
            status: 'warning',
            restartedCount: 0,
        ...
        if (showOutput) {
          logStatus('warning', `port ${instance.port} is managed by Mage Desktop`, 'cannot be restarted with this command');
      ...
      restartSpin?.start(`Restarting Mage on port ${instance.port}...`);
      try {
        await runStop({
          explicitPort: true,
          port: instance.port,
        ...
        restartSpin?.stop(`Restarted Mage on port ${restartedPort}`);
        if (showOutput && !restartSpin) {
          logStatus('success', `port ${restartedPort} restarted`, `mode: ${launchMode}`);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        restartSpin?.error(`Failed to restart Mage on port ${instance.port}`);
        if (showOutput && !restartSpin) {
