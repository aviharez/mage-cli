...
// MAGE_RELAY_URL env override, then the stored setting, then the default —
// so the pairing link points at the same relay the host connects out to.
function resolveRelayUrl(settings) {
  const envUrl = process.env.MAGE_RELAY_URL;
...
function createSettingsAccessors() {
  const settingsPath = path.join(getMageDataDir(), SETTINGS_FILE_NAME);
...
// Mirror of encodePairingConnectionPayload in @mage/ui (the bin cannot
...
function encodePairingConnectUrl(payload) {
  ...
  return `mage://connect?v=2&p=${encoded}`;
}
...
async function resolveConnectUrlServerUrl(options) {
  ...
  if (typeof hostOverride !== 'string' && !process.env.MAGE_HOST) {
...
function createConnectUrlCommand({ serveCommand }) {
  return async function connectUrlCommand(options = {}) {
    assertSafeBrowserPort(options.port, { context: 'Mage connect-url' });
    ...
    clackIntro('Mage pairing link');
    if (serverState.autoStarted) {
      logStatus('success', `started Mage on port ${options.port}`);
    }
    ...
    if (resolvedServerUrl.source === 'lan-detected') {
      clackLog.info('Detected a LAN address because Mage is bound to all interfaces. Use --server to override it.');
    } else if (resolvedServerUrl.source === 'loopback-fallback') {
      clackLog.warn('Mage is bound to all interfaces, but no LAN address was detected. Use --server to provide a reachable URL.');
      ...
      if (options.relay) {
        logStatus('warn', '[LAN_UNREACHABLE]', 'Mage only listens on this machine, so devices will always connect through the relay. Restart with --lan to allow direct home-network connections.');
      } else {
        logStatus('warn', '[LAN_UNREACHABLE]', 'Mage only listens on this machine, so other devices cannot use this link. Restart with --lan, or use --server to provide a reachable URL.');
      }
    }
    clackLog.info('Scan or paste this link into another Mage client. It is single-use and expires.');
    if (options.qr === true) {
