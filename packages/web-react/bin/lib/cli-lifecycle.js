...
function createLivePortInstance(port, info, host) {
  if (!hasMageRuntimeInfo(info)) return null;
  return {
    port,
    pid: Number.isFinite(info.pid) ? info.pid : null,
    pidFilePath: path.join(getRunDir(), `mage-${port}.pid`),
    instanceFilePath: path.join(getRunDir(), `mage-${port}.json`),
    mtime: 0,
    startedAt: 0,
    launchMode: 'daemon',
...
async function resolveDoctorPortStatuses(options = {}) {
  ...
  if (options.explicitPort) {
    ...
    if (desktopEntry && desktopEntry.port === requestedPort) {
      statuses.push({
        ...
        detail: 'Use a CLI instance port from `mage serve` for tunneling.',
      });
    ...
    statuses.push({
      ...
      detail: `Start one with \`mage serve --port ${requestedPort}\`.`,
    });
    return { statuses, availableEntries: [] };
  }
  ...
  if (runningEntries.length === 0) {
    statuses.push({
      ...
      detail: 'Start one with `mage serve`.',
    });
  }
...
async function discoverRunningInstances(options = {}) {
  ...
  const getProcessState = typeof options.getMageProcessState === 'function'
    ? options.getMageProcessState
    : (pid) => getMageProcessState(pid, options);
  try {
    const files = fs.readdirSync(runDir);
    const pidFiles = files.filter((file) => file.startsWith('mage-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('mage-', '').replace('.pid', ''), 10);
      if (!Number.isFinite(port) || port <= 0) continue;
      const pidFilePath = path.join(runDir, file);
      const pid = readPidFile(pidFilePath);
      if (!pid) {
        removePidFile(pidFilePath);
        removeInstanceFile(path.join(runDir, `mage-${port}.json`));
        continue;
      }

      const instanceFilePath = path.join(runDir, `mage-${port}.json`);
      ...
      // confirms Mage. Cmdline identity alone can match a recycled PID
      // from another Mage process on a different port. Try all plausible
      ...
      if (!hasMageRuntimeInfo(liveInfo)) {
...
async function discoverMageInstanceOnPort(port, options = {}) {
...
async function discoverLifecycleInstances(options = {}, deps = {}) {
  ...
  const liveInstance = await discoverMageInstanceOnPort(options.port, {
    ...deps,
    host: options.host,
    runningInstances,
  });
...
async function discoverUnconfirmedRegistryInstanceOnPort(port, options = {}) {
  ...
  const storedOptions = readInstanceOptions(instanceFilePath);
  const processState = getMageProcessState(pid);
  if (processState === 'dead') {
    removePidFile(pidFilePath);
    removeInstanceFile(instanceFilePath);
    return null;
