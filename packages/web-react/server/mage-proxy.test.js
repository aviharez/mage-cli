...
import { createSseBoundaryTracker, registerMageProxy, writeSseChunkWithBackpressure } from './lib/mage/proxy.js';
...
describe('Mage proxy SSE forwarding', () => {
  let upstreamServer;
  let proxyServer;
  ...
  it('forwards event streams with nginx-safe headers', async () => {
    ...
    registerMageProxy(app, {
      ...
      getRuntime: () => ({
        magePort: upstreamPort,
        isMageReady: true,
        mageNotReadySince: 0,
        isRestartingMage: false,
      }),
      getMageAuthHeaders: () => ({ Authorization: 'Bearer test-token' }),
      buildMageUrl: (requestPath) => `http://127.0.0.1:${upstreamPort}${requestPath}`,
      ensureMageApiPrefix: () => {},
    });
  ...
  it('holds a request through Mage warmup and succeeds once ready (no 503/backoff)', async () => {
    const upstream = express();
    ...
    // Mage becomes ready shortly after the request arrives.
    setTimeout(() => { runtime.isMageReady = true; }, 200);
  ...
  it('returns 503 fast when Mage never becomes ready', async () => {
    const app = express();
    registerMageProxy(app, {
      fs: {},
      os: {},
      ...
      getMageAuthHeaders: () => ({}),
      buildMageUrl: (requestPath) => `http://127.0.0.1:1${requestPath}`,
      ensureMageApiPrefix: () => {},
    });
    proxyServer = await listen(app);
    const proxyPort = proxyServer.address().port;
  ...
  it('routes generic API requests through external Mage base URL', async () => {
    const upstream = express();
    upstream.get('/config/providers', (_req, res) => {
    ...
    registerMageProxy(app, {
      ...
      getRuntime: () => ({
        magePort: 3902,
        mageBaseUrl: externalBaseUrl,
        isMageReady: true,
        mageNotReadySince: 0,
        isRestartingMage: false,
      }),
      getMageAuthHeaders: () => ({}),
      buildMageUrl: (requestPath) => `${externalBaseUrl}${requestPath}`,
      ensureMageApiPrefix: () => {},
    });
    proxyServer = await listen(app);
    const proxyPort = proxyServer.address().port;
  ...
  it('replays parsed JSON bodies to generic API proxy requests', async () => {
    ...
    registerMageProxy(app, {
      ...
      getMageAuthHeaders: () => ({ Authorization: 'Bearer replay-token' }),
      buildMageUrl: (requestPath) => `${externalBaseUrl}${requestPath}`,
      ensureMageApiPrefix: () => {},
    });
    proxyServer = await listen(app);
  ...
  it('sanitizes experimental session list responses and forwards query params', async () => {
      ...
      res.json([
        {
          ...
          metadata: { mage: { kind: 'review', originalSessionID: 'ses_original' } },
    ...
    registerMageProxy(app, {
      ...
      getMageAuthHeaders: () => ({ Authorization: 'Bearer session-token' }),
      buildMageUrl: (requestPath) => `${externalBaseUrl}${requestPath}`,
      ensureMageApiPrefix: () => {},
    });
    proxyServer = await listen(app);
  ...
  it('uses the long proxy timeout budget for slow upstream responses', async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ error: 'Mage upstream timed out' });
  });
});
