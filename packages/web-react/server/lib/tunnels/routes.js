export const createTunnelRoutesRuntime = (dependencies) => {
  ...
  const registerRoutes = (app) => {
    app.get('/api/mage/tunnel/check', async (req, res) => {
      try {
    ...
    app.post('/api/mage/tunnel/doctor', handleTunnelDoctor);
    app.get('/api/mage/tunnel/doctor', handleTunnelDoctor);

    app.get('/api/mage/tunnel/providers', (_req, res) => {
      const providers = tunnelProviderRegistry.listCapabilities();
      return res.json({ providers });
    });

    app.get('/api/mage/tunnel/status', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.put('/api/mage/tunnel/managed-remote-token', async (req, res) => {
      try {
    ...
    app.post('/api/mage/tunnel/start', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.post('/api/mage/tunnel/stop', (_req, res) => {
      let revokedBootstrapCount = 0;
      let invalidatedSessionCount = 0;
      const activeTunnelId = tunnelAuthController.getActiveTunnelId();
