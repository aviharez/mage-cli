...
const RATE_LIMIT_MAX_ATTEMPTS = Number(process.env.MAGE_RATE_LIMIT_MAX_ATTEMPTS) || 10;
const RATE_LIMIT_LOCKOUT_MS = 15 * 60 * 1000;
const RATE_LIMIT_CLEANUP_MS = 60 * 60 * 1000;
const RATE_LIMIT_NO_IP_MAX_ATTEMPTS = Number(process.env.MAGE_RATE_LIMIT_NO_IP_MAX_ATTEMPTS) || 3;
...
const isUrlAuthReadableHttpPath = (pathname) => {
  return pathname === '/api/event'
    ...
    || pathname === '/api/mage/events'
    || pathname === '/api/mage/realtime-proxy/sse'
...
const isUrlAuthWebSocketPath = (pathname) => {
  return pathname === '/api/event/ws'
    ...
    || pathname === '/api/mage/realtime-proxy/ws'
...
function getOrCreateJwtSecret() {
  const envSecret = process.env.MAGE_JWT_SECRET;
  if (envSecret) {
...
function persistJwtSecret(secret) {
  if (process.env.MAGE_JWT_SECRET) {
    const error = new Error('Global sign-out is unavailable while MAGE_JWT_SECRET is set');
