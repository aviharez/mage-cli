...
| `cursor` | Cursor | `providers/cursor.js` | Environment/token files, Mage-managed credentials, or explicit one-time Cursor import |
...
| `ollama-cloud` | Ollama Cloud | `providers/ollama-cloud.js` | Manual cookie stored under `~/.config/mage/quota/` |
| `wafer` | Wafer.ai | `providers/wafer.js` | `wafer`, `wafer-ai`, `wafer_ai`, `wafer.ai` |
| `mage-go` | Mage Go | `providers/mage-go.js` | Manual workspace ID and auth cookie stored under `~/.config/mage/quota/` |
...
Mage Go, Ollama Cloud, and Cursor credentials are explicitly managed through Settings. The server validates credentials before atomic `0600` writes and never returns secrets through its API. Mage never scans browser cookie stores or automatically reads Cursor storage; Cursor import is an explicit one-time user action and never modifies Cursor's database.
