 ...
 * @param {string} [options.targetSource] - 'mage' or 'agents'
  ...
  for (const plan of skillPlans) {
    ...
    if (fs.existsSync(targetDir)) {
      ...
      if (!decision && !hasAutoPolicy) {
        conflicts.push({ skillName: plan.slug, scope, source: targetSource === 'agents' ? 'agents' : 'mage' });
      }
    }
  }
  ...
  for (const plan of skillPlans) {
    ...
    try {
      ...
      try {
        ...
        installed.push({ skillName: plan.slug, scope, source: targetSource === 'agents' ? 'agents' : 'mage' });
      } catch (extractError) {
