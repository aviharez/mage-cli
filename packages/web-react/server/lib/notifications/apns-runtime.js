...
//     for self-hosters who set MAGE_APNS_* and MAGE_PUSH_RELAY_DISABLED=true.
...
const DEFAULT_BUNDLE_ID = 'com.mage.app';
const DEFAULT_RELAY_URL = 'https://api.mage.dev/v1/push/send';
...
export const createApnsRuntime = (deps) => {
  ...
  const resolveApnsConfig = async () => {
    let keyId = trimmedEnv('MAGE_APNS_KEY_ID');
    let teamId = trimmedEnv('MAGE_APNS_TEAM_ID');
    let bundleId = trimmedEnv('MAGE_APNS_BUNDLE_ID');
    let environment = (trimmedEnv('MAGE_APNS_ENVIRONMENT') || '').toLowerCase();
    let p8 = normalizePem(process.env.MAGE_APNS_P8 || '');

    const p8Path = trimmedEnv('MAGE_APNS_P8_PATH');
    if (!p8 && p8Path) {
      try {
        p8 = (await fsPromises.readFile(p8Path, 'utf8')).trim();
      } catch (error) {
        console.warn('[APNs] Failed to read MAGE_APNS_P8_PATH:', error?.message ?? error);
      }
    }
  ...
  // is the fallback for self-hosters who set MAGE_APNS_* and disable the relay.
  const resolveRelayConfig = () => {
    if (trimmedEnv('MAGE_PUSH_RELAY_DISABLED') === 'true') return null;
    const url = trimmedEnv('MAGE_PUSH_RELAY_URL') || DEFAULT_RELAY_URL;
    return {
      url,
      registerUrl: url.replace(/\/send$/, '/register-token'),
      environment:
        (trimmedEnv('MAGE_APNS_ENVIRONMENT') || 'sandbox').toLowerCase() === 'production'
          ? 'production'
          : 'sandbox',
    };
  };
  ...
  const sendViaRelay = async (deviceTokens, payload, relay) => {
    ...
    const title = typeof payload?.title === 'string' && payload.title.length > 0 ? payload.title : 'Mage';
    const { privateKey, publicJwk } = await getOrCreateRelayKeypair();
    const ts = Date.now();
  ...
  const sendViaDirectApns = async (deviceTokens, payload) => {
    ...
    if (!config) {
      if (!warnedUnconfigured) {
        ...
        console.warn(
          '[APNs] Relay disabled and no direct config; set MAGE_APNS_KEY_ID / MAGE_APNS_TEAM_ID / MAGE_APNS_P8 for direct send.',
        );
      }
      return;
    }
