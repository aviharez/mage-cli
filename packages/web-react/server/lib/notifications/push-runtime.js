...
export const createPushRuntime = (deps) => {
  ...
  const ensurePushInitialized = async () => {
    ...
    if (subject === 'mailto:mage@localhost') {
      console.warn('[Push] No public origin configured for VAPID; set MAGE_VAPID_SUBJECT or enable push once from a real origin.');
    }
