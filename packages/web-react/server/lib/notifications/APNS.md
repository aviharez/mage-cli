...
4. The **relay** (`mage-website/apps/api`, Cloudflare Worker) verifies the signature +
...
- **Relay (default):** server has no Apple key; `MAGE_PUSH_RELAY_URL` defaults to
  `https://api.mage.dev/v1/push/send` (register URL is derived as `…/register-token`).
- **Direct (fallback):** set `MAGE_PUSH_RELAY_DISABLED=true` + `MAGE_APNS_KEY_ID/
  TEAM_ID/P8` to sign+send from the server itself (HTTP/2 + ES256 JWT); no relay binding needed.

## Config

Server (`apns-runtime.js`):
- `MAGE_PUSH_RELAY_URL` (default the public relay), `MAGE_APNS_ENVIRONMENT`
  (`sandbox` default / `production`). The signing keypair is auto-generated — nothing to set.
- Direct fallback: `MAGE_APNS_KEY_ID`, `MAGE_APNS_TEAM_ID`, `MAGE_APNS_P8`
  (or `_P8_PATH`), `MAGE_APNS_BUNDLE_ID`, `MAGE_PUSH_RELAY_DISABLED=true`.
...
1. Apple **Keys** (not Certificates) → create an **APNs Auth Key** (`.p8`) → Key ID + Team ID;
   enable **Push Notifications** on App ID `com.mage.app`.
2. In the **mage-website** repo → Actions secrets: `APNS_P8` (PEM), `APNS_KEY_ID`,
