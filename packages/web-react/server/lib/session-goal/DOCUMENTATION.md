...
objective stored under `metadata.mage.goal`, with the small model as
an independent progress auditor. Built on Mage's backend-driven
architecture (session-assist is the structural template): the loop lives in
the web server and survives UI disconnects.

## Goal payload (`metadata.mage.goal`)
...
- `routes.js` — `PUT/GET/DELETE /api/goals/objective/:sessionId`
  (Mage-owned, registered before the generic proxy; JSON parsing via
...
stamp `metadata.mage.goal` onto the fresh session (objective = the
