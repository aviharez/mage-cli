export const createGracefulShutdownRuntime = (dependencies) => {
  ...
  const runShutdown = async (options = {}) => {
    ...
    if (!shouldSkipMageStop()) {
      const portToKill = getMagePort();
      const mageProcess = getMageProcess();

      if (mageProcess) {
        console.log('Stopping Mage process...');
        try {
          await mageProcess.close();
        } catch (error) {
      ...
      if (!(await waitForPortRelease(portToKill, 5000))) {
        console.warn(`Timed out waiting for Mage port ${portToKill} to be released during shutdown`);
      }
    } else {
      console.log('Skipping Mage shutdown (external server)');
    }
