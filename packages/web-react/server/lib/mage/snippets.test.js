...
describe('snippets', () => {
  beforeEach(() => {
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mage-snippets-'));
  });
  ...
  test('loads project snippets with aliases and description', () => {
    writeSnippet('.mage/snippet/review.md', '---\naliases: [rev]\ndescription: Review helper\n---\nReview carefully.');
  ...
  test('snippet directory wins over snippets directory', () => {
    writeSnippet('.mage/snippets/same.md', 'Old');
    writeSnippet('.mage/snippet/same.md', 'New');

    expect(getSnippet('same', projectDir)?.content).toBe('New');
  });

  test('canonical snippet names take precedence over colliding aliases', () => {
    writeSnippet('.mage/snippets/review.md', 'Review by name');
    writeSnippet('.mage/snippet/helper.md', '---\naliases: [review, help]\n---\nReview by alias');
  ...
  test('canonical snippet names override earlier same-directory aliases', () => {
    writeSnippet('.mage/snippet/alias-first.md', '---\naliases: [review, assist]\n---\nReview by alias');
    writeSnippet('.mage/snippet/review.md', 'Review by name');

    const snippetDir = path.join(projectDir, '.mage', 'snippet');
  ...
  test('expands snippets recursively with prepend and append blocks', () => {
    writeSnippet('.mage/snippet/base.md', 'Base text');
    writeSnippet('.mage/snippet/review.md', '<prepend>Before</prepend>Review #base<append>After</append>');
