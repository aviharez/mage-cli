...
let rootDir;
let plugins;
let refreshMageAfterConfigChange;
let app;
let cleanupPaths;
...
describe('mage plugin routes', () => {
  beforeAll(async () => {
    rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mage-plugin-routes-'));
    userConfigPath = path.join(rootDir, 'user-mage.json');
  ...
  beforeEach(() => {
    ...
    refreshMageAfterConfigChange = mock(async () => undefined);
    cleanupPaths = [];
    app = createApp();
  });
  ...
  test('POST /entry creates entry and requires reload', async () => {
    ...
    expect(refreshMageAfterConfigChange).toHaveBeenCalledWith('plugin entry creation');
  });
  ...
  test('PATCH /entry/:id updates entry in same array index', async () => {
    ...
    expect(refreshMageAfterConfigChange).toHaveBeenCalledWith('plugin entry update');
  });
  ...
  test('DELETE /entry/:id removes entry and prunes plugin key', async () => {
    ...
    expect(refreshMageAfterConfigChange).toHaveBeenCalledWith('plugin entry deletion');
  });
  ...
  test('POST /file writes plugin dir file', async () => {
    ...
    expect(refreshMageAfterConfigChange).toHaveBeenCalledWith('plugin file creation');
  });
  ...
  test('PUT /file/:id updates file content', async () => {
    ...
    expect(refreshMageAfterConfigChange).toHaveBeenCalledWith('plugin file update');
  });
  ...
  test('DELETE /file/:id unlinks file', async () => {
    ...
    expect(refreshMageAfterConfigChange).toHaveBeenCalledWith('plugin file deletion');
  });
