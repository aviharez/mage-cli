export const parseServeCliOptions = ({
  ...
  const envPassword =
    ...
    env.MAGE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.MAGE_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.MAGE_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.MAGE_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.MAGE_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.MAGE_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.MAGE_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.MAGE_API_ONLY === '1' || env.MAGE_API_ONLY === 'true';
