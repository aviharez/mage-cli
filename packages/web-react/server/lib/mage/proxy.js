...
export const normalizeForwardedDirectoryHeaders = (headers) => {
  const rawDirectory = headers?.['x-mage-directory'];
  if (typeof rawDirectory !== 'string') {
    return headers;
  }

  if (headers['x-mage-directory-encoding'] !== 'uri') {
    return headers;
  }

  try {
    headers['x-mage-directory'] = decodeURIComponent(rawDirectory);
  } catch {
    // Leave malformed values untouched; upstream will reject invalid paths.
  }
  delete headers['x-mage-directory-encoding'];
  return headers;
};
...
export const registerMageProxy = (app, deps) => {
  const {
    fs,
    os,
    path,
  ...
  if (app.get('mageProxyConfigured')) {
    return;
  }

  const runtime = getRuntime();
  if (runtime.magePort) {
    console.log(`Setting up proxy to Mage on port ${runtime.magePort}`);
  } else {
    console.log('Setting up Mage API gate (Mage not started yet)');
  }
  app.set('mageProxyConfigured', true);
  ...
  const resolveProxyTarget = () => {
    try {
      const resolved = normalizeProxyTarget(buildMageUrl('/', ''));
      if (resolved) {
        return resolved;
      }
    } catch {
    ...
    const externalBase = normalizeProxyTarget(runtimeState.mageBaseUrl);
    if (externalBase) {
      return externalBase;
    }
  ...
  const PROXY_TIMEOUT_MARKER = Symbol('mageProxyTimedOut');
  ...
  const sendProxyErrorResponse = (res, statusCode) => {
    ...
    res.status(statusCode).json({ error: statusCode === 504 ? 'Mage upstream timed out' : 'Mage service unavailable' });
    return true;
  };
  ...
  const forwardSseRequest = async (req, res) => {
    ...
    try {
      ...
      const headers = normalizeForwardedDirectoryHeaders(
        collectForwardProxyHeaders(req.headers, getMageAuthHeaders())
      );
      headers.accept ??= 'text/event-stream';
      headers['cache-control'] ??= 'no-cache';

      upstream = await fetch(buildMageUrl(upstreamPath, ''), {
        method: 'GET',
        headers,
      ...
      console.error('[proxy] Mage SSE proxy error:', error?.message ?? error);
      if (!res.headersSent) {
        res.status(503).json({ error: 'Mage service unavailable' });
      } else {
        res.end();
      }
    } finally {
  ...
  const fetchSessionListPayload = async (upstreamPath, { req = null, timeoutMs = null } = {}) => {
    const headers = req
      ? {
          ...normalizeForwardedDirectoryHeaders(collectForwardProxyHeaders(req.headers, getMageAuthHeaders())),
    ...
    const upstream = await fetch(buildMageUrl(upstreamPath, ''), {
      method: 'GET',
      headers,
  ...
  const forwardSanitizedSessionListRequest = async (req, res, next, logLabel) => {
    try {
      ...
      console.error(`[proxy] Mage ${logLabel} proxy error:`, error?.message ?? error);
      if (!res.headersSent) {
        res.status(503).json({ error: 'Mage service unavailable' });
        return;
      }
  ...
  // Readiness gate — while Mage is starting/restarting, HOLD the request and
  ...
  // request until Mage is ready (typically well under a second) lets the
  ...
  const isStillWaiting = (runtimeState) => {
    const waitElapsed = runtimeState.mageNotReadySince === 0 ? 0 : Date.now() - runtimeState.mageNotReadySince;
    return (
      (!runtimeState.isMageReady && (runtimeState.mageNotReadySince === 0 || waitElapsed < OPEN_CODE_READY_GRACE_MS)) ||
      runtimeState.isRestartingMage ||
      !runtimeState.magePort
    );
  };
  ...
  app.use('/api', async (req, res, next) => {
    if (
      ...
      req.path.startsWith('/config/mage-resolution') ||
    ...
    app.get('/api/session', async (req, res, next) => {
      ...
      try {
        ...
        if (!globalSessions && successfulProjectReads === 0) {
          return res.status(504).json({ error: 'Mage session list timed out' });
        }
  ...
  // Generic proxy for non-SSE Mage API routes.
  const apiProxy = createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        // Inject Mage auth headers
        const authHeaders = getMageAuthHeaders();
        if (authHeaders.Authorization) {
          proxyReq.setHeader('Authorization', authHeaders.Authorization);
        }

        if (req.headers?.['x-mage-directory-encoding'] === 'uri') {
          const rawDirectory = req.headers['x-mage-directory'];
          if (typeof rawDirectory === 'string') {
            try {
              proxyReq.setHeader('x-mage-directory', decodeURIComponent(rawDirectory));
            } catch {
              proxyReq.setHeader('x-mage-directory', rawDirectory);
            }
          }
          proxyReq.removeHeader?.('x-mage-directory-encoding');
        }

        // Defensive: request identity encoding from upstream Mage.
      ...
      error: (err, req, res) => {
        console.error('[proxy] Mage proxy error:', err.message);
