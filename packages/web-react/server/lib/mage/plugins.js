...
function parsedKindForSpec(spec) {
  ...
  // Do NOT use `includes(path.sep)` — scoped npm packages legitimately contain '/' (e.g. `@gitlab/mage-gitlab-auth`).
  return isPathSpec(spec) ? 'path' : 'npm';
}

function getActiveMageConfigDir() {
...
function ensureProjectConfigPath(workingDirectory) {
  ...
  const configDir = path.join(workingDirectory, '.mage');
  fs.mkdirSync(configDir, { recursive: true });
  return path.join(configDir, 'mage.json');
}
