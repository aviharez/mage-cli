...
describe('project directory runtime', () => {
  ...
  describe('resolveProjectDirectory', () => {
    it('resolves symlinks in x-mage-directory header', async () => {
      const runtime = createTestRuntime({
        fsPromises: {
          stat: async () => ({ isDirectory: () => true }),
      ...
      const req = {
        get: (header) => header === 'x-mage-directory' ? '/home/user/workspace/project' : null,
        query: {},
      };
    ...
    it('decodes marked x-mage-directory header values', async () => {
      ...
      const req = {
        get: (header) => {
          if (header === 'x-mage-directory') return encodeURIComponent(pathWithUnicode);
          if (header === 'x-mage-directory-encoding') return 'uri';
          return null;
        },
        query: {},
      };
    ...
    it('preserves raw percent sequences without directory encoding marker', async () => {
      ...
      const req = {
        get: (header) => header === 'x-mage-directory' ? rawPath : null,
        query: {},
      };
    ...
    it('falls back to query directory when an unmarked encoded header is invalid', async () => {
      ...
      const req = {
        get: (header) => header === 'x-mage-directory' ? encodeURIComponent(validPath) : null,
  ...
  describe('resolveOptionalProjectDirectory', () => {
    ...
    it('resolves symlinks when directory is provided', async () => {
      ...
      const req = {
        get: (header) => header === 'x-mage-directory' ? '/symlink/workspace/project' : null,
        query: {},
      };
