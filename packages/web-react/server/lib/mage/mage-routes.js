export const registerMageRoutes = (app, dependencies) => {
  ...
  app.get('/api/mage/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/mage/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(mageDataDir, 'run', `mage-${currentPort}.json`);
      ...
      let restartCmdFallback = `mage serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(mageDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== Mage update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting Mage...
              ${restartCmd || 'echo Service manager will restart Mage.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/mage/models-metadata', async (_req, res) => {
    try {
