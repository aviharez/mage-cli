...
export const registerMageRoutes = (app, dependencies) => {
  const {
    crypto,
    clientReloadDelayMs,
  ...
  const isBundledMageBinaryActive = async () => {
    const settings = await readSettingsFromDiskMigrated();
    const resolution = await getMageResolutionSnapshot(settings);
    return resolution?.source === 'bundled' || resolution?.detectedSourceNow === 'bundled';
  };

  const readMageCurrentVersion = async () => {
    const healthResponse = await fetch(buildMageUrl('/global/health', ''), {
      method: 'GET',
  ...
  app.get('/api/config/mage-resolution', async (_req, res) => {
    try {
      const settings = await readSettingsFromDiskMigrated();
      const resolution = await getMageResolutionSnapshot(settings);
      res.json(resolution);
    } catch (error) {
      console.error('Failed to resolve Mage binary:', error);
      res.status(500).json({ error: 'Failed to resolve Mage binary' });
    }
  });

  app.post('/api/mage/upgrade', async (req, res) => {
    try {
      if (await isBundledMageBinaryActive()) {
        return res.status(409).json({
          success: false,
          error: 'Mage is bundled with Mage Desktop and cannot be upgraded separately.',
        });
      }
      ...
      const target = typeof req.body?.target === 'string' && req.body.target.trim().length > 0
        ...
        : undefined;
      const response = await fetch(buildMageUrl('/global/upgrade', ''), {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        return res.status(response.status).json({
          success: false,
          error: payload?.error || response.statusText || 'Failed to upgrade Mage',
        });
      }

      try {
        await refreshMageAfterConfigChange('Mage upgrade');
      } catch (restartError) {
        return res.status(500).json({
          success: false,
          upgraded: true,
          error: restartError instanceof Error
            ? `Mage upgraded, but restart failed: ${restartError.message}`
            : 'Mage upgraded, but restart failed',
        });
      }

      return res.json({ ...(payload ?? { success: true }), restarted: true });
    } catch (error) {
      console.error('Failed to upgrade Mage:', error);
      return res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to upgrade Mage',
      });
    }
  });

  app.get('/api/mage/upgrade-status', async (_req, res) => {
    try {
      if (await isBundledMageBinaryActive()) {
        const current = await readMageCurrentVersion().catch(() => ({ ok: false, currentVersion: null }));
        return res.json({
          available: false,
      ...
      const [healthResponse, latestVersion] = await Promise.all([
        fetch(buildMageUrl('/global/health', ''), {
          method: 'GET',
          headers: { Accept: 'application/json', ...getMageAuthHeaders() },
        }),
        fetchLatestMageVersion(),
      ]);
      const health = await healthResponse.json().catch(() => null);
      if (!healthResponse.ok) {
        return res.status(healthResponse.status).json({
          available: null,
          error: health?.error || healthResponse.statusText || 'Failed to read Mage version',
        });
      }
      const currentVersion = typeof health?.version === 'string' ? health.version.replace(/^v/, '') : null;
      ...
      return res.status(500).json({
        available: null,
        error: error instanceof Error ? error.message : 'Failed to check Mage upgrade status',
      });
    }
  });

  app.get('/api/mage/health', async (_req, res) => {
    try {
      const healthResponse = await fetch(buildMageUrl('/global/health', ''), {
        method: 'GET',
        headers: { Accept: 'application/json', ...getMageAuthHeaders() },
      ...
      if (!healthResponse.ok) {
        return res.status(healthResponse.status).json({
          healthy: false,
          error: health?.error || healthResponse.statusText || 'Mage health check failed',
        });
      }
      ...
      return res.status(503).json({
        healthy: false,
        error: error instanceof Error ? error.message : 'Mage health check failed',
      });
    }
  });

  app.get('/api/mage/version', async (_req, res) => {
    try {
      const healthResponse = await fetch(buildMageUrl('/global/health', ''), {
        method: 'GET',
        headers: { Accept: 'application/json', ...getMageAuthHeaders() },
      ...
      return res.status(500).json({
        version: null,
        error: error instanceof Error ? error.message : 'Failed to read Mage version',
      });
    }
  });
  ...
  app.get('/api/provider/:providerId/source', async (req, res) => {
    try {
      ...
      const headerDirectory = typeof req.get === 'function' ? req.get('x-mage-directory') : null;
  ...
  app.delete('/api/provider/:providerId/auth', async (req, res) => {
    try {
      ...
      if (removed) {
        await refreshMageAfterConfigChange(`provider ${providerId} disconnected (${scope})`);
      }
  ...
  app.post('/api/mage/directory', async (req, res) => {
    try {
      ...
      console.error('Failed to update Mage working directory:', error);
  ...
  app.put('/api/behavior/agents-md', async (req, res) => {
    try {
      ...
      // Refresh Mage so it picks up the new AGENTS.md without a full restart
      try {
        await refreshMageAfterConfigChange('global behavior (AGENTS.md) updated');
      } catch {
