...
describe('settings helpers', () => {
  ...
  it('accepts Mage update notification preference as a persisted shared setting', () => {
    const helpers = createTestHelpers();

    expect(helpers.sanitizeSettingsUpdate({ showMageUpdateNotifications: false })).toEqual({
      showMageUpdateNotifications: false,
    });
    expect(helpers.sanitizeSettingsUpdate({ showMageUpdateNotifications: true })).toEqual({
      showMageUpdateNotifications: true,
    });
  });

  it('accepts dismissed Mage update toast version as a persisted shared setting', () => {
    const helpers = createTestHelpers();

    expect(helpers.sanitizeSettingsUpdate({ mageUpdateToastDismissedVersion: ' 1.16.0 ' })).toEqual({
      mageUpdateToastDismissedVersion: '1.16.0',
    });
    expect(helpers.sanitizeSettingsUpdate({ mageUpdateToastDismissedVersion: '' })).toEqual({
      mageUpdateToastDismissedVersion: '',
    });
  });
  ...
  it('includes transient desktop LAN access runtime status in desktop settings response', () => {
    const helpers = createTestHelpers();
    const previousRuntime = process.env.MAGE_RUNTIME;
    const previousActive = process.env.MAGE_DESKTOP_LAN_ACCESS_ACTIVE;
    const previousReason = process.env.MAGE_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    try {
      process.env.MAGE_RUNTIME = 'desktop';
      process.env.MAGE_DESKTOP_LAN_ACCESS_ACTIVE = 'false';
      process.env.MAGE_DESKTOP_LAN_ACCESS_BLOCKED_REASON = 'missing-password';
      ...
      if (typeof previousRuntime === 'string') process.env.MAGE_RUNTIME = previousRuntime;
      else delete process.env.MAGE_RUNTIME;
      if (typeof previousActive === 'string') process.env.MAGE_DESKTOP_LAN_ACCESS_ACTIVE = previousActive;
      else delete process.env.MAGE_DESKTOP_LAN_ACCESS_ACTIVE;
      if (typeof previousReason === 'string') process.env.MAGE_DESKTOP_LAN_ACCESS_BLOCKED_REASON = previousReason;
      else delete process.env.MAGE_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    }
  });
