...
import { createMageWatcherRuntime } from './watcher.js';
...
describe('createMageWatcherRuntime', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('waits for Mage readiness and forwards unwrapped global SSE payloads', async () => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    const payloads = [];
    const fetchCalls = [];

    const watcher = createMageWatcherRuntime({
      waitForMagePort: async () => {
