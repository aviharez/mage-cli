...
describe('server utils runtime', () => {
  it('prefers shell PATH for managed Mage before appending process-only entries', () => {
    const home = os.homedir();
    const currentPath = [
      path.join(home, '.mage', 'bin'),
    ...
    expect(runtime.buildManagedMagePath()).toBe([
      path.join(home, '.mage', 'bin'),
      path.join(home, '.bun', 'bin'),
      '/opt/homebrew/bin',
      '/usr/bin',
  ...
  it('uses login shell PATH for managed Mage when process PATH is minimal', () => {
    const home = os.homedir();
  ...
  it('adds existing Windows package-manager directories to managed Mage PATH', () => {
    const root = createTempDir('mage-win-path-');
