...
export const createProjectDirectoryRuntime = (dependencies) => {
  ...
  const resolveProjectDirectory = async (req) => {
    const rawHeaderDirectory = typeof req.get === 'function' ? req.get('x-mage-directory') : null;
    const headerEncoding = typeof req.get === 'function' ? req.get('x-mage-directory-encoding') : null;
