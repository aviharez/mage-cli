export const registerConfigEntityRoutes = (app, dependencies) => {
  ...
  // Build the response for a config mutation based on whether Mage actually
  // reloaded the change. When connected to an external Mage server that
  // Mage cannot restart, the change is persisted to disk but the running
  ...
  const completeMcpMutation = async (res, action, name, applyChange) => {
    ...
    try {
      await refreshMageAfterConfigChange(`mcp ${action}`);
      return res.json({
        success: true,
        requiresReload: true,
      ...
      return res.json({
        ...
        requiresReload: false,
        reloadFailed: true,
        message: `MCP server "${name}" ${action}d, but Mage reload failed.`,
        warning: error.message || 'Mage reload failed after the MCP configuration changed',
      });
    }
  };
  ...
  app.post('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      const refreshResult = await refreshMageAfterConfigChange('agent creation', {
        agentName
      });

      res.json(buildConfigMutationResponse(refreshResult, {
        liveMessage: `Agent ${agentName} created successfully. Reloading interface…`,
        manualRestartMessage: `Agent ${agentName} saved. Restart your connected Mage server to apply the change.`,
      }));
    } catch (error) {
  ...
  app.patch('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      const refreshResult = await refreshMageAfterConfigChange('agent update');
  ...
  app.delete('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      const refreshResult = await refreshMageAfterConfigChange('agent deletion');

      res.json(buildConfigMutationResponse(refreshResult, {
        liveMessage: `Agent ${agentName} deleted successfully. Reloading interface…`,
        manualRestartMessage: `Agent ${agentName} deleted. Restart your connected Mage server to apply the change.`,
      }));
    } catch (error) {
  ...
  app.post('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshMageAfterConfigChange('command creation', {
        commandName
      });
  ...
  app.patch('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshMageAfterConfigChange('command update');
  ...
  app.delete('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshMageAfterConfigChange('command deletion');
