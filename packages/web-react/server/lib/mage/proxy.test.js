...
describe('normalizeForwardedDirectoryHeaders', () => {
  it('decodes marked directory headers before forwarding to Mage', () => {
    const headers = normalizeForwardedDirectoryHeaders({
      'x-mage-directory': encodeURIComponent('/Users/example/project'),
      'x-mage-directory-encoding': 'uri',
    });

    expect(headers).toEqual({
      'x-mage-directory': '/Users/example/project',
    });
  });

  it('preserves unmarked percent sequences from direct clients', () => {
    const headers = normalizeForwardedDirectoryHeaders({
      'x-mage-directory': '/Users/example/project%20literal',
    });
