...
function ensureProjectSkillDir(workingDirectory) {
  const projectSkillDir = path.join(workingDirectory, '.mage', 'skills');
  if (!fs.existsSync(projectSkillDir)) {
    fs.mkdirSync(projectSkillDir, { recursive: true });
  }
  const legacyProjectSkillDir = path.join(workingDirectory, '.mage', 'skill');
...
function createSkill(skillName, config, workingDirectory, scope) {
  ...
  const requestedSource = config?.source === 'agents' ? 'agents' : 'mage';
