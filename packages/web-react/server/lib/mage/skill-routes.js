...
export const registerSkillRoutes = (app, dependencies) => {
  ...
  const fetchMageDiscoveredSkills = async (workingDirectory) => {
    if (!getMagePort()) {
      return [];
    }
    ...
    try {
      const client = createMageClient({
        baseUrl: buildMageUrl('/', '').replace(/\/$/, ''),
        directory: workingDirectory || undefined,
        headers: getMageAuthHeaders(),
      ...
      console.error('Failed to list Mage skills:', error);
      return [];
    }
  };
  ...
  app.get('/api/config/skills', async (req, res) => {
    try {
      ...
      const mageSkills = await fetchMageDiscoveredSkills(directory);
      const localSkills = discoverSkills(directory);
      const skills = mergeDiscoveredSkills(mageSkills, localSkills);
  ...
  app.get('/api/config/skills/catalog/source', async (req, res) => {
    try {
      ...
      const resolvedDiscovered = mergeDiscoveredSkills(
        await fetchMageDiscoveredSkills(directory),
  ...
  app.post('/api/config/skills/install', async (req, res) => {
    try {
      ...
      if (isClawdHubSource(source)) {
        ...
        if (requiresReload) {
          await refreshMageAfterConfigChange('skills install');
        }
  ...
  app.get('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      const discoveredSkill = (await fetchMageDiscoveredSkills(directory))
  ...
  app.post('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshMageAfterConfigChange('skill creation');
  ...
  app.patch('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshMageAfterConfigChange('skill update');
  ...
  app.delete('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshMageAfterConfigChange('skill deletion');
