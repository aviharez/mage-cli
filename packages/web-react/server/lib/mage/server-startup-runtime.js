export const createServerStartupRuntime = (dependencies) => {
        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('Mage startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'mage:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`Mage server listening on ${bindHost}:${activePort}`);
  ...
  const attachProcessHandlers = ({ attachSignals }) => {
    if (attachSignals && !getSignalsAttached()) {
      ...
      // the managed Mage child is always torn down gracefully instead of
