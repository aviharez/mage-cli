...
describe('mage plugins data layer', () => {
  beforeAll(async () => {
    rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mage-plugins-'));
  ...
  test('routes project entries to project config and user entries to custom user config', () => {
    ...
    expect(readJson(path.join(projectDir, '.mage', 'mage.json')).plugin).toEqual(['project-plugin']);
  });

  test('re-resolves custom config env between calls', () => {
    const firstConfigPath = path.join(rootDir, 'first', 'mage.json');
    const secondConfigPath = path.join(rootDir, 'second', 'mage.json');
  ...
  test('lists entries from user and project layers with scopes and parsed kinds', () => {
    ...
    writeJson(path.join(projectDir, '.mage', 'mage.json'), { plugin: ['./local-plugin.js'] });
    ...
    expect(entries).toEqual([
      ...
      expect.objectContaining({ spec: './local-plugin.js', scope: 'project', kind: 'config', parsedKind: 'path', sourcePath: path.join(projectDir, '.mage', 'mage.json') }),
    ]);
    fs.rmSync(userConfigPath, { force: true });
  ...
  test('rejects duplicate plugin dir files unless overwrite is true', () => {
    ...
    expect(fs.readFileSync(path.join(projectDir, '.mage', 'plugins', 'dup.js'), 'utf8')).toBe('two');
  });

  test('lists only valid plugin dir files', () => {
    const dir = path.join(projectDir, '.mage', 'plugins');
