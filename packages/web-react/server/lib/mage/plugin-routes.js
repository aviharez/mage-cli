...
export const registerPluginRoutes = (app, dependencies) => {
  ...
  const completePluginMutation = async (res, operation, _noun, applyChange) => {
    ...
    try {
      await refreshMageAfterConfigChange(`plugin ${operation}`);
      ...
      return res.json({
        ...
        message: `Plugin ${pastTense}, but Mage reload failed.`,
        reloadDelayMs: clientReloadDelayMs,
        reloadFailed: true,
        warning: error.message || 'Mage reload failed after plugin config changed',
      });
    }
  };
