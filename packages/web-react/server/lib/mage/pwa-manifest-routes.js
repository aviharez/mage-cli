const DEFAULT_PWA_APP_NAME = 'Mage - AI Coding Assistant';
...
export const registerPwaManifestRoute = (app, dependencies) => {
  ...
  const getRecentPwaSessionShortcuts = async (req) => {
    ...
    const listSessions = async (directory) => {
      ...
      const response = await fetch(buildMageUrl(`/session${query}`, ''), {
        method: 'GET',
        headers: {
  ...
  app.get('/manifest.webmanifest', async (req, res) => {
    ...
    const manifest = {
      ...
      description: 'Web interface companion for Mage AI coding agent',
      id: '/',
      start_url: '/',
      scope: '/',
      display: 'standalone',
