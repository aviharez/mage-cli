export const createSettingsHelpers = (dependencies) => {
  ...
  const sanitizeSettingsUpdate = (payload) => {
    ...
    // Absolute path to the mage CLI binary (optional override).
    ...
    if (typeof candidate.mageBinary === 'string') {
      const normalized = normalizeDirectoryPath(candidate.mageBinary).trim();
      result.mageBinary = normalized;
    }
    ...
    if (typeof candidate.mageUpdateToastDismissedVersion === 'string') {
      const version = candidate.mageUpdateToastDismissedVersion.trim();
      result.mageUpdateToastDismissedVersion = version.slice(0, VERSION_STRING_MAX_LENGTH);
    }
  ...
  const formatSettingsResponse = (settings) => {
    ...
    return {
      ...
      ...(process.env.MAGE_RUNTIME === 'desktop'
        ? {
            desktopLanAccessActive: process.env.MAGE_DESKTOP_LAN_ACCESS_ACTIVE === 'true',
