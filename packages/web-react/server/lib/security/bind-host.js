...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.MAGE_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `Mage refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or MAGE_UI_PASSWORD before exposing it over LAN, '
  + 'or set MAGE_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';
