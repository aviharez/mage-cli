...
### `isAvailable()`
Returns boolean indicating whether OpenAI API key is configured (checks environment variable `OPENAI_API_KEY` or Mage auth file).
...
2. Mage auth file (`auth.openai`, `auth.codex`, or `auth.chatgpt`).
...
### Manual verification
1. Configure OpenAI API key via environment variable or Mage settings.
