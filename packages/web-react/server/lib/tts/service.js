...
function getOpenAIApiKey() {
  ...
  // Then check mage auth file (same as usage tracker)
  try {
...
class TTSService {
  ...
  async generateSpeechStream(options) {
    ...
    if (!client) {
      throw new Error('TTS service not available. Configure OpenAI in Mage, provide an API key, or set a custom server URL in settings.');
    }
  ...
  async generateSpeechBuffer(options) {
    ...
    if (!client) {
      throw new Error('OpenAI API key not configured. Set OPENAI_API_KEY environment variable or configure OpenAI in Mage.');
    }
