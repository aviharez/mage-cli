...
describe('normalizeCustomOpenAIBaseURL', () => {
  const originalRuntime = process.env.MAGE_RUNTIME;
  const originalAllowRemote = process.env.MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS;

  afterEach(() => {
    // Restore env vars after each test
    if (originalRuntime === undefined) {
      delete process.env.MAGE_RUNTIME;
    } else {
      process.env.MAGE_RUNTIME = originalRuntime;
    }
    if (originalAllowRemote === undefined) {
      delete process.env.MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
    } else {
      process.env.MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS = originalAllowRemote;
    }
  });

  it('rejects remote URLs when MAGE_RUNTIME is not set (web)', () => {
    delete process.env.MAGE_RUNTIME;
    delete process.env.MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  it('allows remote URLs when MAGE_RUNTIME is desktop', () => {
  ...
  it('allows remote URLs when MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS is true', () => {
    delete process.env.MAGE_RUNTIME;
    process.env.MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'true';
  ...
  it('denies remote URLs on desktop when env var is explicitly false', () => {
    process.env.MAGE_RUNTIME = 'desktop';
    process.env.MAGE_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'false';
