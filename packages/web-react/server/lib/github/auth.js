...
const STORAGE_DIR = MAGE_DATA_DIR;
const STORAGE_FILE = path.join(STORAGE_DIR, 'github-auth.json');
const SETTINGS_FILE = path.join(MAGE_DATA_DIR, 'settings.json');
...
function writeJsonFile(payload) {
  ...
  // Atomic write so multiple Mage instances can safely share the same file.
...
export function getGitHubClientId() {
  const raw = process.env.MAGE_GITHUB_CLIENT_ID;
...
export function getGitHubScopes() {
  const raw = process.env.MAGE_GITHUB_SCOPES;
