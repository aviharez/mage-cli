...
- This module owns GitHub auth, Octokit access, repo resolution, and Pull Request status resolution for Mage.
...
- Auth storage: `~/.config/mage/github-auth.json`
- Writes are atomic and file mode is `0o600`.
- Client ID resolution order: `MAGE_GITHUB_CLIENT_ID` -> `settings.json` -> default.
- Scope resolution order: `MAGE_GITHUB_SCOPES` -> `settings.json` -> default.
...
- PR state is persisted in local storage under `mage.github-pr-status`.
