...
const createCommandTimeoutMs = () => {
  const raw = Number(process.env.MAGE_FS_EXEC_TIMEOUT_MS);
...
const createGitReadCacheTtlMs = () => {
  const raw = Number(process.env.MAGE_GIT_READ_CACHE_TTL_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 30 * 1000;
};

const createGitCheckIgnoreTimeoutMs = () => {
  const raw = Number(process.env.MAGE_GIT_CHECK_IGNORE_TIMEOUT_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 2500;
};
...
const resolveWorkspacePath = ({ targetPath, baseDirectory, path, os, normalizeDirectoryPath, mageUserConfigRoot }) => {
  ...
  if (isPathWithinRoot(resolved, mageUserConfigRoot, path, os)) {
    return { ok: true, base: path.resolve(mageUserConfigRoot), resolved };
  }
...
const resolveWorkspacePathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, mageUserConfigRoot }) => {
...
const resolveReadPathFromContext = async ({ req, targetPath, scope, resolveProjectDirectory, path, os, fsPromises, normalizeDirectoryPath, mageUserConfigRoot }) => {
...
export const registerFsRoutes = (app, dependencies) => {
  ...
  app.get('/api/fs/list', async (req, res) => {
    ...
    const isPlansDirectory = (value) => {
      ...
      return normalized.endsWith('/.mage/plans') || normalized.endsWith('.mage/plans');
    };
