...
import { FileSystemWatcher } from "@mybcabisnis/mage-schema/filesystem-watcher"
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof MAGE_LIBC === "undefined" ? undefined : MAGE_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@mage/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.MAGE_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.MAGE_EXPERIMENTAL_FILEWATCHER)) {
