...
import { ToolFailure } from "@mybcabisnis/mage-llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@mage/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.MAGE_WEBSEARCH_PROVIDER === "exa" || process.env.MAGE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.MAGE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("MAGE_EXPERIMENTAL") || truthy("MAGE_ENABLE_EXA") || truthy("MAGE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("MAGE_ENABLE_PARALLEL") || truthy("MAGE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `mage/${InstallationVersion}`,
