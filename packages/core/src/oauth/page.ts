...
// etc.).
//
// The visual language mirrors the Mage app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Mage is now connected to ${escapeHtml(provider)}.` : "Mage is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Mage couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Mage couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Mage.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Mage</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Mage couldn't finish connecting to "+PROVIDER+"."):"Mage couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Mage."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Mage is now connected to "+PROVIDER+"."):"Mage is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Mage wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 42" fill="none" aria-label="Mage" role="img" shape-rendering="crispEdges">
  <path d="M0 6H6V12H12V18H18V12H24V6H30V36H24V18H18V24H12V18H6V36H0V6Z" fill="#6f6f6f"/>
  <path d="M42 6H54V12H60V36H54V24H42V36H36V12H42V6Z M42 12V18H54V12H42Z" fill="#6f6f6f" fill-rule="evenodd"/>
  <path d="M66 6H90V12H72V30H84V24H78V18H90V36H66V6Z" fill="#e5e5e5"/>
  <path d="M96 6H120V12H102V18H120V24H102V30H120V36H96V6Z" fill="#e5e5e5"/>
</svg>`
