...
export function fileLogger(file = path.join(Global.Path.log, "mage.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.MAGE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.MAGE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
