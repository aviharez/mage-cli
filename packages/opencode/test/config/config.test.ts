...
import { EffectFlock } from "@mybcabisnis/mage-shared/util/effect-flock"
...
async function writeManagedSettings(settings: object, filename = "mage.json") {
...
test("ignores legacy tui keys in mage config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://mage.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "mage.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["MAGE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { mage: { options: { apiKey: "{env:MAGE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["mage"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["MAGE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["MAGE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "mage.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .mage directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mage")
...
test("loads agents from .mage/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mage")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .mage/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mage")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .mage/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".mage")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = await Filesystem.readJson<{ model: string }>(path.join(tmp.path, "mage.json"))
...
test("does not try to install dependencies in read-only MAGE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.MAGE_CONFIG_DIR
  process.env.MAGE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.MAGE_CONFIG_DIR
    else process.env.MAGE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable MAGE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "mage.json"),
        JSON.stringify({ $schema: "https://mage.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .mage config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".mage")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .mage config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "mage.json"),
        JSON.stringify({
          $schema: "https://mage.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mage config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "mage.json"),
        JSON.stringify({
          $schema: "https://mage.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".mage")
...
test("local .mage config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .mage directory config enables it
      const opencodeDir = path.join(dir, ".mage")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "mage.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/mage")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/mage")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.mage/plugin/oh-my-opencode.js"]
...
describe("MAGE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["MAGE_DISABLE_PROJECT_CONFIG"]
    process.env["MAGE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["MAGE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["MAGE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .mage/ directories when flag is set", async () => {
    const originalEnv = process.env["MAGE_DISABLE_PROJECT_CONFIG"]
    process.env["MAGE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .mage directory with a command
          const opencodeDir = path.join(dir, ".mage", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .mage should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["MAGE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MAGE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["MAGE_CONFIG_DIR"]
      process.env["MAGE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["MAGE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
  ...
  test("MAGE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["MAGE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["MAGE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["MAGE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["MAGE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from MAGE_CONFIG_DIR, not project
...
describe("MAGE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in MAGE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MAGE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["MAGE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://mage.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["MAGE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["MAGE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in MAGE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["MAGE_CONFIG_CONTENT"]
