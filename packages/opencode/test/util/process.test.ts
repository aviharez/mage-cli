...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.MAGE_TEST ?? "")'), {
      env: {
        MAGE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "MAGE_TEST_SHELL"], {
      shell: true,
      env: {
        MAGE_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("MAGE_TEST_SHELL=ok")
  })
