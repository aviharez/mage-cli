...
import { Module } from "@mybcabisnis/mage-shared/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.MAGE_DISABLE_LSP_DOWNLOAD) return
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!Flag.MAGE_EXPERIMENTAL_LSP_TY) {
      return undefined
    }
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "mage-jdtls-data"))
    return {
      process: spawn(
        java,
        [
