...
const Headers = Schema.Struct({
  ...
  "x-mage-directory": Schema.optional(Schema.String),
})
...
const auth = Layer.succeed(
  ...
  Authorization.of({
    basic: (effect, { credential }) =>
      Effect.gen(function* () {
        if (!Flag.MAGE_SERVER_PASSWORD) return yield* effect

        const user = Flag.MAGE_SERVER_USERNAME ?? "mage"
        if (credential.username !== user) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        if (Redacted.value(credential.password) !== Flag.MAGE_SERVER_PASSWORD) {
          return yield* new Unauthorized({ message: "Unauthorized" })
        }
        return yield* effect
      }),
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-mage-directory"] || process.cwd()
