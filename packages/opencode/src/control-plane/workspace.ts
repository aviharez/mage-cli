...
import { Slug } from "@mybcabisnis/mage-shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    MAGE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
...
async function startSync(space: Info) {
  if (!Flag.MAGE_EXPERIMENTAL_WORKSPACES) return
