    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.MAGE_SERVER_PASSWORD
            ? {
              Authorization: `Basic ${Buffer.from(`${Flag.MAGE_SERVER_USERNAME ?? "mage"}:${Flag.MAGE_SERVER_PASSWORD}`).toString("base64")}`,
            }
            : undefined,
        ...
        const plugins = Flag.MAGE_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.MAGE_PURE && cfg.plugin_origins?.length) {
