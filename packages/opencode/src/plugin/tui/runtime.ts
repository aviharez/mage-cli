...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@mybcabisnis/mage-plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@mybcabisnis/mage-core/effect/app-node-builder"
import { errorData, errorMessage } from "@mybcabisnis/mage-tui/util/error"
...
import { hasTheme, upsertTheme } from "@mybcabisnis/mage-tui/context/theme"
...
import { createCommandShim } from "@mybcabisnis/mage-tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@mybcabisnis/mage-tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".mage"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".mage", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".mage", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.MAGE_PURE ? [] : pluginOrigins
    if (Flag.MAGE_PURE && pluginOrigins.length) {
    }
