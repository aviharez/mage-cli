...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run mage",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for mage session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found mage session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["mage", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting mage...")
        UI.println()

        const mageArgs = sessionId ? ["-s", sessionId] : []
        const mageProcess = Process.spawn(["mage", ...mageArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await mageProcess.exited
        if (code !== 0) throw new Error(`mage exited with code ${code}`)
      },
    })
  },
})
