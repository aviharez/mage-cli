...
import { Marketplace, buildMcpConfig, writeMcpToConfig, recordInstall } from "../../marketplace"
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: mage mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in mage.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .mage/ subdirectory too)
  ...
  // Default to mage.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., mage x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        await writeMcpToConfig(name, mcpConfig, configPath)
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        const files = await findMcpConfigFiles(name, Instance.worktree)
        if (files.length === 0) {
          prompts.log.error(
            `"${name}" is not defined in a local config file (managed/remote config); cannot remove`,
          )
          prompts.outro("Done")
          return
        }
        ...
        await AppRuntime.runPromise(
          Effect.gen(function* () {
            const mcp = yield* MCP.Service
            if (yield* mcp.hasStoredTokens(name)) {
              yield* mcp.removeAuth(name)
            }
          }),
        )

        prompts.log.success(`MCP server "${name}" removed`)
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "mage-debug", version: InstallationVersion },
              },
              id: 1,
            }),
          })
