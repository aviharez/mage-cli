...
import mage from "./theme/opencode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@mybcabisnis/mage-plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "mage")
        draft.active = typeof active === "string" ? active : "mage"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.mage, store.mode)
    })
