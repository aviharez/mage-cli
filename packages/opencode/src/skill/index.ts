...
import { SkillPlugin } from "@mybcabisnis/mage-core/plugin/skill"
import { Permission } from "@/permission"
import { FSUtil } from "@mybcabisnis/mage-core/fs-util"
import { Config } from "@/config/config"
import { FrontmatterError } from "@mybcabisnis/mage-core/v1/config/error"
...
// Built-in skill that ships with opencode. The model's intuition for what a
// mage.json should look like is often wrong, and opencode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch opencode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_MAGE_SKILL_NAME = "customize-opencode"
const CUSTOMIZE_MAGE_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating opencode's own configuration: mage.json, mage.jsonc, files under .mage/, or files under ~/.config/mage/. Also use when creating or fixing opencode agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring opencode itself."
const CUSTOMIZE_MAGE_SKILL_BODY = SkillPlugin.CustomizeOpencodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, MAGE_SKILL_PATTERN)
  }
