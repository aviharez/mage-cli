...
import {
  ...
  type JsonSchema,
  type LLMEvent,
} from "@mybcabisnis/mage-llm"
import type { LLMClientShape } from "@mybcabisnis/mage-llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @mybcabisnis/mage-llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @mybcabisnis/mage-llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
