...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@mybcabisnis/mage-sdk/v2"
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Mage stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const mageProvider = providers.find((p) => p.id === "mage")
  if (mageProvider) {
    if (mageProvider.models["big-pickle"]) {
      return { providerID: ProviderID.opencode, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(mageProvider.models))
    if (best) {
      return {
