import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@mybcabisnis/mage-plugin/tui"
...
import type { useMageKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useMageKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getMageModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getMageModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
