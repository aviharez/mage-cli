...
import { registerMageSpinner } from "../register-spinner"
...
import { MAGE_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useMageKeymap } from "../../keymap"
...
export function Prompt(props: PromptProps) {
  ...
  function promptModelWarning() {
    toast.show({
      variant: "warning",
      message: "No model available to send prompts",
      duration: 3000,
    })
  }
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.MAGE_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
                ...
                <Show when={local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <>
                      ...
                      <Show when={store.mode === "normal" && local.permission.mode === "auto" && !local.permission.yolo}>
                        <text fg={fadeColor(theme.textMuted, agentMetaAlpha())}>auto</text>
                      </Show>
                      <Show when={store.mode === "normal" && local.permission.yolo}>
