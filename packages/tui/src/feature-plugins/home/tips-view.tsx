...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("mage.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public mage.ai link",
  ...
  "Create {highlight}mage.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/mage/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.mage/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.mage/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.mage/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.mage/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Mage from reading sensitive files",
  "Use {highlight}mage run{/highlight} for non-interactive scripting",
  "Use {highlight}mage --continue{/highlight} to resume the last session",
  "Use {highlight}mage run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}mage serve{/highlight} for headless API access to Mage",
  "Use {highlight}mage run --attach{/highlight} to connect to a running server",
  "Run {highlight}mage upgrade{/highlight} to update to the latest version",
  "Run {highlight}mage auth list{/highlight} to see all configured providers",
  "Run {highlight}mage agent create{/highlight} for guided agent creation",
  "Use {highlight}/mage{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}mage github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/mage fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.mage/themes/{/highlight} directory",
  ...
  "Run {highlight}mage debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} with Mage Zen for curated, tested models",
