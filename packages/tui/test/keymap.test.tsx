...
import { getMageModeStack, MAGE_BASE_MODE, MageKeymapProvider, registerMageKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerMageKeymap(keymap, renderer, config)
    ...
    return (
      <MageKeymapProvider keymap={keymap}>
        <box />
      </MageKeymapProvider>
    )
  }
...
test("mode-less bindings stay active when mage mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getMageModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getMageModeStack(keymap).push("autocomplete")
