import { SessionMessage } from "@mybcabisnis/mage-schema/session-message"
import { SessionInput } from "@mybcabisnis/mage-schema/session-input"
import { PromptInput } from "@mybcabisnis/mage-schema/prompt-input"
import { Session } from "@mybcabisnis/mage-schema/session"
import { Project } from "@mybcabisnis/mage-schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@mybcabisnis/mage-schema/schema"
import { Workspace } from "@mybcabisnis/mage-schema/workspace"
...
import { Agent } from "@mybcabisnis/mage-schema/agent"
import { Model } from "@mybcabisnis/mage-schema/model"
import { Location } from "@mybcabisnis/mage-schema/location"
import { Revert } from "@mybcabisnis/mage-schema/revert"
import { SessionEvent } from "@mybcabisnis/mage-schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Mage process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Mage process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
