...
/**
 * Scoped override for `Flag.MAGE_WORKSPACE_ID`. Saves the previous value
...
export function withFixedWorkspaceID(id: WorkspaceV2.ID): Effect.Effect<void, never, Scope.Scope> {
  return Effect.gen(function* () {
    const previous = Flag.MAGE_WORKSPACE_ID
    Flag.MAGE_WORKSPACE_ID = id
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.MAGE_WORKSPACE_ID = previous
      }),
    )
  })
}
