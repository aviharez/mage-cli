...
import { Hash } from "@mybcabisnis/mage-core/util/hash"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const mageFile = path.join(tmp, ".git", "mage")
      expect(yield* Effect.promise(() => Bun.file(mageFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "mage")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "mage")
      const wrongCache = path.join(parentDir, ".git", "mage")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "mage")
      const cacheB = path.join(bareB, "mage")
      const wrongCache = path.join(parentDir, ".git", "mage")
