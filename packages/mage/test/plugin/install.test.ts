...
describe("plugin.install.task", () => {
  test("writes both server and tui config entries", async () => {
    ...
    const server = await read(path.join(tmp.path, ".mage", "mage.jsonc"))
  ...
  test("preserves JSONC comments when adding plugins to server and tui config", async () => {
    ...
    const server = path.join(cfg, "mage.jsonc")
...
`,
  ...
  test("preserves JSONC comments when force replacing plugin version", async () => {
    ...
    const cfg = path.join(tmp.path, ".mage", "mage.jsonc")
...
`,
  ...
  test("writes to global scope when global flag is set", async () => {
    ...
    expect(await Filesystem.exists(path.join(global, "mage.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(tmp.path, ".mage", "mage.jsonc"))).toBe(false)
  })
  ...
  test("writes local scope under directory when vcs is not git", async () => {
    ...
    expect(ok).toBe(true)
    expect(await Filesystem.exists(path.join(directory, ".mage", "mage.jsonc"))).toBe(true)
    expect(await Filesystem.exists(path.join(worktree, ".mage", "mage.jsonc"))).toBe(false)
  })
  ...
  test("force replaces version in both server and tui configs", async () => {
    await using tmp = await tmpdir()
    const target = await plugin(tmp.path, ["server", "tui"])
    const server = path.join(tmp.path, ".mage", "mage.json")
