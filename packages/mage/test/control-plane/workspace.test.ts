...
import { ProjectTable } from "@mybcabisnis/mage-core/project/sql"
import { AbsolutePath } from "@mybcabisnis/mage-core/schema"
import { Session as SessionNs } from "@/session/session"
import { SessionID } from "@/session/schema"
import { SessionTable } from "@mybcabisnis/mage-core/session/sql"
import { SessionProjector } from "@mybcabisnis/mage-core/session/projector"
import { EventSequenceTable } from "@mybcabisnis/mage-core/event/sql"
...
import { WorkspaceV2 } from "@mybcabisnis/mage-core/workspace"
import { WorkspaceTable } from "@mybcabisnis/mage-core/control-plane/workspace.sql"
...
const originalEnv = {
  MAGE_AUTH_CONTENT: process.env.MAGE_AUTH_CONTENT,
  MAGE_EXPERIMENTAL_WORKSPACES: process.env.MAGE_EXPERIMENTAL_WORKSPACES,
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@mage.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.MAGE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=mage-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.MAGE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.MAGE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.MAGE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=mage-test")
