...
`,
  ...
  const pure = process.env.MAGE_PURE
  const meta = process.env.MAGE_PLUGIN_META_FILE
  process.env.MAGE_PURE = "1"
  process.env.MAGE_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.MAGE_PURE
    } else {
      process.env.MAGE_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.MAGE_PLUGIN_META_FILE
    } else {
      process.env.MAGE_PLUGIN_META_FILE = meta
    }
  }
})
