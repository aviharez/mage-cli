...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ mage }) =>
    Effect.gen(function* () {
      const result = yield* mage.spawn(["--replay-limit", "10"])

      mage.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ mage }) =>
    Effect.gen(function* () {
      const result = yield* mage.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ mage }) =>
    Effect.gen(function* () {
      const result = yield* mage.spawn(["--mini", "--port", "4096"])
