...
describe("mage read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // MAGE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["mcp", "list"])
        mage.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["providers", "list"])
        mage.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["models"])
        mage.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["agent", "list"])
        mage.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh MAGE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["session", "list"])
        mage.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["stats"])
        mage.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under MAGE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ mage }) =>
      Effect.gen(function* () {
        const r = yield* mage.spawn(["db", "path"])
        mage.expectExit(r, 0, "db path")
