...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/mage.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/mage.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/mage.jsonc", dir: ".mage", suggestion: "mage" },
        expected:
          'Directory ".mage" in /tmp/mage.jsonc is not valid. Rename the directory to "mage" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/mage.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
