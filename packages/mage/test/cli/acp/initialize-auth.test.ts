...
describe("mage acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ mage }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ mage }))
        ...
        expect(initialized.agentInfo?.name).toBe("Mage")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ mage })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("mage-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "mage-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.MAGE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("mage-login")
