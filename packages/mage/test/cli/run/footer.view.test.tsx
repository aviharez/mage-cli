...
import type { QuestionRequest } from "@mybcabisnis/mage-sdk/v2"
import { MageKeymapProvider, registerMageKeymap } from "@mybcabisnis/mage-tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerMageKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerMageKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "mage", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerMageKeymap(keymap, renderer, tuiConfig)
