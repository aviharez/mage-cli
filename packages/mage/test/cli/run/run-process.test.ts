// Subprocess integration tests for `mage run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `mage.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `MAGE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("mage run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mage.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mage.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* mage.run("think", { extraArgs: ["--thinking"] })
        mage.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* mage.run("think again")
        mage.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mage.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mage.run("say hi", { format: "json" })
        mage.expectExit(result, 0)

        const events = mage.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ mage }) =>
      Effect.gen(function* () {
        const result = yield* mage.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mage.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        ...
        const result = yield* mage.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, mage }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* mage.run("request permission", { permission: { bash: "ask" } })
        mage.expectExit(denied, 0)
        ...
        const allowed = yield* mage.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        mage.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* mage.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        mage.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, mage }) =>
      Effect.gen(function* () {
        ...
        const server = yield* mage.serve()

        const result = yield* mage.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, mage }) =>
      Effect.gen(function* () {
        const result = yield* mage.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, mage }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* mage.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
