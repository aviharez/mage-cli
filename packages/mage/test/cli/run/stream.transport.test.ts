...
import { MageClient, type GlobalEvent } from "@mybcabisnis/mage-sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<MageClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<MageClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<MageClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<MageClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<MageClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://mage.test"),
...
function sdk(
  input: {
    ...
    subscribe?: MageClient["event"]["subscribe"]
    globalEvent?: MageClient["global"]["event"]
    promptAsync?: MageClient["session"]["promptAsync"]
    status?: MageClient["session"]["status"]
    messages?: MageClient["session"]["messages"]
    children?: MageClient["session"]["children"]
    permissions?: MageClient["permission"]["list"]
    questions?: MageClient["question"]["list"]
  } = {},
) {
  const client = new MageClient()

  const subscribe: MageClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: MageClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: MageClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: MageClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: MageClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: MageClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: MageClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: MageClient["question"]["list"] = input.questions ?? (() => ok([]))
