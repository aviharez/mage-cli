// Subprocess test harness for the mage CLI. Spawns the real binary against
...
//
// Configuration flows through mage's built-in test affordances:
//   - MAGE_CONFIG_CONTENT      : provider config inline, no files to find
//   - MAGE_TEST_HOME           : pins os.homedir() → tmpdir
//   - MAGE_DISABLE_PROJECT_CONFIG : skip walking up for mage.json
//   - MAGE_PURE                : skip external plugin discovery + install
//   - MAGE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `mage.run` is fully wired. The shape supports adding more
// builders (`mage.serve(opts)`, `mage.acp(opts)`, `mage.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of MageCli.
...
const mageRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(mageRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    MAGE_PURE: "1",
    MAGE_DISABLE_AUTOUPDATE: "1",
    MAGE_DISABLE_AUTOCOMPACT: "1",
    MAGE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `mage run`. New flags should
...
// `mage serve` is a long-lived process — it never exits on its own.
...
// `mage acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type MageCli = {
  ...
  // Spawn `mage serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `mage acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("mage.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          MAGE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("mage.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("mage.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "mage server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `mage serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("mage.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const mage: MageCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, mage })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `mage.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "mage") {
...
// every call site. Use this for any test that needs the mage CLI fixture.
//
...
// resources (e.g. `mage.serve`) without an extra `Effect.scoped` wrapper —
