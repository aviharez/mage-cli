...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "mage.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "mage.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mage.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "mage.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["mage.json", "mage.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "mage.json"),
        path.join(tmp.path, "mage.jsonc"),
        path.join(project, "mage.json"),
        path.join(project, "mage.jsonc"),
      ])
    })
  })
