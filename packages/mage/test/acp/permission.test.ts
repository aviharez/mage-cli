...
import type { Event, MageClient } from "@mybcabisnis/mage-sdk/v2"
...
type PermissionReplyParams = Parameters<MageClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "mage-acp-permission-"))
