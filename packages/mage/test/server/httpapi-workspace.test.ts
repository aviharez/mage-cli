...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-mage-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-mage-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://mage.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-mage-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-mage-workspace")
