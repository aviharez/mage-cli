...
const original = {
  ...
  envPassword: process.env.MAGE_SERVER_PASSWORD,
  envUsername: process.env.MAGE_SERVER_USERNAME,
}
const auth = { username: "mage", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.MAGE_SERVER_PASSWORD = original.MAGE_SERVER_PASSWORD
  Flag.MAGE_SERVER_USERNAME = original.MAGE_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.MAGE_SERVER_PASSWORD
  else process.env.MAGE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.MAGE_SERVER_USERNAME
  else process.env.MAGE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.MAGE_SERVER_PASSWORD = auth.password
  Flag.MAGE_SERVER_USERNAME = auth.username
  process.env.MAGE_SERVER_PASSWORD = auth.password
  process.env.MAGE_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-mage-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-mage-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-mage-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    await using tmp = await tmpdir({
      init: async (directory) => {
        ...
        await Bun.write(
          path.join(directory, "mage.json"),
          JSON.stringify({ formatter: false, lsp: false, plugin: [pathToFileURL(plugin).href] }),
        )
        return { initialized, completed }
      },
    })
    const previous = process.env.MAGE_DISABLE_DEFAULT_PLUGINS
    process.env.MAGE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.MAGE_DISABLE_DEFAULT_PLUGINS
      else process.env.MAGE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-mage-ticket": "1" },
      })
