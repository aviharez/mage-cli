...
type Sdk = ReturnType<typeof createMageClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.MAGE_SERVER_PASSWORD = input?.password
      Flag.MAGE_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".mage", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-mage-directory")).toBe(false)
        expect(request!.headers.has("x-mage-workspace")).toBe(false)
      }),
    ),
  )
