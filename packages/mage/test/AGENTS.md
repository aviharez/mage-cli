...
- `config?: Partial<Config.Info>` - Write an `mage.json` config file
...
- Directories are created in the system temp folder with prefix `mage-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("mage-test-")
  }),
)
```
