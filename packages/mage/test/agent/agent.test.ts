...
it.instance("plan agent denies edits except .mage/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".mage/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".mage", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.MAGE_TEST_HOME
      process.env.MAGE_TEST_HOME = test.directory
