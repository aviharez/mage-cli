...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.MAGE_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.MAGE_TEST_HOME = prev
      }),
  )
...
describe("skill", () => {
  ...
  it.live("discovers skills from .mage/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".mage", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".mage", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".mage", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .mage/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".mage", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".mage", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".mage", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".mage", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".mage", "skill", "mage-skill", "SKILL.md"),
                `---
name: mage-skill
description: A skill in the .mage/skill directory.
---
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["mage-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              Bun.write(
                path.join(dir, ".mage", "skill", "agent-skill", "SKILL.md"),
                `---
name: mage-skill
description: A skill in the .mage/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".mage", "skills", "agent-skill", "SKILL.md"),
                `---
name: mage-skill
description: A skill in the .mage/skills directory.
---
