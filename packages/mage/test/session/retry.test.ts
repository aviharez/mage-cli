...
import type { NamedError } from "@mybcabisnis/mage-core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "mage")).toEqual({
      message: SessionRetry.GO_UPSELL_MESSAGE,
      action: {
        reason: "free_tier_limit",
        provider: "mage",
        title: "Free limit reached",
        message: "Subscribe to Mage Go for reliable access to the best open-source models, starting at $5/month.",
        label: "subscribe",
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "mage-go")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://mage.ai/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      action: {
        ...
        label: "open settings",
        link: "https://mage.ai/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      },
    })
  })
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "mage-go")?.action?.message).toBe(
