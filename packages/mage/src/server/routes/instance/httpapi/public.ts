...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for mage serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(MageHttpApi)
...
export const PublicApi = MageHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "mage",
    version: "1.0.0",
