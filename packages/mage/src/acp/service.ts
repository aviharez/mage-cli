...
import type { AssistantMessage, Message, MageClient, SessionMessageResponse } from "@mybcabisnis/mage-sdk/v2"
...
export const AuthMethodID = "mage-login"
...
export class Service extends Context.Service<Service, Interface>()("@mage/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `mage auth login` in the terminal",
      name: "Login with mage",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: MageClient) {
...
function makeUsageService(sdk: MageClient) {
...
async function loadDirectorySnapshot(sdk: MageClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, mage provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const mageProvider = providers[ProviderV2.ID.make("mage")]
  const mageModel = mageProvider ? Provider.sort(Object.values(mageProvider.models))[0] : undefined
  if (mageProvider && mageModel) return { providerID: mageProvider.id, modelID: mageModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Mage service failure", service })
}
