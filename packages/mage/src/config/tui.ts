...
import { isRecord } from "@mybcabisnis/mage-tui/util/record"
...
import { TuiKeybind } from "@mybcabisnis/mage-tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@mage/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old mage.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.mage`
  // folders between cwd and home, and MAGE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.MAGE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit MAGE_TUI_CONFIG override, if set.
  if (Flag.MAGE_TUI_CONFIG) {
    const configFile = Flag.MAGE_TUI_CONFIG
  ...
  // 4. `.mage` directories (and MAGE_CONFIG_DIR) discovered while
