...
const TUI_SCHEMA_URL = "https://mage.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from mage.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const mage = await mageFiles(input)
  for (const file of mage) {
...
async function mageFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "mage"),
    ...(await Filesystem.findUp(["mage.json", "mage.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "mage"))
  }
  if (Flag.MAGE_CONFIG) files.push(Flag.MAGE_CONFIG)
