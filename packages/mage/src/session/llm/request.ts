...
const USER_AGENT = `mage/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const mageProjectID = input.model.providerID.startsWith("mage")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("mage")
        ? {
            ...(mageProjectID ? { "x-mage-project": mageProjectID } : {}),
            "x-mage-session": input.sessionID,
            "x-mage-request": input.user.id,
            "x-mage-client": input.flags.client,
