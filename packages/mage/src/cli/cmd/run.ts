import type { PermissionV1 } from "@mybcabisnis/mage-core/v1/permission"
import { FSUtil } from "@mybcabisnis/mage-core/fs-util"
// CLI entry point for `mage run` and `mage --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`mage --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`mage --mini --attach`): connects to a running
//      mage server and runs interactive mode against it.
//
...
import { createMageClient, type MageClient, type ToolPart } from "@mybcabisnis/mage-sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<MageClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run mage with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running mage server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to MAGE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to MAGE_SERVER_USERNAME or 'mage')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: MageClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: MageClient, sessionID: string) {
      ...
      async function current(sdk: MageClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: MageClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: MageClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: MageClient) {
        ...
        async function loop(client: MageClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createMageClient({
        baseUrl: "http://mage.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
