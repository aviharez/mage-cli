// Shared type vocabulary for the direct interactive mode (`mage --mini`).
//
...
import type { MageClient, PermissionRequest, QuestionRequest, ToolPart } from "@mybcabisnis/mage-sdk/v2"
import type { TuiConfig } from "@mybcabisnis/mage-tui/config"
...
type PromptModel = Parameters<MageClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<MageClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<MageClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<MageClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<MageClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<MageClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<MageClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<MageClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<MageClient["question"]["reject"]>[0]
