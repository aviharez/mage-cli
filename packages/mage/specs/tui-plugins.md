...
- Author package entrypoint is `@mybcabisnis/mage-plugin/tui`.
...
- Import types from `@mybcabisnis/mage-plugin/tui`.
- `@mybcabisnis/mage-plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@mybcabisnis/mage-plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@mybcabisnis/mage-plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@mybcabisnis/mage-plugin/tui`.
