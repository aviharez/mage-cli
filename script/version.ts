...
//   - packages/wiki/config.mjs              → version: "..."
//   - packages/wiki/src/content/i18n/id.json  → app.lander.eyebrow (vX.Y.Z)
//   - packages/wiki/src/components/Lander.astro → TERM_SCRIPT sys line
...
// ── 2. packages/wiki/config.mjs → version: "..." ──────────────────────────────
const configPath = path.join(root, "packages/wiki/config.mjs")
...
if (oldConfigMatch) {
  ...
  console.log(`  ✓ packages/wiki/config.mjs  ${oldConfigMatch[1]} → ${targetVersion}`)
} else {
  console.warn(`  ⚠ packages/wiki/config.mjs — version field not found, skipped`)
}

// ── 3. id.json → app.lander.eyebrow ──────────────────────────────────────────
const i18nPath = path.join(root, "packages/wiki/src/content/i18n/id.json")
...
if (oldEyebrowMatch) {
  ...
  console.log(`  ✓ packages/wiki/src/content/i18n/id.json  ${oldFull} → v${targetVersion}`)
} else {
  console.warn(`  ⚠ packages/wiki/src/content/i18n/id.json — app.lander.eyebrow not found, skipped`)
}

// ── 4. Lander.astro → TERM_SCRIPT sys line ───────────────────────────────────
const landerPath = path.join(root, "packages/wiki/src/components/Lander.astro")
...
if (oldLanderMatch) {
  ...
  console.log(`  ✓ packages/wiki/src/components/Lander.astro  ${oldFull} → mage v${targetVersion}`)
} else {
  console.warn(`  ⚠ packages/wiki/src/components/Lander.astro — version pattern not found, skipped`)
}
